...
/**
 * For unified workspace Node builds, write and return a custom Nitro entry
 * that wraps the `node-middleware` preset entry with per-app scope init.
 *
 * The generated `dist/server.mjs` dispatcher imports every app's server
 * bundle into ONE Node process, so per-app module graphs need the same
 * `setGlobalScopeId` + `setModuleGraphEnvDefaults` treatment the unified
 * Cloudflare worker entry gets (see `generateScopeInitSource`): globalThis
 * registries stay per-app, and per-app identity env (app id, base path,
 * audience, route access) is delivered per module graph instead of through
 * the shared `process.env`, where the first app's values would poison every
 * sibling.
 *
 * ESM evaluates imports depth-first in declaration order, so importing the
 * scope-init module FIRST guarantees the scope id is set before any registry
 * module in this bundle initializes or registers built-ins.
 *
 * Returns null outside unified workspace node builds (single-app node builds
 * keep Nitro's stock entry and behavior).
 */
export function workspaceNodeMiddlewareEntry(
  targetPreset: string,
  appBasePath: string,
): string | null {
  if (targetPreset !== "node-middleware") return null;
  if (process.env.AGENT_NATIVE_WORKSPACE !== "1") return null;
  const appScopeId = workspaceAppScopeIdFromBasePath(appBasePath);
  if (!appScopeId) return null;

  // Resolve Nitro's real node-middleware runtime entry from core's own
  // module graph (apps do not depend on nitro directly under pnpm).
  const req = createRequire(import.meta.url);
  const nitroPkgDir = path.dirname(req.resolve("nitro/package.json"));
  const realEntry = path.join(
    nitroPkgDir,
    "dist",
    "presets",
    "node",
  ...
  if (!fs.existsSync(realEntry)) {
    throw new Error(
      `Nitro node-middleware entry not found at ${realEntry}. ` +
        `The installed nitro version may have moved its preset runtimes.`,
    );
  }

  const tmpDir = path.join(cwd, ".deploy-tmp", "workspace-node");
  ...
  fs.writeFileSync(
    entryFile,
    `// AUTO-GENERATED by @agent-native/core deploy build (workspace node entry)
// Scope-init MUST be the first import: see workspaceNodeMiddlewareEntry.
import "./_scope-init.mjs";
export * from ${JSON.stringify(realEntry.split(path.sep).join("/"))};
`,
  );
  return entryFile;
}
...
`;
  ...
  // Unified workspace Node builds (`agent-native deploy --preset node`):
  // every app's bundle is imported into ONE Node process by the generated
  // dist/server.mjs dispatcher, so this app's module graph needs the same
  // per-app global-registry scoping + module-graph env defaults the unified
  // Cloudflare worker gets. Wrap Nitro's node-middleware entry in a custom
  // entry whose FIRST import is a generated scope-init module — ESM evaluates
  // imports depth-first in order, so the scope id is set before any registry
  // module in this bundle initializes. Per-app identity keys must never touch
  // the shared process.env (first app would poison every sibling).
  const workspaceNodeEntry = workspaceNodeMiddlewareEntry(preset, appBasePath);

  const nitro = await createNitro({
    rootDir: cwd,
    dev: false,
    preset,
    ...(workspaceNodeEntry ? { entry: workspaceNodeEntry } : {}),
    baseURL: appBasePath || "/",
    minify: true,
    serverDir: "./server",
