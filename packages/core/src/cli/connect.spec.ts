...
describe("writeConfigs", () => {
  it("writes a JSON HTTP entry for claude-code (project scope)", () => {
    ...
    expect(cfg.mcpServers["agent-native-mail"]).toEqual({
      type: "http",
      url: "https://mail.jami.studio/mcp",
  ...
  it("writes a Codex TOML block with HTTP url + auth header", () => {
    ...
    try {
      ...
      expect(toml).toContain('url = "https://mail.jami.studio/mcp"');
...
describe("runConnect", () => {
  ...
  it("warns gracefully and does not fail connect when the supplemental publish-token mint fails", async () => {
    ...
    const fetchImpl = vi.fn(async (url: string) => {
      if (String(url).endsWith("/.well-known/oauth-protected-resource")) {
        return new Response(
          JSON.stringify({
            resource: "https://plan.jami.studio/mcp",
          }),
          { status: 200, headers: { "content-type": "application/json" } },
        );
      }
  ...
  it("writes OAuth-native Claude Code entries after validating metadata", async () => {
    ...
    const fetchImpl = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      return new Response(
        JSON.stringify({
          resource: "https://mail.jami.studio/mcp",
        }),
        { status: 200, headers: { "content-type": "application/json" } },
      );
    }) as unknown as typeof fetch;
  ...
  it("normalizes a legacy MCP URL without losing its app base path", async () => {
    ...
    const fetchImpl = vi.fn(async (url: string) => {
      expect(String(url)).toBe(
        "https://mail.jami.studio/mail/.well-known/oauth-protected-resource",
      );
      return new Response(
        JSON.stringify({
          resource: "https://mail.jami.studio/mail/_agent-native/mcp",
        }),
        { status: 200, headers: { "content-type": "application/json" } },
      );
    }) as unknown as typeof fetch;
    ...
    await runConnect(
      [
        "https://mail.jami.studio/mail/_agent-native/mcp",
        "--client",
        "claude-code",
        "--scope",
        "project",
    ...
    expect(cfg.mcpServers["agent-native-mail"]).toEqual({
      type: "http",
      url: "https://mail.jami.studio/mail/mcp",
    });
  });
  ...
  it("prompts for hosted apps when no URL is provided", async () => {
    ...
    expect(cfg.mcpServers["agent-native-calendar"]).toEqual({
      type: "http",
      url: "https://calendar.jami.studio/mcp",
      headers: {
