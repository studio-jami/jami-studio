...
describe("session replay", () => {
  ...
  it("stops the recorder after a definitive 4xx upload rejection", async () => {
    // A permanently-misconfigured ingest endpoint (e.g. missing storage
    // credentials -> HTTP 400 on every chunk) must not retry every 5s forever
    // from every open tab: a definitive 4xx drops the rejected batch and ends
    // the recording episode instead.
    const { fetchMock } = installBrowser("https://app.jami.studio/inbox");
    vi.stubGlobal("CompressionStream", undefined);
    fetchMock.mockImplementation((input: unknown) => {
      if (String(input).includes("/_agent-native/auth/session")) {
    ...
    const replayUploadCalls = () =>
      fetchMock.mock.calls.filter((call: unknown[]) =>
        String(call[0]).includes("/api/analytics/replay"),
      ).length;
    recordOptions.emit({ type: 3, data: { href: "/first" } });
    await waitForAssertion(() => expect(replayUploadCalls()).toBe(1));
    await waitForAssertion(() =>
      expect(warn).toHaveBeenCalledWith(
        "[session-replay] dropping upload (HTTP 400)",
        expect.any(Error),
      ),
    );
    ...
    // The recording episode is over: further flush attempts never hit the
    // replay endpoint again.
    const uploadsAfterStop = replayUploadCalls();
    await flushSessionReplay("retry");
    await flushSessionReplay("manual");
    await tick();
    expect(replayUploadCalls()).toBe(uploadsAfterStop);
  });
