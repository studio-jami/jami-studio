...
export async function flushSessionReplay(reason = "manual"): Promise<void> {
  ...
  try {
    ...
    if (splitBatch) {
      ...
      restoreReplayEvents(state, events);
      // Any-cause failure accounting (network errors, 5xx, not-yet-exhausted
      // transient 4xx): exponential backoff for automatic retries, and a
      // circuit breaker so a permanently-failing endpoint costs one warning
      // instead of an endless retry storm from every open tab.
      state.uploadFailureCount += 1;
      state.uploadRetryNotBeforeMs =
        Date.now() + replayUploadBackoffDelayMs(state.uploadFailureCount);
      if (state.uploadFailureCount >= UPLOAD_CONSECUTIVE_FAILURE_LIMIT) {
