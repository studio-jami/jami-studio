...
- **JSON-RPC** — Jami Studio apps use `POST /_agent-native/a2a`; external/legacy peers may use `POST /a2a`
...
For Jami Studio workspaces, prefer the `agentNative` helper when code or a
...
dataset, or workflow. In production Jami Studio apps, set `A2A_SECRET` in each
...
Set `A2A_SECRET` on every production app that calls or receives A2A traffic. Jami Studio callers sign JWT bearer tokens with this secret so receivers can verify the caller identity before the agent loop starts.
