...
The defining idea behind Jami — just another machine interface — is that the agent and the UI are **equal partners**, two ways into the same machine surface — see [What Is Jami?](/docs/what-is-jami) for the why. This section is about the other half of that promise: what it feels like to actually work with the agent once it's docked next to your app.
