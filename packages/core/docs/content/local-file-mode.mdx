...
description: "Run Jami Studio apps with local Markdown, MDX, and other repo files as the source of truth - including Obsidian-style MDX docs with custom components."
---

# Local File Mode

Local File Mode lets a Jami Studio app attach its normal UI and action surface
...
also be edited by Codex, Claude Code, the Jami Studio sidebar agent, or a normal
editor.
...
or Jami Studio Desktop bridge exposes them. If no bridge is running, the skill
...
plain hosted browser folder access. If you open `content.jami.studio`,
...
Content locally or use Jami Studio Desktop so the trusted local bridge can
