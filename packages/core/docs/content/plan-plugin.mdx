...
> Note: the bare `npx skills@latest add studio-jami/jami-studio --skill visual-plan` (Vercel/open Skills CLI) installs **instructions only** — it does not register the MCP connector. Use the Jami Studio CLI above when you want the connector wired up too.

### Claude Code (plugin) {#claude-code}

The public `studio-jami/jami-studio` repo is itself a Claude Code plugin marketplace, so you add it directly — no build step. Inside Claude Code:
...
**No submission or review is required to distribute or install this.** `studio-jami/jami-studio` is a self-hosted, public git marketplace, so users add it directly with the commands above on **both Claude Code and Codex** — no application or approval. The universal CLI route needs no marketplace at all.
