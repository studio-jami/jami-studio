...
description: "Install the Jami Studio Plan skills (/visual-plan, /visual-recap, /visualize-repo) plus the hosted Plan MCP connector as a Claude Code or Codex plugin, or with the universal CLI. How updates work and whether you need to submit anything."
---

# Plan plugin & marketplace

The Jami Studio **Plan** app ships as one installable bundle. A single install adds the Plan slash-command skills **and** wires up the hosted Plan MCP connector, so the agent can generate plans and the skills can publish them straight into the Plan app.
...
- **The Plan MCP connector** — registered against the hosted app at `https://plan.jami.studio` (MCP endpoint `https://plan.jami.studio/_agent-native/mcp`, server name `plan`).
...
This keeps plan content out of the Jami Studio Plan database. Hosted sharing,
...
Jami Studio Desktop has a separate local-file sync path for hosted plans: the
...
> The plugin (`agent-native-visual-plans`) carries app id `visual-plans`, which is why the Claude Code plugin name and Codex plugin name are both `agent-native-visual-plans`. The Plan app's display name is "Jami Studio Plan".
...
Works for any host — Claude Code, Codex, Cursor, Cline, Goose, ChatGPT custom MCP apps, Claude Cowork, and anything else MCP-compatible. The Jami Studio CLI installs the Plan skills, registers the hosted Plan MCP connector, **and runs auth for the selected local client(s) in the same step**, so your first tool call does not hit an OAuth wall:
...
- `--no-connect` — register the connector without authenticating; run `npx @agent-native/core@latest connect https://plan.jami.studio --client all` later, or choose a narrower `--client`.
...
> Note: the bare `npx skills@latest add Jami Studio/agent-native --skill visual-plan` (Vercel/open Skills CLI) installs **instructions only** — it does not register the MCP connector. Use the Jami Studio CLI above when you want the connector wired up too.

### Claude Code (plugin) {#claude-code}

The public `Jami Studio/agent-native` repo is itself a Claude Code plugin marketplace, so you add it directly — no build step. Inside Claude Code:
...
After install, **start a new Codex thread** so the skills and MCP tools load into the session. The plugin ships a URL-only connector (`[mcp_servers.plan]` → `https://plan.jami.studio/_agent-native/mcp`); `codex mcp login plan` runs the OAuth flow. The universal CLI route above also works for Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`) if you prefer one command that installs and authenticates together, or when you want local-files or self-hosted mode.

<Callout tone="info">

**Older installs:** if your config still has an `agent-native-plans` entry pointing at the same URL, running `npx -y @agent-native/core@latest reconnect https://plan.jami.studio --client codex` for Codex, or the same command with your target `--client`, consolidates it to the canonical `plan` name.
...
**No submission or review is required to distribute or install this.** `Jami Studio/agent-native` is a self-hosted, public git marketplace, so users add it directly with the commands above on **both Claude Code and Codex** — no application or approval. The universal CLI route needs no marketplace at all.
