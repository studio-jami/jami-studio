...
- **The Plan MCP connector** — registered against the hosted app at `https://plan.jami.studio` (MCP endpoint `https://plan.jami.studio/mcp`, server name `plan`).
...
After install, **start a new Codex thread** so the skills and MCP tools load into the session. The plugin ships a URL-only connector (`[mcp_servers.plan]` → `https://plan.jami.studio/mcp`); `codex mcp login plan` runs the OAuth flow. The universal CLI route above also works for Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`) if you prefer one command that installs and authenticates together, or when you want local-files or self-hosted mode.
