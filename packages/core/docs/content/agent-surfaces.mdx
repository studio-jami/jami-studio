...
Jami Studio is deliberately composable. Most apps start with chat, add actions,
...
| ----------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| **Rich chat on Jami Studio** | You want a standalone or embedded chat backed by the built-in agent loop.                                   | [Chat template](/docs/template-chat), `<AgentChatSurface>`, `<AssistantChat>`               |
...
| **Rich chat on your agent**   | You built the agent elsewhere and want Jami Studio's composer, transcript, tool cards, and native widgets. | `AgentChatRuntime`, `<AssistantChat runtime={runtime}>`                                     |
...
another agent, or an existing product calling into Jami Studio.
...
- **A2A** — from another Jami Studio app or agent peer
...
Code, Codex, Pi, Cursor, Mastra, or similar runtimes inside a Jami Studio app.
Use them when you are building a coding-agent product, not as the default way to
start a local Jami Studio workflow.
...
## Rich chat on Jami Studio {#rich-chat}
...
runtime and you want Jami Studio's chat UI around it. `AgentChatRuntime` is the
boundary: your runtime streams normalized events, and Jami Studio renders the
...
not the end-user app chat or A2A transport, and Jami Studio does not currently
...
The server plugin mounts Jami Studio routes into your host app and resolves
...
<Diagram id="doc-block-27ug5y" title="How the sidecar bridges to a host app" summary={"The plugin mounts Jami Studio routes server-side; the React sidecar streams page context in and host commands out."}>
...
<div class="diagram-sidecar">
  ...
  <div class="diagram-panel accent-panel">
    ...
    <div class="diagram-box" data-rough>
      Jami Studio routes<br /><small class="diagram-muted"
...
| ---------------------------------------------------------------- | ------------------------- |
| "I want the framework's agent, but chat should be the main UI."  | Rich chat on Jami Studio |
...
- [**A2A Protocol**](/docs/a2a-protocol) — call agents from other Jami Studio apps
