---
title: "FAQ"
description: "Common questions about Jami Studio — what it is, who it's for, what you can build, and how it works."
---

# FAQ

Common questions about Jami Studio, organized from "I'm just looking" to "I'm wiring up auth right now."

## The basics {#general}

### What is Jami? {#what-is-jami}

Jami is short for **Just another machine interface**. Jami Studio is a framework for building apps around one canonical machine surface — typed actions over shared SQL state — where every way in, the human UI and the AI agent alike, is just another interface with equal standing. The usual path starts with chat, adds typed actions, renders structured results inline, and grows into durable pages around the same SQL state. The invariant is that agents and humans share the same actions, database, and state. See [What Is Jami?](/docs/what-is-jami) for the full explanation.

### Who is this for? {#who-is-this-for}

Jami Studio is for people who want a real app and an AI agent to work from the same data and actions. The common paths are:
...
- **Connect another agent or code tool** if you want Claude, ChatGPT, Codex, Cursor, or GitHub Copilot / VS Code to use a Jami Studio app — see [External Agents](/docs/external-agents) and [Skills Guide](/docs/skills-guide).

### How is this different from adding AI to an existing app? {#how-is-this-different}

Most apps bolt AI on as an afterthought that can't actually _do_ things in the app. In a Jami Studio app the agent is a first-class citizen that shares the same actions, database, and state as the UI, so it can do anything the buttons can — and modify the app's own code. See [What Is Jami?](/docs/what-is-jami#the-ladder).

<Diagram id="doc-block-1s6b04" title="Bolted-on AI vs. Jami" summary="A bolted-on chat sidebar lives in its own world. Jami shares the same actions, database, and state as the UI.">
...
<div class="diagram-vs">
  ...
  <div class="diagram-col">
    <span class="diagram-pill ok">Jami</span>
...
Yes. The framework and all templates are open source. You can run everything locally, self-host, or use Jami Studio's cloud for managed hosting, collaboration, and team features.
...
If you'd rather not manage any of this, the hosted version on `jami.studio` (operated by Jami Studio) bundles inference and hosting into a per-seat plan.
...
Yes, via the [A2A (Agent-to-Agent) protocol](/docs/a2a-protocol). Every Jami Studio app automatically gets an A2A endpoint. From the mail app, you can tag the analytics agent to query data. An agent discovers what other agents are available, calls them over the protocol, and shows results in the UI. No configuration needed — the agent card is auto-generated from your template's actions.
...
### Which AI coding tools work with Jami Studio? {#which-ai-tools-work}
...
- **Jami Studio** — cloud-hosted agent with visual editing and collaboration
...
Anywhere. The server runs on Nitro, which compiles to any deployment target: Node.js, Cloudflare Workers/Pages, Netlify, Vercel, Deno Deploy, AWS Lambda, and Bun. You can also use Jami Studio's hosting for managed deployments. See the [Deployment guide](/docs/deployment).
...
### How do I upgrade an older Jami Studio app? {#upgrade-older-app}
...
- [**What Is Jami?**](/docs/what-is-jami) — the full explanation behind the answers above
