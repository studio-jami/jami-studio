...
Conservez le Jami Studio CLI comme chemin de documentation par défaut pour les agents locaux, car il
enregistre également le connecteur MCP. `Jami Studio/agent-native` est un vrai GitHub
...
lieu. Les Jami Studio skills nouvellement copiés incluent un `agent-native-skill.json`
marqueur permettant à la sortie d'état future d'identifier la source et le hachage.

Les applications et espaces de travail Jami Studio générés incluent également le framework fourni
