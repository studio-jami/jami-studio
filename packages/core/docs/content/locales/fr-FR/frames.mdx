...
- **Cadre cloud Jami Studio** : un cadre géré et hébergé avec collaboration,
...
<div class="diagram-frames">
  ...
  <div class="diagram-card" data-rough>
    <span class="diagram-pill">Frame cloud Jami Studio</span
    ><small class="diagram-muted"
...
- **Mode Code** : ajoute les outils de codage partagés (`bash`, `read`, `edit`, `write`)
  ...
  boîte de dialogue expliquant que les modifications de code nécessitent Jami Studio Desktop ou Jami Studio ;
...
<Diagram id="doc-block-xwnfo4" title={"Contrôle de demande de code"} summary={"Un message codé nécessite une trame compatible avec le code. Avec un connecté, la requête y est acheminée ; sans cela, le panneau explique que les modifications de code nécessitent Desktop ou Jami Studio."}>
...
## Cadre nuage Jami Studio {#cloud-frame}

[Jami Studio](https://www.jami.studio) fournit un cadre géré qui héberge le
...
Demandes de code depuis l'itinéraire du panneau intégré vers le cadre Jami Studio de la même manière
...
Le cadre de développement local (le package privé `@agent-native/frame` dans le dépôt du framework) est un package d'outils internes qui n'est pas publié sur npm. Il charge le serveur de développement de l'application active dans une iframe et monte le panneau intégré à côté, en sélectionnant l'application via le paramètre de requête `app`. Le terminal CLI intégré nécessite Jami Studio Desktop, qui fournit le code local et l'accès PTY aux besoins du terminal ; sans cela, le panneau affiche la surface de discussion et vous invite à ouvrir le bureau pour utiliser le CLI.
