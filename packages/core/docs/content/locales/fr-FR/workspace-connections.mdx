...
| `clips`        | recherche, importation, réunions            | cerveau, clips                                   |
...
Dispatch expose des actions de plan de contrôle qui utilisent les mêmes fonctions de stockage partagé qu’une application pourrait appeler directement : son action `upsert-workspace-connection` valide l’entrée par rapport au catalogue des fournisseurs, puis délègue à la même fonction `upsertWorkspaceConnection()` présentée dans cette page :
...
- [**Dispatch**](/docs/dispatch) — l’application qui gère généralement la configuration des connexions de l’espace de travail et le coffre-fort
- [**Sécurité**](/docs/security) — le coffre-fort d’identifiants, la portée des secrets et le modèle d’accès vers lesquels pointent les connexions
- [**Intégration**](/docs/onboarding#checking-workspace-connections-in-onboarding) — vérifier l’état de préparation des connexions depuis la liste de configuration d’un modèle
- [**Clients MCP**](/docs/mcp-clients) — partager les identifiants des serveurs MCP dans l’espace de travail, un modèle voisin de configuration réutilisable
- [**Brain**](/docs/template-brain) — l’implémentation de référence d’une page Sources de fournisseurs basée sur ce modèle
