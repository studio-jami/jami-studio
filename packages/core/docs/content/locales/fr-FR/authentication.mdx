...
| Hébergé par un fournisseur propriétaire (`*.jami.studio`) | Espace de noms isolé par application (chacune possède sa propre base de données d'authentification) ; `COOKIE_DOMAIN=.jami.studio` est ignoré par défaut |
