...
L'idée déterminante derrière l'agent natif est que l'agent et le UI sont des **partenaires égaux** — voir [What Is Jami?](/docs/what-is-jami) pour le pourquoi. Cette section concerne l'autre moitié de cette promesse : ce que l'on ressent en travaillant réellement avec l'agent une fois qu'il est ancré à côté de votre application.
...
**Voix.** Le compositeur a un microphone. Dictez une demande au lieu de la saisir, avec des options de fournisseur allant de la transcription hébergée de Jami Studio à l'apport de votre propre clé en passant par une solution de secours du navigateur.
