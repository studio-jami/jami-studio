...
Chaque application hébergée sur `*.jami.studio` exécute son propre déploiement avec son **propre magasin d'utilisateurs distinct**. `mail.jami.studio` et `calendar.jami.studio` ne partagent pas de base de données, de table de session ou de domaine de cookie. Ainsi, « Connectez-vous une fois, utilisez chaque application » ne peut pas être un cookie partagé : il doit s'agir d'une **fédération d'identité**, avec [Dispatch](/docs/dispatch) agissant en tant qu'autorité d'identité pour l'espace de travail.
...
1. **Application → Dispatch (autorisation).** La route `GET /_agent-native/identity/login` de l’application (celle du bouton « Se connecter avec Jami Studio ») construit la redirection et envoie l’utilisateur à l’autorité d’identité :
...
<Endpoint id="cross-app-sso-identity-authorize-fr-fr" title={"Point de terminaison d'autorisation d'identité"} method="GET" path="/_agent-native/identity/authorize" summary={"Dispatch (autorité d'identité) authentifie l'humain et le redirige avec un jeton d'identité signé"} auth={"Session de répartition (connexion interactive si aucune)"} params={[
  ...
  },
  {
    ...
    "type": "string",
    "required": true,
    "description": "Callback de l’application URL. Validé par rapport à une liste blanche stricte (`*.jami.studio` ou localhost par défaut)."
  },
  {
    "name": "state",
    "in": "query",
...
- **Set** → l'application affiche une option **"Connectez-vous avec Jami Studio"** qui exécute le flux ci-dessus. La connexion locale directe (e-mail/mot de passe, Google) fonctionne toujours en parallèle.
...
Tout déploiement Dispatch peut servir de centre d'identité ; vous n'êtes pas limité à `dispatch.jami.studio`. Définissez `AGENT_NATIVE_IDENTITY_HUB_URL` sur chaque application client pour pointer vers votre instance Dispatch :
...
- **Par défaut :** `*.jami.studio` et localhost uniquement (la constante `DEFAULT_ALLOWED_HOST_SUFFIXES`).
...
Sans `IDENTITY_SSO_ALLOWED_HOST_SUFFIXES`, un Dispatch auto-hébergé ne peut émettre des jetons qu'aux applications sur `*.jami.studio`. Définissez la variable d'environnement sur votre déploiement Dispatch pour déverrouiller d'autres domaines.
...
- L'écran de connexion affiche désormais **"Connectez-vous avec Jami Studio"**. Cliquez dessus.
