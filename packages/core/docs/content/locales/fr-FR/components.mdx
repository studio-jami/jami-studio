...
Jami Studio fournit une barre latérale complète, mais la barre latérale ne constitue pas le contrat. Le
...
| `createAgentChatAdapter()`           | `@agent-native/core/client` ou `/client/chat` | Vous avez besoin du transport Jami Studio SSE intégré comme adaptateur d'interface utilisateur d'assistant de bas niveau. |
...
conversation pendant que Jami Studio conserve le compositeur, la transcription, les fiches outils et
