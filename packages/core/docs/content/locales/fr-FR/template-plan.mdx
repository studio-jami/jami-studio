---
title: "Plans visuels"
description: "Jami Studio Plans transforme le plan de votre agent de codage en un document structuré et consultable : diagrammes, wireframes, code annoté, commentaires et liens de partage. Installez une fois à partir du CLI ; les évaluateurs avec lesquels vous partagez des contenus modifient en tant qu'invité et se connectent uniquement pour enregistrer ou partager."
---
...
Jami Studio Plans est un mode de plan visuel pour les agents de codage. Ça devient un ordinaire
...
Utilisez le Jami Studio CLI. Il s'agit de la configuration recommandée car elle installe le
...
`https://plan.jami.studio/_agent-native/mcp` : voir [MCP Clients](/docs/mcp-clients) pour la configuration spécifique au client.
...
connecteur, utilisez donc le chemin Jami Studio CLI lorsque vous souhaitez la configuration en une seule commande.
...
[Jami Studio Plans extension](https://marketplace.visualstudio.com/items?itemName=Builder.agent-native)
...
et inclut une commande pour exécuter le flux de connexion Jami Studio MCP existant pour VS
...
Le connecteur Plans hébergé se trouve sur `https://plan.jami.studio/_agent-native/mcp`.
...
`https://plan.jami.studio/local-plans/local-<opaque-id>#bridge=http://127.0.0.1:...`.
...
Le mode Fichiers locaux empêche le contenu du plan ou du récapitulatif d'accéder au Jami Studio
...
Jami Studio Desktop offre également aux plans hébergés un pont de dossier local natif. Ceci
...
Ouvrez un plan dans Jami Studio Desktop, utilisez les **Fichiers locaux** actions du menu Plan,
puis :
...
- Application : `https://plan.jami.studio`
- MCP: `https://plan.jami.studio/_agent-native/mcp`
...
- [**Skills**](/docs/skills-guide) — comment Jami Studio installe skills
