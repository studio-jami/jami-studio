...
`https://plan.jami.studio/mcp` : voir [MCP Clients](/docs/mcp-clients) pour la configuration spécifique au client.
...
- Le modèle de chat par défaut de l'application est Sonnet 5 ; changez-le à tout moment dans les **Paramètres**. Haiku est l'option Claude moins chère, et Luna est l'option moins chère de la famille GPT-5.6.
...
Le connecteur Plans hébergé se trouve sur `https://plan.jami.studio/mcp`.
...
- MCP: `https://plan.jami.studio/mcp`
