...
Pour l'installer, copiez le fichier de [studio-jami/jami-studio](https://github.com/studio-jami/jami-studio/blob/main/.github/workflows/pr-visual-recap-fork.yml) dans le répertoire `.github/workflows/` de votre dépôt aux côtés du `pr-visual-recap.yml` existant. Les mêmes secrets (`PLAN_RECAP_TOKEN`, `ANTHROPIC_API_KEY`) s'appliquent.
...
L'option **réutilisable** écrit à la place un appelant mince d'environ 20 lignes. Il délègue à `studio-jami/jami-studio/.github/workflows/pr-visual-recap-reusable.yml` via `uses:`. Chaque appelant récupère automatiquement la dernière logique lors de l'exécution du workflow, sans qu'aucune mise à jour locale ne soit nécessaire.
...
# Appelant léger : la logique complète du flux de travail réside dans studio-jami/jami-studio.
