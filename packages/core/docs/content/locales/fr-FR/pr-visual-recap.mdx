...
Lorsque vous installez des plans de manière interactive, le Jami Studio CLI vous demande s'il faut ajouter
...
`recap doctor` rejette les données JSON mal formées ou non prises en charge et signale lorsque GitHub ne présente aucun runner en ligne correspondant à toutes les étiquettes configurées. L'enregistrement, l'installation, les mises à niveau, la capacité et la maintenance du runner restent sous votre responsabilité ; Jami Studio sélectionne les étiquettes, mais ne fournit ni n'exploite le runner.
...
| `PLAN_RECAP_APP_URL`     | `https://plan.jami.studio`    | Secret. Uniquement lors de l'auto-hébergement de l'application Plans à une origine différente.                                                                  |
...
Pour l'installer, copiez le fichier de [Jami Studio/agent-native](https://github.com/BuilderIO/agent-native/blob/main/.github/workflows/pr-visual-recap-fork.yml) dans le répertoire `.github/workflows/` de votre dépôt aux côtés du `pr-visual-recap.yml` existant. Les mêmes secrets (`PLAN_RECAP_TOKEN`, `ANTHROPIC_API_KEY`) s'appliquent.
...
récapitulatif sans envoyer le contenu du récapitulatif à la base de données du pla Jami Studio, exécutez le
...
L'option **réutilisable** écrit à la place un appelant mince d'environ 20 lignes. Il délègue à `Jami Studio/agent-native/.github/workflows/pr-visual-recap-reusable.yml` via `uses:`. Chaque appelant récupère automatiquement la dernière logique lors de l'exécution du workflow, sans qu'aucune mise à jour locale ne soit nécessaire.
...
# Appelant léger : la logique complète du flux de travail réside dans Jami Studio/agent-native.
