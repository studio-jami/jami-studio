...
- **Votre propre environnement d'exécution derrière le chat Jami Studio :** consultez [Agent Surfaces](/docs/agent-surfaces#byo-agent) et [Native Interface de chat](/docs/native-chat-ui#byo-agent-runtimes) lorsqu'un agent créé avec un autre framework devrait alimenter `<AssistantChat runtime={...}>`.
...
Ajoutez un connecteur MCP distant à l'hôte sur lequel vous souhaitez utiliser Jami Studio.
...
**Connectez-vous**, connectez-vous avec votre compte Jami Studio, approuvez les scopes MCP,
...
analysez/découvrez les outils, connectez-vous avec Jami Studio, approuvez les étendues et activez
...
L'application propriétaire skills installe les instructions et le connecteur MCP hébergé avec le Jami Studio CLI :
...
besoin d'un connecteur tel que `npx @agent-native/core@latest connect https://assets.jami.studio`.
...
- Connectez votre propre agent à Dispatch avec `npx @agent-native/core@latest connect https://dispatch.jami.studio` ; utilisez une application directe URL uniquement lorsque vous souhaitez une application isolée.
