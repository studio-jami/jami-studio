...
> Remarque : le `npx skills@latest add studio-jami/jami-studio --skill visual-plan` nu (Vercel/open Skills CLI) installe **instructions uniquement** — il n'enregistre pas le connecteur MCP. Utilisez le Jami Studio CLI ci-dessus lorsque vous souhaitez également câbler le connecteur.

### Code Claude (plugin) {#claude-code}

Le référentiel public `studio-jami/jami-studio` est lui-même un marché de plugins Claude Code, vous l'ajoutez donc directement – sans étape de construction. À l'intérieur du code Claude :
...
**Aucune soumission ou révision n'est requise pour distribuer ou installer ceci.** `studio-jami/jami-studio` est un marché git public et auto-hébergé, les utilisateurs l'ajoutent donc directement avec les commandes ci-dessus sur **à la fois le code Claude et le Codex** — aucune demande ni approbation. L'itinéraire universel CLI ne nécessite aucun marché.
