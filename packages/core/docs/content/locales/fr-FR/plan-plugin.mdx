...
description: "Installez les skills Jami Studio Plan (/visual-plan, /visual-recap, /visualize-repo) ainsi que le connecteur Plan MCP hébergé comme plugin Claude Code ou Codex, ou avec le CLI universel. Comment fonctionnent les mises à jour et si vous devez soumettre quelque chose."
---

# Plugin et place de marché du plan

L'application Jami Studio **Plan** est livrée comme un bundle installable. Une seule installation ajoute les skills de commandes slash Plan **et** connecte le connecteur Plan MCP hébergé, afin que l'agent puisse générer des plans et que les skills puissent les publier directement dans l'application Plan.
...
- **Le connecteur Plan MCP** — enregistré auprès de l'application hébergée sur `https://plan.jami.studio` (point de terminaison MCP `https://plan.jami.studio/_agent-native/mcp`, nom de serveur `plan`).
...
Cela maintient le contenu du plan hors de la base de données du pla Jami Studio. Partage hébergé,
...
Jami Studio Desktop dispose d'un chemin de synchronisation de fichiers locaux distinct pour les forfaits hébergés : le
...
> Le plugin (`agent-native-visual-plans`) porte l'identifiant d'application `visual-plans`, c'est pourquoi le nom du plugin Claude Code et le nom du plugin Codex sont tous deux `agent-native-visual-plans`. Le nom d'affichage de l'application Plan est "Jami Studio Plan".
...
Fonctionne avec n'importe quel hôte — Claude Code, Codex, Cursor, Cline, Goose, apps MCP personnalisées ChatGPT, Claude Cowork et tout autre hôte compatible MCP. Le CLI Jami Studio installe les skills Plan, enregistre le connecteur Plan MCP hébergé **et exécute l'authentification pour les clients locaux sélectionnés dans la même étape**, afin que votre premier appel d'outil ne rencontre pas de mur OAuth :
...
- `--no-connect` : enregistre le connecteur sans authentification ; exécutez `npx @agent-native/core@latest connect https://plan.jami.studio --client all` plus tard ou choisissez un `--client` plus étroit.
...
> Remarque : le `npx skills@latest add Jami Studio/agent-native --skill visual-plan` nu (Vercel/open Skills CLI) installe **instructions uniquement** — il n'enregistre pas le connecteur MCP. Utilisez le Jami Studio CLI ci-dessus lorsque vous souhaitez également câbler le connecteur.

### Code Claude (plugin) {#claude-code}

Le référentiel public `Jami Studio/agent-native` est lui-même un marché de plugins Claude Code, vous l'ajoutez donc directement – sans étape de construction. À l'intérieur du code Claude :
...
Après l'installation, **démarrez un nouveau thread Codex** pour que les outils skills et MCP se chargent dans la session. Le plugin est livré avec un connecteur uniquement URL (`[mcp_servers.plan]` → `https://plan.jami.studio/_agent-native/mcp`) ; `codex mcp login plan` exécute le flux OAuth. La route universelle CLI ci-dessus fonctionne également pour Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`) si vous préférez une commande qui s'installe et s'authentifie ensemble, ou lorsque vous souhaitez des fichiers locaux ou un mode auto-hébergé.

> **Installations plus anciennes :** si votre configuration a toujours une entrée `agent-native-plans` pointant vers le même URL, exécutant `npx -y @agent-native/core@latest reconnect https://plan.jami.studio --client codex` pour Codex, ou la même commande avec votre `--client` cible, la consolide avec le nom canonique `plan`.
...
**Aucune soumission ou révision n'est requise pour distribuer ou installer ceci.** `Jami Studio/agent-native` est un marché git public et auto-hébergé, les utilisateurs l'ajoutent donc directement avec les commandes ci-dessus sur **à la fois le code Claude et le Codex** — aucune demande ni approbation. L'itinéraire universel CLI ne nécessite aucun marché.
