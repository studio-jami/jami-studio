...
description: "Utilisez Jami Studio sans tête, en tant que chat enrichi, dans une application existante ou en tant qu'application native d'agent complète."
...
Jami Studio est délibérément composable. Vous pouvez utiliser l'agent sans trop dépenser UI,
...
| **Chat enrichi sur Jami Studio** | Vous souhaitez un chat autonome ou intégré soutenu par la boucle d'agent intégrée.                                                      | [Chat template](/docs/template-chat), `<AgentChatSurface>`, `<AssistantChat>`               |
| **Chat enrichi sur votre agent**  | Vous avez créé l'agent ailleurs et souhaitez le compositeur, la transcription, les fiches outils et les widgets natifs de Jami Studio. | `AgentChatRuntime`, `<AssistantChat runtime={runtime}>`                                     |
...
un autre agent ou un produit existant appelant Jami Studio.
...
Code, Codex, Pi, Cursor, Mastra ou environnements d'exécution similaires dans une application Jami Studio.
...
## Chat enrichi sur Jami Studio {#rich-chat}
...
runtime et vous voulez que le chat UI de Jami Studio l'entoure. `AgentChatRuntime` est le
limite : votre runtime diffuse les événements normalisés et Jami Studio restitue le
...
pas le chat de l'application utilisateur final ni le transport A2A, et Jami Studio ne le fait pas actuellement
...
Le plugin serveur monte les routes Jami Studio dans votre application hôte et résout
...
<Diagram id="doc-block-b2ci7x" title={"Comment le side-car relie-t-il à une application hôte"} summary={"Le plugin monte les routes Jami Studio côté serveur ; le side-car React diffuse le contexte de la page et les commandes de l'hôte."}>
...
<div class="diagram-sidecar">
  ...
  <div class="diagram-panel accent-panel">
    ...
    <div class="diagram-box" data-rough>
      routes Jami Studio<br /><small class="diagram-muted"
...
| "Je veux l'agent du framework, mais le chat devrait être le UI principal." | Chat enrichi sur Jami Studio |
...
- [**Actions**](/docs/actions) — définissez l’opération une fois ; toutes les surfaces ci-dessus appellent la même
- [**Native Interface de chat**](/docs/native-chat-ui) — affichez les résultats typés sous forme de tableaux, graphiques et cartes dans le chat
- [**Generative UI**](/docs/generative-ui) — générez une UI en bac à sable, temporaire ou persistante, dans le chat
- [**Automation-First Apps**](/docs/pure-agent-apps) — le modèle complet sans navigateur pour les tâches, files d’attente, scripts et agents externes
- [**External Agents**](/docs/external-agents) — connectez des hôtes compatibles MCP à une application
- [**A2A Protocol**](/docs/a2a-protocol) — appelez des agents depuis d’autres applications Jami Studio
