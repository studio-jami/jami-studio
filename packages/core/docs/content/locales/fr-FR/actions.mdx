...
- **Un outil MCP** — exposé aux applications Claude, ChatGPT personnalisées MCP, Claude Desktop/Code, Cursor, Codex et tout autre hôte MCP.
...
Une définition, sept consommateurs. Il s'agit de l'échelon 3 du [ladder](/docs/what-is-jami#the-ladder).
...
| `"frontend"` | Un appel de navigateur via `useActionMutation` / `useActionQuery` / `callAction` (marqué avec l'en-tête `X-Jami Studio-Frontend: 1`).                                   |
...
Conservez `link` comme solution de secours pour la CLI et les hôtes MCP sans interface utilisateur ; c'est également la cible de lancement de l'intégration. Le pont d'intégration (session de démarrage d'intégration signée, rendu de transplantation ou d'image contrôlée, pont hôte `ui/*`, CSP et serrage en hauteur) appartient à [External Agents](/docs/external-agents#mcp-app-bridge).
