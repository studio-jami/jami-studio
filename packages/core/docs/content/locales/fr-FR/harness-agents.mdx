...
description: "Exécutez Claude Code, Codex, Pi et d'autres harnais de codage complets en tant qu'agents intégrés dans Jami Studio, avec leur propre boucle, sandbox, outils natifs et sessions avec reprise basées sur SQL."
...
> Codex, Pi) dans Jami Studio en tant qu'agent. Créer une application ? Commencez par
...
modèle d'approbation et comportement du bac à sable. Jami Studio les exécute via le
...
propre boucle de bout en bout, donc Jami Studio la pilote comme une session, pas comme une seule
appel de modèle.

<Diagram id="doc-block-1a0qxam" title={"Un harnais possède sa boucle ; Jami Studio pilote la session"} summary={"L'AgentHarness substrat creates/resumes la session native, diffuse ses événements dans la transcription normale et conserve l'état de reprise dans SQL entre les tours."}>
...
| Afficher un **espace de travail de codage de style Claude-Code/Codex**                   | [Jami Studio Code UI](/docs/code-agents-ui) |
...
Surfaces adjacentes : placez un agent que vous avez construit ailleurs derrière le chat de Jami Studio
...
Jami Studio peut agir comme un [ACP](https://agentclientprotocol.com) (Agent Client
...
- **Jami Studio Code / Desktop** exécute `codex exec` sur la machine de l'utilisateur. Si
...
- **`ai-sdk-harness:codex`** charge `@ai-sdk/harness-codex`, qui pilote Codex
  ...
  Jami Studio copie le fichier d'authentification local Codex CLI dans le bac à sable avant le
...
Un harnais possède un état de session natif de longue durée. Jami Studio le conserve dans SQL
afin qu'un thread puisse survivre à travers les tours, les processus et les déploiements. Le `resumeState`
est **opaque** — Jami Studio le stocke et le rend, mais ne l'inspecte jamais ou
l'interprète.
...
ensemble intentionnel** de Jami Studio actions à `createSession.tools` lorsque vous
...
Une session de harnais diffuse les valeurs `AgentHarnessEvent`, lesquelles Jami Studio
...
- Ne rejouez pas l'historique complet des discussions de Jami Studio dans un harnais à chaque tour. Reprendre
...
- [Jami Studio Code UI](/docs/code-agents-ui) : la surface de l'espace de travail de codage réutilisable.
