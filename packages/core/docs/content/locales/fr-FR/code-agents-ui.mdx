---
title: "Code Jami Studio UI"
description: "Créez et personnalisez les surfaces de code Jami Studio avec le package UI partagé, le pont hôte de bureau et le magasin d'exécution CLI."
---
...
| ---------------------------------------------------------------------------------------- | -------------------------------------- |
| Afficher un **espace de travail de codage de style Claude-Code/Codex** UI\*\*            | **Jami Studio Code UI** (cette page)  |
...
La division actuelle converge intentionnellement : la barre latérale de l'agent standard et les équipes d'agents s'exécutent sur le cycle de vie principal de `run-manager`, tandis que le code Jami Studio utilise des sessions locales de longue durée soutenues par le magasin d'exécution de code basé sur des fichiers et le vocabulaire partagé du contrôleur d'exécution en arrière-plan.
...
Les hôtes peuvent mélanger les sources d’exécution dans la même liste. Sessions de code Jami Studio locales
...
Le code Jami Studio peut utiliser une connexion locale Codex CLI au lieu d'une clé OpenAI API.
...
Le mode Auto est le mode par défaut pour les sessions de code Jami Studio locales. Utiliser le mode Plan pour
...
Les exécutions de codes Jami Studio locales sont stockées dans :
...
Le code Jami Studio utilise le même `AgentComposerFrame` + `PromptComposer` /
...
L'agent de développement de la barre latérale et le code Jami Studio utilisent tous deux le même minimum
...
Jami Studio Code traite la migration comme une fonctionnalité et non comme une catégorie d'application distincte. `/migrate` peut être un objectif intégré, une commande de projet ou un pack d'instructions personnalisé au-dessus du même contrat hôte.

### Migration vers Jami Studio avec `/migrate` {#migrate}

`/migrate` est l'objectif intégré pour déplacer une application existante, URL ou un produit décrit vers Jami Studio. Il s'agit d'un objectif slash dans l'espace de travail Code - pas d'un modèle distinct à échafauder ni d'un produit unique - il partage donc le même magasin de session, la même transcription, les mêmes contrôles d'exécution et le même hub de bureau que toutes les autres sessions Code, et vous pouvez le reprendre, l'attacher, l'inspecter et l'arrêter de la même manière.
...
Lorsque l'hôte implémente `listCodePacks`, le UI partagé affiche les commandes du projet et le skills dans le rail. Les lignes de commande insèrent `/<command>`, et les lignes de compétences insèrent une invite ciblée « Utiliser la compétence `<skill>`… » afin que le rail reste exploitable. Les objectifs slash intégrés `/migrate` et `/audit` restent réservés aux contrôles de code globaux Jami Studio, tout comme les noms de contrôle d'exécution tels que `status` et `resume` – ce sont des sous-commandes invoquées sans barre oblique (`npx @agent-native/core@latest code status`, `npx @agent-native/core@latest code resume`), et non des objectifs slash.
