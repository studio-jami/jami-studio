...
- **Générez des images avec l'IA.** Images de héros, maquettes de produits, illustrations — de préférence déléguées aux actifs, avec une génération d'images gérée par Jami Studio prête à être activée une fois déployée et des clés de fournisseur directes comme solution de secours actuelle.
...
Démo en direct : [slides.jami.studio](https://slides.jami.studio).
