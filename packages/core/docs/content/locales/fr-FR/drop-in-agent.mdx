...
Un bouton qui confie le travail à l'agent (au lieu d'exécuter un appel `llm()` en ligne — l'anti-modèle du [ladder](/docs/what-is-jami#the-ladder)) :
...
- **Apportez votre propre environnement d'exécution d'agent.** Si un agent que vous avez créé ailleurs le devrait
  alimentez la conversation pendant que Jami Studio conserve le compositeur, la transcription et l'outil
...
Lorsque le UI doit exécuter la même opération qu'un outil d'agent exécuterait (échelon 3 du [ladder](/docs/what-is-jami#rung-three)), utilisez `useActionMutation` :
