...
- **Générer des images et des vidéos.** La génération d'images gérée par Jami Studio est disponible lorsqu'elle est activée, et Gemini alimente la génération vidéo ainsi que le repli manuel des images.
...
Démo en direct : [assets.jami.studio](https://assets.jami.studio).
...
1. **Installer une fois.** Cela ajoute les instructions de compétence et enregistre le connecteur MCP hébergé ensemble :
   ...
   ```bash
   npx skills@dernier ajout d'actifs Jami Studio/agent-native --skill
   ```

   Le Vercel/open Skills CLI installe uniquement le fichier d'instructions ; ce n'est pas le cas
   exécutez la configuration du connecteur MCP. Utilisez le chemin Jami Studio CLI ci-dessus quand vous le souhaitez
