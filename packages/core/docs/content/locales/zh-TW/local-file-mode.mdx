---
title: "本機檔案模式"
description: "使用本機 Markdown、MDX 和其他儲存庫檔案作為事實來源執行 Jami Studio 應用 - 包括帶有自訂元件的黑曜石風格 MDX 檔案。"
---

# 本機檔案模式

本機檔案模式允許 Jami Studio 應用程式附加其正常的 UI 和操作介面
...
也可以由 Codex、Claude 程式碼、Jami Studio 側邊欄代理或普通編輯
編輯器。
...
或 Jami Studio 桌面橋公開它們。如果沒有橋在執行，則該技能
...
普通託管瀏覽器資料夾存取。如果你開啟`content.jami.studio`，
...
本機內容或使用 Jami Studio Desktop，以便受信任的本機網橋可以
