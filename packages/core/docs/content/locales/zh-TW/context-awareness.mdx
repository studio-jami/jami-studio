...
`sendToAgentChat()` 是提交的聊天路徑受支援的瀏覽器包裝器，有時在內部被視為 `agentNative.submitChat`。應用程式 UI 應呼叫包裝器，而不是直接發布 `agentNative.submitChat`，因為包裝器處理本機側邊欄、Jami Studio/Frame 路由、MCP 應用程式主機路由、分頁 ID 和程式碼請求路由。
