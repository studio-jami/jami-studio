...
description: "用於 scheduled jobs、queues、scripts、integration workers 和外部 agents 的無瀏覽器 Jami Studio apps。"
---

# Automation-First Apps

Automation-first app 是無瀏覽器的 Jami Studio app，用於 scheduled jobs、queues、scripts、integration workers 和外部 agents。它不是預設入門路徑，但在工作流程暫時不需要頁面時很有用。它仍然是一個真實的應用程式：actions、sessions、application state、run history、settings、credentials 和共享紀錄均位於 SQL 中。
