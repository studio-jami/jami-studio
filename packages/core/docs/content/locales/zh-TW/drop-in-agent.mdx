...
將工作交給代理的按鈕（而不是執行行內 `llm()` 呼叫 - [ladder](/docs/what-is-jami#the-ladder) 的反模式）：
...
- **自帶代理執行時。**如果您在其他地方建置的代理應該
  在 Jami Studio 保留撰寫器、文字紀錄和工具的同時推動對話
...
當 UI 需要執行代理工具將執行的相同操作（[ladder](/docs/what-is-jami#rung-three) 的梯級 3）時，請使用 `useActionMutation`：
