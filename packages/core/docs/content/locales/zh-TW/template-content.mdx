...
自訂塊。開啟 `/local-files`，在瀏覽器或 Jami Studio Desktop 中選取一個資料夾，並將目前檔案樹匯出為 Markdown/MDX
`content/`。
...
和`resources/`。當本機內容應用、Jami Studio 桌面或受信任
...
保護 Jami Studio 桌面內的本機資料夾橋，以進行讀寫
...
模式和 Jami Studio 桌面，以及在 Finder 中顯示可透過
桌面橋或伺服器支援的本機檔案模式。
...
本機內容或使用 Jami Studio Desktop，以便選定的工作空間路徑可以
