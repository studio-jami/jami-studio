---
title: "聊天範本"
description: "一個最小的聊天優先 Jami Studio 應用程式：持久的聊天對話串、actions、應用程式狀態、即時同步、驗證以及新增您自己的 UI 的空間。"
---

# 聊天範本

聊天是基本的 Jami Studio 應用程式起點。它為您提供了一個幹淨的 ChatGPT 風格的 shell，聊天位於中心，對話串清單位於左側，標準應用程式導覽、驗證、即時同步、actions 和一個範例操作。當您想要一個無需提交域範本即可建置的真正的瀏覽器應用程式時，請從這裡開始。
...
- **您希望將自己的代理後端插入熟悉的聊天 UI**，同時保持 Jami Studio 的 actions、狀態、驗證和部署形狀。
