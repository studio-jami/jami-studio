...
Jami Studio 應用程式使用 [Better Auth](https://better-auth.com) 進行帳戶優先設計的驗證。使用者在第一次存取時建立一個帳戶，並從第一天起就獲得真實身分。
...
| 第一方託管 (`*.jami.studio`)        | 每個應用程式的獨立命名空間（每個應用程式都有自己的驗證資料庫）；預設情況下忽略 `COOKIE_DOMAIN=.jami.studio` |
