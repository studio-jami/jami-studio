...
- **JSON-RPC** — Jami Studio 應用程式使用 `POST /_agent-native/a2a`；外部/遺留節點可以使用 `POST /a2a`
...
export default defineNitroPlugin((nitro) => {
  mountA2A(nitro, {
    ...
    // Jami Studio workspace calls and production deployments.
...
- `POST /_agent-native/a2a` — 主要 Jami Studio JSON-RPC 端點。
- `POST /_agent-native/a2a/_process-task` — 內部非同步處理器路由，使用 `A2A_SECRET` 簽名。

對於公開傳統/簡單路徑的外部代理，用戶端還回退到 `/a2a`。正式環境 Jami Studio 部署應設定 `A2A_SECRET`；如果沒有它，託管執行時將無法關閉，而不是接受未經驗證的遠端工作。
...
對於 Jami Studio 工作區，在程式碼或 a 時更喜歡 `agentNative` 幫助器
...
資料集或工作流程。在正式環境 Jami Studio 應用程式中，在每個中設定 `A2A_SECRET`
...
在每個呼叫或接收 A2A 流量的正式環境應用上設定 `A2A_SECRET`。Jami Studio 呼叫者使用此秘密簽署 JWT 不記名權杖，以便接收者可以在代理迴圈開始之前驗證呼叫者身分。
