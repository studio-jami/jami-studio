---
title: "Skills指南"
description: "skills 如何在 Jami Studio 中工作：框架 skills、域 skills 以及建立自訂 skills。"
---
...
Jami Studio 執行時從 `.agents/skills/` 讀取 skills。 Claude 程式碼獨立從 `.claude/skills/` 讀取 skills。要使編碼代理可用但對執行時代理隱藏的技能：
...
將 Jami Studio CLI 保留為本機代理的預設檔案路徑，因為它
還註冊 MCP 連線器。 `Jami Studio/agent-native`是真正的GitHub
...
地點。新複製的 Jami Studio skills 包括 `agent-native-skill.json`
標記，以便將來的狀態輸出可以識別來源和雜湊值。

產生的 Jami Studio 應用程式和工作區還包括框架提供的
