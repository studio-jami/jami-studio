...
使用者在 Jami Studio 應用程式中建立的每個資源（檔案、儀表板、設計、簡報、螢幕錄製、會議紀錄、表單、預訂連結）**預設情況下對建立者來說是私人的**。僅當建立者明確共用它或將其可見性更改為 `org` 或 `public` 時，其他人才能看到它。

所有地方都使用一種熟悉的共享模型：相同的共享按鈕、相同的對話框、相同的三級可見性模型，以及相同的按使用者和按組織授權，涵蓋每個範本，無需為每個應用程式重新實作。

## 為什麼選取一種模型 {#why}

大多數應用程式框架都會共用每個功能的專案。結果：每個類似檔案的介面最終都會有自己的共用對話框、自己的權限模式、自己的存取檢查錯誤。在 Jami Studio 中，共用是一個**框架原語**。架構列、存取檢查幫助程式、共用快顯視窗和代理可呼叫共用 actions 均隨核心一起提供。新範本透過新增兩欄和一行註冊來獲得完整的分享故事。
...
- **外部真實來源應用程式** - 存取控制位於上游系統中，而不是 Jami Studio 應用程式中。
