...
Jami Studio 應用程式使用 [Nitro](https://nitro.build) 作為伺服器路由和外掛。大多數產品行為應該存在於 [Actions](/docs/actions) 中；自訂路由適用於 actions 不適合的協議表面：上傳、流式傳輸、公開頁面、webhooks、OAuth 回呼和特定於提供者的 API。
...
應用程式僅提供 API 並不表示需要 Express 伺服器。Jami Studio 應用程式已經內建 Nitro/H3 伺服器，`defineAction()` 是預設的 API 表面。將操作定義在 `actions/` 中；框架會將每個 action 掛載到 `/_agent-native/actions/<name>`，並包含 schema 驗證和請求脈絡。
...
聊天，包括 Jami Studio、Anthropic、AI SDK 提供者、使用者/應用模型預設值，
...
Jami Studio 不依賴於檔案系統觀察程式或置頂記憶狀態。當 actions 或框架助手更改資料時，資料庫同步版本會遞增。用戶端 `useDbSync()` 掛鉤輪詢 `/_agent-native/poll` 並使 React 查詢快取無效。
