---
title: "視覺計畫"
description: "Jami Studio 計畫將您的編碼代理的計畫轉變為結構化的、可審閱的檔案 - 圖表、線框圖、帶註解的程式碼、註解和共用連結。從 CLI 安裝一次；您與之共用的審閱者以訪客身分進行編輯，登入後僅可儲存或共用。"
---
...
Jami Studio 計畫是編碼代理的視覺化計畫模式。它變成了普通
...
使用 Jami Studio CLI。這是推薦的設定，因為它安裝了
...
`https://plan.jami.studio/_agent-native/mcp` — 請參閱 [MCP Clients](/docs/mcp-clients) 以了解特定於用戶端的設定。
...
連線器，因此當您需要單指令設定時請使用 Jami Studio CLI 路徑。

> **更喜歡一次性安裝外掛？** Claude 程式碼和 Codex 可以新增
> `Jami Studio/agent-native` 直接作為外掛市場，捆綁了
...
並包含一個指令來執行 VS 的現有 Jami Studio MCP 連線流程
...
本機檔案模式可防止計畫或回顧內容進入 Jami Studio
...
Jami Studio Desktop 還為託管計畫提供了本機本機資料夾橋。這個
與本機檔案隱私模式不同：託管計畫資料庫仍然是
共用、評論、歷史紀錄和即時評論的真實來源，而桌面
可以將目前計畫的來源檔案鏡像到您選取的資料夾。

在 Jami Studio 桌面中開啟計畫，使用計畫選單的**本機檔案** actions，
然後：
...
已終止`，不要繼續重試。使用
`npx -y @agent-native/core@latest reconnect https://plan.jami.studio --client codex`對於 Codex，或在支援 OAuth 的主機中重新執行`/mcp` → **驗證**。開始一個
...
- [**Skills**](/docs/skills-guide) — Jami Studio 如何安裝 skills
