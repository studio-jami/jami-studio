...
Jami Studio 附帶了完整的側邊欄，但側邊欄不是合同。
...
| `createAgentChatAdapter()`           | `@agent-native/core/client` 或 `/client/chat` | 您需要內建 Jami Studio SSE 傳輸作為低階助手 UI 轉接器。    |
