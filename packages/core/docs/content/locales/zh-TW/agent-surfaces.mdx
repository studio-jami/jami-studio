---
title: "代理表面"
description: "將 Jami Studio 無頭使用，作為豐富的聊天，在現有應用程式中，或作為完整的 Jami Studio 應用程式。"
search: "無頭代理豐富聊天完整應用程式 BYO 代理執行時 AgentChatRuntime 嵌入 actions MCP A2A HTTP CLI"
---

# 代理表面

Jami Studio 是故意可組合的。不用太多就可以使用代理 UI，
...
| **Jami Studio 上的豐富聊天** | 您想要由內建代理迴圈支援的獨立或嵌入式聊天。                                       | [Chat template](/docs/template-chat), `<AgentChatSurface>`, `<AssistantChat>`               |
| **與您的代理進行豐富的聊天**  | 您在其他地方建置了代理，並想要 Jami Studio 的編寫器、指令碼、工具卡和本機小工具。 | `AgentChatRuntime`, `<AssistantChat runtime={runtime}>`                                     |
...
- **A2A** - 來自另一個 Jami Studio 應用程式或代理對等點
...
Jami Studio 應用內的程式碼、Codex、Pi、Cursor、Mastra 或類似執行時。
在建置編碼代理產品時使用它們，而不是作為預設方式
啟動本機 Jami Studio 工作流程。
...
執行時，你想要 Jami Studio 的聊天 UI 圍繞它。 `AgentChatRuntime` 是
邊界：您的執行時流規範化事件，Jami Studio 呈現
...
不是最終使用者應用聊天或 A2A 傳輸，並且 Jami Studio 目前沒有
...
伺服器外掛將 Jami Studio 路由安裝到您的主機應用程式中並解析
主機身分伺服器端：
...
<Diagram id="doc-block-t9iu4i" title={"Sidecar 如何橋接到主機應用程式"} summary={"該外掛在伺服器端掛載 Jami Studio 路由； React sidecar 流輸入頁面脈絡並輸出主機指令。"}>
...
<div class="diagram-sidecar">
  ...
  <div class="diagram-panel accent-panel">
    ...
    <div class="diagram-box" data-rough>
      Jami Studio 路由<br /><small class="diagram-muted"
...
| “我想要框架的代理，但是聊天應該是主要的 UI。”        | Jami Studio 上的豐富聊天 |
...
- [**A2A Protocol**](/docs/a2a-protocol) — 從其他 Jami Studio 應用呼叫代理
