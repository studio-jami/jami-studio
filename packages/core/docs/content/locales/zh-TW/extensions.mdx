...
如果您使用過 QuickBooks Online，您就會看到該模型：QBO 提供核心會計產品，使用者可以使用小型自訂小工具（自訂報告、工資計算器、稅收規則檢查器），這些小工具位於同一個應用程式內並使用相同的資料。擴充功能是該想法的 Jami Studio 版本，只不過您的使用者不編寫任何程式碼。他們描述他們想要什麼，然後代理建置它。
...
> 公開了插槽。如需更深入的自訂，請讓代理編輯範本的原始碼。完整的程式碼
> 自訂是 Jami Studio 的核心能力；擴充功能則是快速、無需部署的自訂層。
