---
title: "資料庫"
description: "將便攜式 SQL 資料庫連線到您的 Jami Studio 應用，並編寫與提供者無關的 Drizzle 程式碼。"
---

# 資料庫

Jami Studio 應用程式使用 [Drizzle ORM](https://orm.drizzle.team) 並支援便攜式 SQL 後端。對於本機開發之外的任何內容，請透過設定 `DATABASE_URL` 連線持久性 SQL 資料庫 - Postgres、libSQL/Turso 或另一個 Drizzle 相容後端。取消設定該變數後，應用程式將回退到零設定本機 SQLite 檔案，以便您可以立即開始開發。
...
框架自動偵測 URL 的方言並相應地設定 Drizzle。內建轉接器涵蓋 Postgres URL、libSQL/Turso URL、SQLite 檔案 URL 和 Cloudflare D1 綁定。常見的正式環境選取包括 Neon、Supabase、Turso/libSQL、普通 Postgres、持久 SQLite 和 Jami Studio 託管環境（如果可用）。

## Jami Studio 託管資料庫 {#builder-managed}

*計畫（尚不可用）：*連線到 Jami Studio 時，您的應用將能夠使用自動設定的託管資料庫，無需連線字串。
