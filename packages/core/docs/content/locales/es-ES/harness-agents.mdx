...
description: "Ejecute Claude Code, Codex, Pi y otros sistemas de codificación completos como agentes integrados dentro de Jami Studio, con su propio bucle, zona de pruebas, herramientas nativas y sesiones reanudables respaldadas por SQL."
...
> Codex, Pi) en Jami Studio como agente. ¿Creando una aplicación? Empezar con
...
modelo de aprobación y comportamiento de la zona de pruebas. Jami Studio los ejecuta a través del
...
propio bucle de extremo a extremo, por lo que Jami Studio lo controla como una sesión, no como una sola
llamada modelo.

<Diagram id="doc-block-dzm02u" title={"Un arnés es dueño de su bucle; Jami Studio dirige la sesión"} summary={"El AgentHarness sustrato creates/resumes la sesión nativa, transmite sus eventos a la transcripción normal y persiste resumeState en SQL entre turnos."}>
...
| Renderizar un código Claude/estilo Codex **espacio de trabajo de codificación UI**   | [Jami Studio Code UI](/docs/code-agents-ui) |
...
Superficies adyacentes: coloque un agente que haya creado en otro lugar detrás del chat de Jami Studio
...
Jami Studio puede actuar como [ACP](https://agentclientprotocol.com) (Cliente Agente
...
- **Jami Studio Código/Escritorio** ejecuta `codex exec` en la máquina del usuario. Si
...
- **`ai-sdk-harness:codex`** carga `@ai-sdk/harness-codex`, que impulsa Codex
  ...
  Jami Studio copia el archivo de autenticación local Codex CLI en el entorno de pruebas antes del
...
Un arnés posee un estado de sesión nativo de larga duración. Jami Studio lo persiste en SQL
para que un hilo pueda sobrevivir a través de turnos, procesos e implementaciones. El `resumeState`
es **opaco**: Jami Studio lo guarda y lo devuelve, pero nunca lo inspecciona ni
lo interpreta.
...
conjunto intencional** de Jami Studio actions a `createSession.tools` cuando
...
Una sesión de aprovechamiento transmite valores `AgentHarnessEvent`, que Jami Studio
...
- No reproduzcas el historial de chat completo de Jami Studio en un arnés en cada turno. Currículum
...
- [Jami Studio Code UI](/docs/code-agents-ui): la superficie del espacio de trabajo de codificación reutilizable.
