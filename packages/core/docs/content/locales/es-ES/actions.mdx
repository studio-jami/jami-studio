...
Una definición, siete consumidores. Este es el peldaño 3 del [ladder](/docs/what-is-jami#the-ladder).
...
| `"frontend"` | Una llamada del navegador a través de `useActionMutation` / `useActionQuery` / `callAction` (etiquetada con el encabezado `X-Jami Studio-Frontend: 1`).                             |
