...
`sendToAgentChat()` es el contenedor del navegador compatible para la ruta del chat enviado que a veces se ve internamente como `agentNative.submitChat`. La aplicación UI debe llamar al contenedor en lugar de publicar `agentNative.submitChat` directamente porque el contenedor maneja las barras laterales locales, el enrutamiento Jami Studio/Frame, el enrutamiento del host de la aplicación MCP, los ID de pestañas y el enrutamiento de solicitud de código.
