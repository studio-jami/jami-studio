...
El marco detecta automáticamente el dialecto del URL y configura el Drizzle en consecuencia. Los adaptadores integrados cubren Postgres URL, libSQL/Turso URL, SQLite file URL y enlaces Cloudflare D1. Las opciones de producción comunes incluyen entornos administrados Neon, Supabase, Turso/libSQL, Postgres simple, SQLite duradero y Jami Studio cuando estén disponibles.

## Base de datos administrada Jami Studio {#builder-managed}

_Planificado (aún no disponible):_ cuando esté conectado a Jami Studio, su aplicación podrá utilizar una base de datos administrada y aprovisionada automáticamente, sin necesidad de cadenas de conexión.
