...
No necesita un servidor Express solo porque la aplicación sea solo API. Una aplicación Jami Studio ya incluye un servidor Nitro/H3, y `defineAction()` es la superficie API predeterminada. Defina las operaciones en `actions/`; el marco monta cada action en `/_agent-native/actions/<name>` con validación de esquema y contexto de solicitud.
...
chat, incluidos Jami Studio, Anthropic, proveedores AI SDK, valores predeterminados del modelo de usuario/aplicación,
