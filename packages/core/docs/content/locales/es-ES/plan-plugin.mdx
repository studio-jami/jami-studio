...
> Nota: el `npx skills@latest add Jami Studio/agent-native --skill visual-plan` desnudo (Vercel/open Skills CLI) instala **solo instrucciones**; no registra el conector MCP. Utilice el Agent-Native CLI anterior cuando desee conectar el conector también.
