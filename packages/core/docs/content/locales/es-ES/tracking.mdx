...
Esto es distinto de la telemetría interna del navegador del marco (`trackEvent()` / vistas de página automáticas; consulte [Browser defaults](#browser-defaults) a continuación), que impulsa el propio análisis de productos de Jami Studio. Utilice `track()` para los eventos de análisis propios de su aplicación que deberían llegar a sus proveedores configurados.

## Repetición de sesión {#session-replay}

Las aplicaciones de Jami Studio pueden habilitar la repetición de sesión del navegador de origen sin agregar un segundo SDK de análisis. Llame a `configureTracking()` una vez en la raíz del navegador y pase la clave pública de Analytics más el endpoint del recolector:
