...
- **Generar imágenes y vídeos.** La generación de imágenes administrada por Jami Studio está disponible cuando está habilitada, y Gemini potencia la generación de vídeos además del respaldo manual de imágenes.
...
Demostración en vivo: [assets.jami.studio](https://assets.jami.studio).
...
1. **Instalar una vez.** Esto agrega las instrucciones de habilidad y registra el conector MCP alojado en conjunto:
   ...
   ```bash
   npx skills@latest agregar Jami Studio/agent-native --skill activos
   ```

   Vercel/open Skills CLI instala únicamente el archivo de instrucciones; no es así
   ejecute la configuración del conector MCP. Utilice la ruta Jami Studio CLI anterior cuando lo desee
