...
Cuando instala Planes de forma interactiva, Jami Studio CLI le pregunta si desea agregar
...
`recap doctor` rechaza JSON con formato incorrecto o no compatible e informa cuando GitHub no expone ningún ejecutor en línea que coincida con todas las etiquetas configuradas. El registro, la instalación, las actualizaciones, la capacidad y el mantenimiento del ejecutor siguen siendo su responsabilidad; Jami Studio selecciona las etiquetas, pero no proporciona ni opera el ejecutor.
...
recapitular sin enviar el contenido del resumen a la base de datos del pla Jami Studio, ejecute el
