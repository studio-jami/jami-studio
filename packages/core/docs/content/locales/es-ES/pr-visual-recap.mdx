...
Para instalarlo, copie el archivo de [studio-jami/jami-studio](https://github.com/studio-jami/jami-studio/blob/main/.github/workflows/pr-visual-recap-fork.yml) en el directorio `.github/workflows/` de su repositorio junto con el `pr-visual-recap.yml` existente. Se aplican los mismos secretos (`PLAN_RECAP_TOKEN`, `ANTHROPIC_API_KEY`).
...
En su lugar, la opción **reutilizable** escribe una persona que llama delgada de ~20 líneas. Delega a `studio-jami/jami-studio/.github/workflows/pr-visual-recap-reusable.yml` vía `uses:`. Cada persona que llama selecciona automáticamente la lógica más reciente cuando se ejecuta el flujo de trabajo, sin necesidad de actualización local.
...
# Llamada ligera: toda la lógica del flujo de trabajo se encuentra en studio-jami/jami-studio.
