...
```bash
npx @agent-native/core@latest connect https://plan.jami.studio --service-token pr-recap
```
...
```bash
npx @agent-native/core@latest connect https://plan.jami.studio --client codex
npx @agent-native/recap-cli@latest recap setup
```
...
| `PLAN_RECAP_APP_URL`     | `https://plan.jami.studio`                                       | Secreto. Solo cuando se aloja la aplicación Planes en un origen diferente.                                                                                                                                                                                                                                                                                         |
...
Para instalarlo, copie el archivo de [Jami Studio/agent-native](https://github.com/BuilderIO/agent-native/blob/main/.github/workflows/pr-visual-recap-fork.yml) en el directorio `.github/workflows/` de su repositorio junto con el `pr-visual-recap.yml` existente. Se aplican los mismos secretos (`PLAN_RECAP_TOKEN`, `ANTHROPIC_API_KEY`).
...
En el monorepo `Jami Studio/agent-native`, el flujo de trabajo ejecuta el resumen CLI desde la fuente confiable de la rama base en lugar de la fuente principal de PR. Esto mantiene los cambios de paquetes normales, incluido `packages/core/**`, elegibles para resúmenes sin ejecutar el código CLI modificado por PR.
...
En su lugar, la opción **reutilizable** escribe una persona que llama delgada de ~20 líneas. Delega a `Jami Studio/agent-native/.github/workflows/pr-visual-recap-reusable.yml` vía `uses:`. Cada persona que llama selecciona automáticamente la lógica más reciente cuando se ejecuta el flujo de trabajo, sin necesidad de actualización local.
...
# Llamada ligera: toda la lógica del flujo de trabajo se encuentra en Jami Studio/agent-native.
...
jobs:
  visual-recap:
    permissions:
      ...
      issues: write
      pull-requests: write
    uses: Jami Studio/agent-native/.github/workflows/pr-visual-recap-reusable.yml@main
    secrets:
...
```yaml
uses: Jami Studio/agent-native/.github/workflows/pr-visual-recap-reusable.yml@v1.2.3
```
