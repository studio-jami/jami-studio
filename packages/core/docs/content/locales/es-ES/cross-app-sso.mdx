...
Cada aplicación alojada en `*.jami.studio` ejecuta su propia implementación con su **propio almacén de usuarios independiente**. `mail.jami.studio` y `calendar.jami.studio` no comparten una base de datos, una tabla de sesión ni un dominio de cookies. Por lo tanto, "iniciar sesión una vez, usar todas las aplicaciones" no puede ser una cookie compartida; tiene que ser una **federación de identidad**, con [Dispatch](/docs/dispatch) actuando como autoridad de identidad para el espacio de trabajo.
...
1. **Aplicación → Dispatch (autorización).** La ruta `GET /_agent-native/identity/login` de la aplicación (a la que enlaza el botón «Iniciar sesión con Jami Studio») crea la redirección y envía al usuario a la autoridad de identidad:
...
<Endpoint id="cross-app-sso-identity-authorize-es-es" title={"Punto final de autorización de identidad"} method="GET" path="/_agent-native/identity/authorize" summary={"El envío (autoridad de identidad) autentica al ser humano y lo redirecciona con un token de identidad firmado"} auth={"Sesión de envío (inicio de sesión interactivo si no hay ninguno)"} params={[
  ...
  },
  {
    ...
    "type": "string",
    "required": true,
    "description": "Devolución de llamada de la aplicación URL. Validado con una lista de permitidos estricta (`*.jami.studio` o localhost de forma predeterminada)."
  },
  {
    "name": "state",
    "in": "query",
...
- **Establecer** → la aplicación muestra una opción **"Iniciar sesión con Jami Studio"** que ejecuta el flujo anterior. El inicio de sesión local directo (correo electrónico/contraseña, Google) todavía funciona junto con él.
...
Cualquier implementación de Dispatch puede servir como centro de identidad; no está limitado a `dispatch.jami.studio`. Configure `AGENT_NATIVE_IDENTITY_HUB_URL` en cada aplicación cliente para que apunte a su instancia de Dispatch:
...
- **Predeterminado:** `*.jami.studio` y localhost únicamente (la constante `DEFAULT_ALLOWED_HOST_SUFFIXES`).
...
Sin `IDENTITY_SSO_ALLOWED_HOST_SUFFIXES`, un Dispatch autohospedado solo puede emitir tokens para aplicaciones en `*.jami.studio`. Configure la variable de entorno en su implementación de Dispatch para desbloquear otros dominios.
...
- La pantalla de inicio de sesión ahora muestra **"Iniciar sesión con Jami Studio"**. Haz clic en él.
