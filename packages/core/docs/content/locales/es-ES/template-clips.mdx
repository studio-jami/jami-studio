...
La lista de extensiones de Chrome Jami Studio Clips es
...
Demostración en vivo: [clips.jami.studio](https://clips.jami.studio).
...
La aplicación Clips alojada en [clips.jami.studio](https://clips.jami.studio)
...
2. **Configurar el estado de producción.** Establecer un `DATABASE_URL` persistente, el normal
   ...
   proveedor de almacenamiento de vídeo. Jami Studio Connect es la ruta de almacenamiento más sencilla; para
...
6. **Conecta integraciones opcionales.** Google Calendar impulsa la pestaña Reuniones,
   `GEMINI_API_KEY` o Jami Studio Connect potencian la limpieza de transcripciones y títulos
