...
| `clips`        | buscar, importar, reuniones             | cerebro, clips                               |
...
Dispatch expone acciones del plano de control que escriben mediante las mismas funciones del almacén compartido que una aplicación podría llamar directamente: su acción `upsert-workspace-connection` valida la entrada frente al catálogo de proveedores y después delega en la misma `upsertWorkspaceConnection()` que se muestra en esta página:
...
- [**Dispatch**](/docs/dispatch) — la aplicación que suele administrar la configuración de conexiones del espacio de trabajo y la bóveda
- [**Seguridad**](/docs/security) — la bóveda de credenciales, el alcance de secretos y el modelo de acceso al que apuntan las conexiones
- [**Incorporación**](/docs/onboarding#checking-workspace-connections-in-onboarding) — comprobar la preparación de conexiones desde la lista de configuración de una plantilla
- [**Clientes MCP**](/docs/mcp-clients) — compartir credenciales de servidores MCP en el espacio de trabajo como patrón relacionado de configuración reutilizable
- [**Brain**](/docs/template-brain) — la implementación de referencia de una página de fuentes de proveedores basada en este modelo
