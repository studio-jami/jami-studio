...
Un botón que entrega el trabajo al agente (en lugar de ejecutar una llamada `llm()` en línea, el antipatrón del [ladder](/docs/what-is-jami#the-ladder)):
...
- **Traiga su propio tiempo de ejecución de agente.** Si un agente que creó en otro lugar debería hacerlo
  impulsa la conversación mientras Jami Studio mantiene el compositor, la transcripción y la herramienta
...
Cuando UI necesita ejecutar la misma operación que ejecutaría una herramienta de agente (rango 3 de [ladder](/docs/what-is-jami#rung-three)), use `useActionMutation`:
