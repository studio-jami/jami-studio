...
- **Su propio tiempo de ejecución detrás del chat Jami Studio:** consulte [Agent Surfaces](/docs/agent-surfaces#byo-agent) y [Native Interfaz de chat](/docs/native-chat-ui#byo-agent-runtimes) cuando un agente creado con otro marco debería impulsar `<AssistantChat runtime={...}>`.
...
Agregue un conector MCP remoto al host donde desea utilizar Jami Studio.
...
**Conéctese**, inicie sesión con su cuenta Jami Studio, apruebe los alcances MCP,
...
escanear/descubrir herramientas, iniciar sesión con Jami Studio, aprobar los ámbitos y habilitar
...
La aplicación propia skills instala las instrucciones y el conector MCP alojado junto con el Jami Studio CLI:
...
necesita un conector como `npx @agent-native/core@latest connect https://assets.jami.studio`.
...
- **Por token**: perfecto con `--full-catalog`, que incorpora un reclamo de `catalog_scope: "full"` en el JWT. Las solicitudes posteriores omiten el filtro compacto para ese token:
  ...
  ```bash
  npx @agent-native/core@latest connect https://plan.jami.studio --códice del cliente --catálogo completo
  ```
...
- Conecte su propio agente a Dispatch con `npx @agent-native/core@latest connect https://dispatch.jami.studio`; use una aplicación directa URL solo cuando desee una aplicación aislada.
