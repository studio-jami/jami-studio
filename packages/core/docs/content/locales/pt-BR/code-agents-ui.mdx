...
description: "Crie e personalize superfícies de código Jami Studio com o pacote UI compartilhado, ponte de host de desktop e armazenamento de execução CLI."
---
...
| Você quer…                                                                               | Usar                                     |
| ---------------------------------------------------------------------------------------- | ---------------------------------------- |
| Renderizar um espaço de trabalho de codificação estilo Claude/Codex **UI**               | **Jami Studio Código UI** (esta página) |
...
A divisão atual está convergindo intencionalmente: a barra lateral padrão do agente e as equipes de agentes são executadas no ciclo de vida principal do `run-manager`, enquanto o código Jami Studio usa sessões locais de longa execução apoiadas pelo armazenamento de execução de código baseado em arquivo e pelo vocabulário compartilhado do controlador de execução em segundo plano.
...
Os hosts podem misturar fontes de execução na mesma lista. Sessões locais do código Jami Studio
...
O código Jami Studio pode usar um login Codex CLI local em vez de uma chave OpenAI API.
...
O modo automático é o padrão para sessões locais do código Jami Studio. Use o modo Plano para
...
As execuções locais do código Jami Studio são armazenadas em:
...
O código Jami Studio usa o mesmo `AgentComposerFrame` + `PromptComposer` /
...
O agente de desenvolvimento da barra lateral e o código Jami Studio usam o mesmo mínimo
...
O código Jami Studio trata a migração como um recurso, não como uma categoria de aplicativo separada. `/migrate` pode ser um objetivo integrado, um comando de projeto ou um pacote de instruções personalizado no mesmo contrato de host.

### Migrando para Jami Studio com `/migrate` {#migrate}

`/migrate` é o objetivo integrado para mover um aplicativo existente, URL, ou produto descrito para Jami Studio. É uma meta de corte no espaço de trabalho do Code - não um modelo separado para scaffold e nem um produto único - então ele compartilha o mesmo armazenamento de sessão, transcrição, controles de execução e hub de desktop como qualquer outra sessão do Code, e você pode retomar, anexar, inspecionar e interrompê-lo da mesma maneira.
