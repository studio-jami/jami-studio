...
- **Gere imagens e vídeos.** A geração de imagens gerenciadas pelo Jami Studio está disponível quando ativada, e o Gemini potencializa a geração de vídeo e o substituto manual de imagens.
...
Demonstração ao vivo: [assets.jami.studio](https://assets.jami.studio).
...
1. **Instale uma vez.** Isso adiciona as instruções de habilidade e registra o conector MCP hospedado em conjunto:
   ...
   ```bash
   npx skills@latest adicionar Jami Studio/agent-native --skill assets
   ```

   O Vercel/open Skills CLI instala apenas o arquivo de instruções; isso não acontece
   execute a configuração do conector MCP. Use o caminho Jami Studio CLI acima quando quiser
