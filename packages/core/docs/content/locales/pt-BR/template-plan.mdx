...
`https://plan.jami.studio/mcp` — consulte [MCP Clients](/docs/mcp-clients) para configuração específica do cliente.
...
- O modelo de chat padrão do próprio aplicativo é o Sonnet 5; altere-o a qualquer
  momento em **Configurações**. Haiku é a opção Claude mais barata, e Luna é a
  opção mais barata da família GPT-5.6.
...
O conector de planos hospedados fica em `https://plan.jami.studio/mcp`.
