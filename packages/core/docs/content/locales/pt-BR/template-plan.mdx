...
Jami Studio Plans é o modo de plano visual para agentes de codificação. Acontece uma coisa comum
...
Use o Jami Studio CLI. Esta é a configuração recomendada porque instala o
...
`https://plan.jami.studio/_agent-native/mcp` — consulte [MCP Clients](/docs/mcp-clients) para configuração específica do cliente.
...
conector, então use o caminho Jami Studio CLI quando desejar a configuração com um comando.

> **Prefere um plugin de instalação única?** Código Claude e Codex podem ser adicionados
> `Jami Studio/agent-native` diretamente como um mercado de plugins, que agrupa o
...
e inclui um comando para executar o fluxo de conexão Jami Studio MCP existente para VS
...
O conector de planos hospedados fica em `https://plan.jami.studio/_agent-native/mcp`.
...
O modo de arquivos locais evita que o conteúdo do plano ou recapitulação vá para o Jami Studio
...
O Jami Studio Desktop também oferece aos planos hospedados uma ponte nativa de pastas locais. Isto
...
Abra um plano no Jami Studio Desktop, use **Arquivos locais** actions do menu do plano,
então:
...
- Aplicativo: `https://plan.jami.studio`
...
- [**Skills**](/docs/skills-guide) — como Jami Studio instala skills
