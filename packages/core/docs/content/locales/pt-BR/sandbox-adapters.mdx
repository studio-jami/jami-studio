...
Jami Studio tem duas costuras de adaptador que eliminam a preocupação por trás de um estreito,
interface trocável:
...
| Renderizar um espaço de trabalho de codificação estilo Claude/Codex **UI**               | [Jami Studio Code UI](/docs/code-agents-ui) |
...
<FileTree
  ...
  entries={[
    ...
    {
      path: "background.ts",
      note: "marcador de fila durável e ciclo de vida da execução em segundo plano",
    },
    {
...
Seleciona um adaptador integrado por ID. Tanto `local` (o padrão) quanto `background` estão conectados; valores desconhecidos voltam para `local` em vez de falhar na execução.

```bash
AGENT_NATIVE_SANDBOX=local        # o padrão - explícito
AGENT_NATIVE_SANDBOX=background   # enfileira cada chamada run-code de forma durável
```

`background` armazena o código bruto em `sandbox_executions` e retorna imediatamente um `executionId` com `status: "queued"`. Um auto-dispatch assinado inicia uma nova invocação serverless; um processo Node de longa duração inicia trabalho desacoplado no processo; a consulta de status e a varredura periódica reativam dispatches perdidos ou leases expirados. Consulte a execução mais tarde para obter o resultado persistido.

`BackgroundQueueAdapter` é um marcador de fila durável, não o executor. O trabalho enfileirado usa um adaptador de execução registrado que não seja de fila quando disponível; caso contrário, volta para o processo filho local. Selecionar apenas `background` não fornece isolamento remoto nem de contêiner e não torna um runtime edge compatível com `node:child_process`.
...
O adaptador integrado `background` já fornece enfileiramento durável e um novo orçamento de execução, enquanto a execução real ainda usa um adaptador que não seja de fila ou o processo filho local como fallback. Um adaptador remoto ou isolado registrado separadamente (Docker ou um executor estilo Vercel-Sandbox) pode fornecer outro limite de execução e também executar trabalho em segundo plano por chamada.

Esse adaptador:

1. Implementaria `SandboxAdapter.run` em um runtime fora do processo.
2. Criaria um túnel para a ponte de loopback ou encaminharia suas chamadas ao processo pai.
3. Forneceria isolamento remoto ou de contêiner independentemente do ciclo de vida da fila durável incluída.

Registre-o programaticamente por meio de `registerSandboxAdapter()`. O loop do agente e `run-code.ts` não precisam mudar.
