...
Você não precisa de um servidor Express só porque o aplicativo é somente API. Um aplicativo Jami Studio já vem com um servidor Nitro/H3, e `defineAction()` é a superfície API padrão. Defina operações em `actions/`; a estrutura monta cada action em `/_agent-native/actions/<name>` com validação de schema e contexto de solicitação.
...
bate-papo, incluindo provedores Jami Studio, Anthropic, AI SDK, padrões de modelo de usuário/aplicativo,
