...
Cada aplicativo hospedado no `*.jami.studio` executa sua própria implantação com seu **próprio armazenamento de usuário separado**. `mail.jami.studio` e `calendar.jami.studio` não compartilham um banco de dados, uma tabela de sessão ou um domínio de cookie. Portanto, "fazer login uma vez, usar todos os aplicativos" não pode ser um cookie compartilhado. Tem que ser uma **federação de identidades**, com [Dispatch](/docs/dispatch) atuando como autoridade de identidade para o espaço de trabalho.
...
1. **Aplicativo → Dispatch (autorização).** A rota `GET /_agent-native/identity/login` do aplicativo (à qual o botão “Entrar com Jami Studio” aponta) cria o redirecionamento e envia o usuário à autoridade de identidade:
...
<Endpoint id="cross-app-sso-identity-authorize-pt-br" title={"Endpoint de autorização de identidade"} method="GET" path="/_agent-native/identity/authorize" summary="Dispatch (autoridade de identidade) autentica o humano e redireciona de volta com um token de identidade assinado" auth={"Sessão de envio (login interativo se não houver)"} params={[
  ...
  },
  {
    ...
    "type": "string",
    "required": true,
    "description": "Retorno de chamada do aplicativo URL. Validado em uma lista de permissões estrita (`*.jami.studio` ou localhost por padrão)."
  },
  {
    "name": "state",
    "in": "query",
...
- **Set** → o aplicativo mostra uma opção **"Entrar com Jami Studio"** que executa o fluxo acima. O login local direto (e-mail/senha, Google) ainda funciona junto com ele.
...
Qualquer implantação do Dispatch pode servir como hub de identidade — você não está limitado ao `dispatch.jami.studio`. Defina `AGENT_NATIVE_IDENTITY_HUB_URL` em cada aplicativo cliente para apontar para sua instância do Dispatch:
...
- **Padrão:** somente `*.jami.studio` e localhost (a constante `DEFAULT_ALLOWED_HOST_SUFFIXES`).
...
Sem `IDENTITY_SSO_ALLOWED_HOST_SUFFIXES`, um Dispatch auto-hospedado só pode emitir tokens para aplicativos em `*.jami.studio`. Defina a variável env em sua implantação do Dispatch para desbloquear outros domínios.
...
- **Saia** do aplicativo.
- A tela de login agora mostra **"Entrar com Jami Studio"**. Clique nele.
