...
A lista de extensões Jami Studio Clips do Chrome é
...
Demonstração ao vivo: [clips.jami.studio](https://clips.jami.studio).
...
O aplicativo Clips hospedado em [clips.jami.studio](https://clips.jami.studio)
...
2. **Configurar o estado de produção.** Definir um `DATABASE_URL` persistente, o normal
   ...
   provedor de armazenamento de vídeo. Jami Studio Connect é o caminho de armazenamento mais fácil; para
...
6. **Conecte integrações opcionais.** Google Calendar alimenta a guia Reuniões,
   `GEMINI_API_KEY` ou Jami Studio Connect possibilitam a limpeza de transcrições e títulos,
