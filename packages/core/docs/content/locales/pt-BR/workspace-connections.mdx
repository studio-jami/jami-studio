...
| `clips`        | pesquisa, importação, reuniões             | cérebro, clipes                               |
...
O Dispatch expõe actions do plano de controle que usam as mesmas funções de armazenamento compartilhado que um aplicativo poderia chamar diretamente — sua action `upsert-workspace-connection` valida a entrada em relação ao catálogo de provedores e então delega para a mesma `upsertWorkspaceConnection()` mostrada nesta página:
...
- [**Dispatch**](/docs/dispatch) — o aplicativo que normalmente gerencia a configuração de conexões do espaço de trabalho e o cofre
- [**Segurança**](/docs/security) — o cofre de credenciais, o escopo de segredos e o modelo de acesso para os quais as conexões apontam
- [**Integração**](/docs/onboarding#checking-workspace-connections-in-onboarding) — verificar a prontidão da conexão na lista de configuração de um modelo
- [**Clientes MCP**](/docs/mcp-clients) — compartilhar credenciais de servidores MCP no espaço de trabalho como um padrão relacionado de configuração reutilizável
- [**Brain**](/docs/template-brain) — a implementação de referência de uma página Sources de provedores criada sobre este modelo
