...
identify("steve@jami.studio", { plan: "pro", company: "Jami Studio" });
```
...
Isso é diferente da telemetria interna do navegador da estrutura (`trackEvent()`/visualizações automáticas de página — veja [Browser defaults](#browser-defaults) abaixo), que alimenta a análise de produto do próprio Jami Studio. Use `track()` para os eventos de análise do seu aplicativo que devem chegar aos provedores configurados.

## Replay de sessão {#session-replay}

Aplicativos Jami Studio podem optar por um replay de sessão do navegador nativo sem adicionar um segundo SDK de análise. Chame `configureTracking()` uma vez na raiz do navegador e passe a chave pública do Analytics mais o endpoint do coletor:
...
As raízes dos modelos do Jami Studio já chamam `configureTracking()`. As implantações hospedadas de modelos podem ativar o replay com variáveis de ambiente do Vite/Netlify, enquanto os consumidores de biblioteca devem preferir a forma explícita `configureTracking({ key, endpoint, sessionReplay })` acima:
