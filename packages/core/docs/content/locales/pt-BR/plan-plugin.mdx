...
- **O conector do Plano MCP** — registrado no aplicativo hospedado em `https://plan.jami.studio` (endpoint MCP `https://plan.jami.studio/mcp`, nome do servidor `plan`).
...
Após a instalação, **inicie um novo thread Codex** para que as ferramentas skills e MCP sejam carregadas na sessão. O plugin vem com um conector somente URL (`[mcp_servers.plan]` → `https://plan.jami.studio/mcp`); `codex mcp login plan` executa o fluxo OAuth. A rota universal CLI acima também funciona para Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`) se você preferir um comando que instale e autentique juntos ou quando desejar arquivos locais ou modo auto-hospedado.
