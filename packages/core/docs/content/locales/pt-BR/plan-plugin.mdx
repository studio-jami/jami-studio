...
description: "Instale as skills do Jami Studio Plan (/visual-plan, /visual-recap, /visualize-repo) mais o conector do Plan MCP hospedado como plug-in Claude Code ou Codex, ou com o CLI universal. Como funcionam as atualizações e se você precisa enviar algo."
---

# Planejar plug-in e mercado

O aplicativo Jami Studio **Plan** é enviado como um pacote instalável. Uma única instalação adiciona as skills de comando slash do Plan **e** conecta o conector do Plan MCP hospedado, para que o agente possa gerar planos e as skills possam publicá-los diretamente no aplicativo Plan.
...
- **O conector do Plano MCP** — registrado no aplicativo hospedado em `https://plan.jami.studio` (endpoint MCP `https://plan.jami.studio/_agent-native/mcp`, nome do servidor `plan`).
...
Isso mantém o conteúdo do plano fora do banco de dados do plano Jami Studio. Compartilhamento hospedado,
...
O Jami Studio Desktop tem um caminho de sincronização de arquivo local separado para planos hospedados: o
...
> O plugin (`agent-native-visual-plans`) carrega o ID do aplicativo `visual-plans`, e é por isso que o nome do plugin Claude Code e o nome do plugin Codex são ambos `agent-native-visual-plans`. O nome de exibição do aplicativo Plan é "Jami Studio Plan".
...
- `--no-connect` — registre o conector sem autenticar; execute `npx @agent-native/core@latest connect https://plan.jami.studio --client all` mais tarde ou escolha um `--client` mais estreito.
...
> Nota: o `npx skills@latest add Jami Studio/agent-native --skill visual-plan` simples (Vercel/open Skills CLI) instala **apenas instruções** — ele não registra o conector MCP. Use o Jami Studio CLI acima quando quiser que o conector também seja conectado.

### Código Claude (plug-in) {#claude-code}

O repositório público `Jami Studio/agent-native` é em si um mercado de plug-ins de código Claude, então você o adiciona diretamente - sem etapa de construção. Código interno Claude:
...
Após a instalação, **inicie um novo thread Codex** para que as ferramentas skills e MCP sejam carregadas na sessão. O plugin vem com um conector somente URL (`[mcp_servers.plan]` → `https://plan.jami.studio/_agent-native/mcp`); `codex mcp login plan` executa o fluxo OAuth. A rota universal CLI acima também funciona para Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`) se você preferir um comando que instale e autentique juntos ou quando desejar arquivos locais ou modo auto-hospedado.

> **Instalações mais antigas:** se sua configuração ainda tiver uma entrada `agent-native-plans` apontando para o mesmo URL, executando `npx -y @agent-native/core@latest reconnect https://plan.jami.studio --client codex` para Codex ou o mesmo comando com seu `--client` de destino, consolida-o no nome canônico `plan`.
...
**Nenhum envio ou revisão é necessário para distribuir ou instalar isso.** `Jami Studio/agent-native` é um mercado git público auto-hospedado, então os usuários o adicionam diretamente com os comandos acima em **Código Claude e Codex** — sem aplicação ou aprovação. A rota universal CLI não precisa de nenhum mercado.
