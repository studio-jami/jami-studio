...
- **Seu próprio tempo de execução por trás do bate-papo Jami Studio:** veja [Agent Surfaces](/docs/agent-surfaces#byo-agent) e [Native Interface de chat](/docs/native-chat-ui#byo-agent-runtimes) quando um agente criado com outra estrutura deve alimentar o `<AssistantChat runtime={...}>`.
...
Adicione um conector MCP remoto ao host onde você deseja usar Jami Studio.
...
**Conecte**, faça login com sua conta Jami Studio, aprove os escopos MCP,
...
ferramentas de verificação/descoberta, login com Jami Studio, aprovação dos escopos e ativação
...
O aplicativo original skills instala as instruções e o conector MCP hospedado junto com o Jami Studio CLI:
...
precisa de um conector como `npx @agent-native/core@latest connect https://assets.jami.studio`.
...
- Conecte seu próprio agente ao Dispatch com `npx @agent-native/core@latest connect https://dispatch.jami.studio`; use um aplicativo direto URL somente quando quiser um aplicativo isolado.
