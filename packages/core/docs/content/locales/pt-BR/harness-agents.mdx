...
description: "Execute código Claude, Codex, Pi e outros equipamentos de codificação completos como agentes incorporados dentro do Jami Studio, com seu próprio loop, sandbox, ferramentas nativas e sessões recuperáveis apoiadas por SQL."
...
> Codex, Pi) em Jami Studio como agente. Construindo um aplicativo? Comece com
...
modelo de aprovação e comportamento do sandbox. Jami Studio executa isso através do
...
próprio loop de ponta a ponta, então Jami Studio o conduz como uma sessão, não como uma única
chamada de modelo.

<Diagram id="doc-block-1r2cc85" title={"Um arnês possui seu laço; Jami Studio conduz a sessão"} summary={"O AgentHarness substrato creates/resumes a sessão nativa, transmite seus eventos para a transcrição normal e persiste resumeState em SQL entre os turnos."}>
...
Superfícies adjacentes: coloque um agente que você construiu em outro lugar atrás do bate-papo de Jami Studio
...
Jami Studio pode atuar como um [ACP](https://agentclientprotocol.com) (Agente Cliente
...
- **Jami Studio Code / Desktop** executa `codex exec` na máquina do usuário. Se
...
- **`ai-sdk-harness:codex`** carrega `@ai-sdk/harness-codex`, que aciona Codex
  ...
  Jami Studio copia o arquivo de autenticação local Codex CLI para o sandbox antes do
...
Um chicote possui um estado de sessão nativo de longa duração. Jami Studio persiste em SQL
para que um thread possa sobreviver entre turnos, processos e implantações. O `resumeState`
é **opaco** — Jami Studio armazena e devolve, mas nunca inspeciona ou
interpreta.
...
conjunto intencional** de Jami Studio actions até `createSession.tools` quando você
...
Uma sessão de aproveitamento transmite valores `AgentHarnessEvent`, que Jami Studio
...
- Não reproduza todo o histórico de bate-papo do Jami Studio em um chicote a cada turno. Currículo
...
- [Jami Studio Code UI](/docs/code-agents-ui) — a superfície reutilizável do espaço de trabalho de codificação.
