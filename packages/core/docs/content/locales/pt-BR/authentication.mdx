...
| Hospedado próprio (`*.jami.studio`)                | Namespace isolado por aplicativo (cada um tem seu próprio banco de dados de autenticação); `COOKIE_DOMAIN=.jami.studio` é ignorado por padrão |
