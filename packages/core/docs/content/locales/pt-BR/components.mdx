...
Jami Studio envia uma barra lateral completa, mas a barra lateral não é o contrato. O
...
| `createAgentChatAdapter()`           | `@agent-native/core/client` ou `/client/chat` | Você precisa do transporte Jami Studio SSE integrado como um adaptador de interface do usuário assistente de baixo nível. |
...
conversa enquanto Jami Studio mantém o compositor, a transcrição, os cartões de ferramentas e
