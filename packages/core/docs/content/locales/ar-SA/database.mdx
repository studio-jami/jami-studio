...
يكتشف إطار العمل اللهجة تلقائيًا من URL ويقوم بتكوين Drizzle وفقًا لذلك. تغطي المحولات المدمجة Postgres URLs وlibSQL/Turso URLs وSQLite file URLs وروابط Cloudflare D1. تتضمن خيارات الإنتاج الشائعة Neon، وSupabase، وTurso/libSQL، وPostgres العادي، وSQLite المتين، والبيئات المُدارة بواسطة Jami Studio عند توفرها.

## قاعدة البيانات المُدارة Jami Studio {#builder-managed}

_مخطط (غير متاح بعد):_ عند الاتصال بـ Jami Studio، سيتمكن تطبيقك من استخدام قاعدة بيانات مُدارة يتم توفيرها تلقائيًا، دون الحاجة إلى سلاسل اتصال.
