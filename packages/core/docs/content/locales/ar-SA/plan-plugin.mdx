...
description: "قم بتثبيت مهارات Jami Studio Plan (/visual-plan، /visual-recap، /visualize-repo) بالإضافة إلى موصل Plan MCP المستضاف كمكوّن Claude Code أو Codex، أو باستخدام CLI العام. كيفية عمل التحديثات وما إذا كنت بحاجة إلى إرسال أي شيء."
---

# المكون الإضافي للخطة والسوق

يأتي تطبيق Jami Studio **Plan** كحزمة واحدة قابلة للتثبيت. يضيف التثبيت الواحد مهارات أوامر Plan المائلة **و** يوصّل موصل Plan MCP المستضاف، حتى يتمكن الوكيل من إنشاء الخطط ويمكن للمهارات نشرها مباشرة في تطبيق Plan.
...
- **موصل Plan MCP** — مسجل في التطبيق المستضاف على `https://plan.jami.studio` (نقطة نهاية MCP `https://plan.jami.studio/_agent-native/mcp`، اسم الخادم `plan`).
...
يؤدي هذا إلى إبقاء محتوى الخطة خارج قاعدة بيانات خطة Jami Studio. المشاركة المستضافة،
...
يحتوي Jami Studio Desktop على مسار مزامنة ملف محلي منفصل للخطط المستضافة:
...
> يحمل المكون الإضافي (`agent-native-visual-plans`) معرف التطبيق `visual-plans`، ولهذا السبب فإن اسم المكون الإضافي Claude Code واسم المكون الإضافي Codex كلاهما `agent-native-visual-plans`. اسم العرض لتطبيق الخطة هو "Jami Studio Plan".
...
يعمل مع أي مضيف — Claude Code وCodex وCursor وCline وGoose وتطبيقات ChatGPT المخصصة MCP وClaude Cowork وأي شيء آخر متوافق مع MCP. يثبت Jami Studio CLI مهارات Plan، ويسجل موصل Plan MCP المستضاف، **ويشغل المصادقة للعميل (العملاء) المحليين المحددين في نفس الخطوة**، حتى لا يصطدم أول استدعاء للأداة بجدار OAuth:
...
- `--no-connect` - تسجيل الموصل دون المصادقة؛ قم بتشغيل `npx @agent-native/core@latest connect https://plan.jami.studio --client all` لاحقًا، أو اختر `--client` الأضيق.
...
> ملاحظة: يقوم `npx skills@latest add Jami Studio/agent-native --skill visual-plan` (Vercel/open Skills CLI) بتثبيت **التعليمات فقط** - ولا يسجل موصل MCP. استخدم Jami Studio CLI أعلاه عندما تريد توصيل الموصل أيضًا.

### رمز Claude (مكون إضافي) {#claude-code}

إن مستودع `Jami Studio/agent-native` العام هو في حد ذاته سوق للمكونات الإضافية لـ Claude Code، لذا يمكنك إضافته مباشرة - دون الحاجة إلى خطوة إنشاء. داخل رمز Claude:
...
بعد التثبيت، **ابدأ سلسلة Codex جديدة** حتى يتم تحميل الأدوات skills وMCP في الجلسة. يشحن البرنامج المساعد موصل URL فقط (`[mcp_servers.plan]` → `https://plan.jami.studio/_agent-native/mcp`)؛ يقوم `codex mcp login plan` بتشغيل التدفق OAuth. يعمل مسار CLI العالمي أعلاه أيضًا مع Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`) إذا كنت تفضل أمرًا واحدًا يتم تثبيته والمصادقة عليه معًا، أو عندما تريد الملفات المحلية أو وضع الاستضافة الذاتية.

> **عمليات التثبيت الأقدم:** إذا كان التكوين الخاص بك لا يزال يحتوي على إدخال `agent-native-plans` يشير إلى نفس URL، أو تشغيل `npx -y @agent-native/core@latest reconnect https://plan.jami.studio --client codex` لـ Codex، أو نفس الأمر مع هدفك `--client`، فسيتم دمجه في اسم `plan` المتعارف عليه.
...
**لا يلزم تقديم أو مراجعة لتوزيع هذا أو تثبيته.** `Jami Studio/agent-native` هو سوق git عام مستضاف ذاتيًا، لذلك يضيفه المستخدمون مباشرة باستخدام الأوامر المذكورة أعلاه على **كل من Claude Code وCodex** — بدون طلب أو موافقة. لا يحتاج مسار CLI العالمي إلى سوق على الإطلاق.
