...
> ملاحظة: يقوم `npx skills@latest add studio-jami/jami-studio --skill visual-plan` (Vercel/open Skills CLI) بتثبيت **التعليمات فقط** - ولا يسجل موصل MCP. استخدم Jami Studio CLI أعلاه عندما تريد توصيل الموصل أيضًا.

### رمز Claude (مكون إضافي) {#claude-code}

إن مستودع `studio-jami/jami-studio` العام هو في حد ذاته سوق للمكونات الإضافية لـ Claude Code، لذا يمكنك إضافته مباشرة - دون الحاجة إلى خطوة إنشاء. داخل رمز Claude:
...
**لا يلزم تقديم أو مراجعة لتوزيع هذا أو تثبيته.** `studio-jami/jami-studio` هو سوق git عام مستضاف ذاتيًا، لذلك يضيفه المستخدمون مباشرة باستخدام الأوامر المذكورة أعلاه على **كل من Claude Code وCodex** — بدون طلب أو موافقة. لا يحتاج مسار CLI العالمي إلى سوق على الإطلاق.
