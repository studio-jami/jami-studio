---
title: "رمز Jami Studio UI"
description: "قم بإنشاء وتخصيص أسطح التعليمات البرمجية Jami Studio باستخدام حزمة UI المشتركة، وجسر مضيف سطح المكتب، ومتجر التشغيل CLI."
---
...
| تريد...                                                                        | استخدام                                |
| ------------------------------------------------------------------------------ | -------------------------------------- |
| عرض نمط Claude/Codex **مساحة عمل الترميز UI**                                  | **رمز Jami Studio UI** (هذه الصفحة)   |
...
يتقارب الانقسام الحالي عن عمد: يعمل الشريط الجانبي للوكيل القياسي وفرق الوكلاء على دورة حياة `run-manager` الأساسية، بينما يستخدم Jami Studio جلسات محلية طويلة التشغيل مدعومة بمخزن تشغيل التعليمات البرمجية القائم على الملفات ومفردات وحدة التحكم المشتركة التي يتم تشغيلها في الخلفية.
...
يمكن للمضيفين مزج مصادر التشغيل في نفس القائمة. جلسات كود Jami Studio المحلية
...
يمكن لرمز Jami Studio استخدام تسجيل دخول Codex CLI محلي بدلاً من مفتاح OpenAI API.
...
الوضع التلقائي هو الوضع الافتراضي لجلسات كود Jami Studio المحلية. استخدم وضع الخطة لـ
...
يتم تخزين عمليات تشغيل كود Jami Studio المحلي في:
...
يستخدم رمز Jami Studio نفس `AgentComposerFrame` + `PromptComposer` /
...
يستخدم كل من وكيل تطوير الشريط الجانبي وكود Jami Studio نفس الحد الأدنى
...
يتعامل كود Jami Studio مع الترحيل كإمكانية، وليس فئة تطبيق منفصلة. يمكن أن يكون `/migrate` هدفًا مدمجًا، أو أمر مشروع، أو حزمة تعليمات مخصصة أعلى نفس عقد المضيف.

### الترحيل إلى Jami Studio باستخدام `/migrate` {#migrate}

`/migrate` هو الهدف المدمج لنقل تطبيق موجود، URL، أو منتج موصوف إلى Jami Studio. إنه هدف مائل في مساحة عمل Code - وليس قالبًا منفصلاً للسقالة وليس منتجًا لمرة واحدة - لذلك فهو يشترك في نفس مخزن الجلسة والنص وعناصر التحكم في التشغيل ومركز سطح المكتب مثل كل جلسة Code أخرى، ويمكنك استئنافها والإرفاق بها وفحصها وإيقافها بنفس الطريقة.
...
عندما يقوم المضيف بتنفيذ `listCodePacks`، يعرض UI المشترك أوامر المشروع وskills في السكة. تُدرج صفوف الأوامر `/<command>`، وتُدرج صفوف المهارات موجه "استخدم مهارة `<skill>`..." المُركز حتى تظل السكة قابلة للتنفيذ. تظل أهداف الشرطة المائلة المضمنة `/migrate` و`/audit` محفوظة لعناصر التحكم في كود Jami Studio العالمية، كما هو الحال مع أسماء عناصر التحكم في التشغيل مثل `status` و`resume` - وهي أوامر فرعية يتم استدعاؤها بدون شرطة مائلة (`npx @agent-native/core@latest code status`، `npx @agent-native/core@latest code resume`)، وليست أهداف الشرطة المائلة.
