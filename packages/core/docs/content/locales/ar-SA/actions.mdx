...
تعريف واحد، سبعة مستهلكين. هذه هي الدرجة الثالثة من [ladder](/docs/what-is-jami#the-ladder).
...
<Endpoint id="doc-block-hmmg3a" title={"نقطة نهاية action المثبتة تلقائيا"} method="GET" path="/_agent-native/actions/get-lead" summary={"كل action يثبت هنا تلقائيا - اسم الملف هو اسم action."} auth={"ملف تعريف ارتباط للجلسة؛ تحمل استدعاءات الواجهة `X-Jami Studio-Frontend: 1`"} params={[
  {
    "name": "leadId",
    "in": "query",
    "type": "string",
...
| `"frontend"` | استدعاء متصفح عبر `useActionMutation` / `useActionQuery` / `callAction` (تم وضع علامة عليها برأس `X-Jami Studio-Frontend: 1`).      |
