---
title: "وكلاء تسخير"
description: "قم بتشغيل Claude Code، وCodex، وPi، وغيرها من أدوات الترميز الكاملة كوكلاء مضمنين داخل Jami Studio، مع الحلقة الخاصة بهم، ووضع الحماية، والأدوات الأصلية، وجلسات SQL المدعومة القابلة للاستئناف."
...
> Codex, Pi) إلى Jami Studio كوكيل. بناء التطبيق؟ ابدأ بـ
...
نموذج الموافقة وسلوك وضع الحماية. يقوم Jami Studio بتشغيل هذه من خلال
...
حلقة خاصة بها من البداية إلى النهاية، لذا فإن Jami Studio يقودها كجلسة، وليس كجلسة واحدة
استدعاء النموذج.

<Diagram id="doc-block-1q7je8a" title={"يمتلك الحزام حلقته. Jami Studio يقود الجلسة"} summary={"تقوم الركيزة AgentHarness creates/resumes بالجلسة الأصلية، بتدفق أحداثها إلى النص العادي، وتستمر في حالة الاستئناف في SQL بين المنعطفات."}>
...
الأسطح المجاورة: ضع الوكيل الذي أنشأته في مكان آخر خلف دردشة Jami Studio
...
يمكن أن يعمل Jami Studio كعميل [ACP](https://agentclientprotocol.com) (العميل
...
- **Jami Studio Code / Desktop** يقوم بتشغيل `codex exec` على جهاز المستخدم. إذا
...
- **`ai-sdk-harness:codex`** يقوم بتحميل `@ai-sdk/harness-codex`، الذي يقوم بتشغيل Codex
  ...
  ينسخ Jami Studio ملف المصادقة Codex CLI المحلي إلى وضع الحماية قبل
...
يمتلك الحزام حالة جلسة أصلية طويلة الأمد. يستمر Jami Studio في SQL
حتى يتمكن الخيط من البقاء عبر المنعطفات والعمليات والنشر. `resumeState`
**غير شفاف** — يقوم Jami Studio بتخزينه وإعادته، ولكن لا يتم فحصه أو فحصه مطلقًا
يفسرها.
...
المجموعة المقصودة\*\* من Jami Studio actions حتى `createSession.tools` عندما
...
تقوم جلسة تسخير ببث قيم `AgentHarnessEvent`، والتي Jami Studio
...
- لا تقم بإعادة تشغيل سجل الدردشة Jami Studio الكامل في كل دورة. استئناف
...
- [Jami Studio Code UI](/docs/code-agents-ui) — سطح مساحة عمل البرمجة القابلة لإعادة الاستخدام.
