...
قائمة ملحق Jami Studio Clips Chrome هي
...
عرض توضيحي مباشر: [clips.jami.studio](https://clips.jami.studio).
...
تطبيق Clips المستضاف على [clips.jami.studio](https://clips.jami.studio)
...
2. **تكوين حالة الإنتاج.** تعيين `DATABASE_URL` المستمر، العادي
   ...
   موفر تخزين الفيديو. Jami Studio Connect هو أسهل مسار للتخزين؛ ل
...
6. **الاتصال بعمليات التكامل الاختيارية.** تعمل Google Calendar على تشغيل علامة التبويب "الاجتماعات"،
   يعمل `GEMINI_API_KEY` أو Jami Studio Connect على تشغيل تنظيف النص والعناوين،
