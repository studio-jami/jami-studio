...
يختلف هذا عن القياس عن بعد للمتصفح الداخلي لإطار العمل (`trackEvent()` / مشاهدات الصفحة التلقائية - راجع [Browser defaults](#browser-defaults) أدناه), والذي يدعم تحليلات المنتج الخاصة بـ Jami Studio. استخدم `track()` لأحداث التحليلات الخاصة بتطبيقك والتي يجب أن تصل إلى مقدمي الخدمة الذين تم تكوينهم.

## إعادة تشغيل الجلسة {#session-replay}

يمكن لتطبيقات Jami Studio الاشتراك في إعادة تشغيل جلسة المتصفح من الطرف الأول دون إضافة SDK تحليلات ثانٍ. استدعِ `configureTracking()` مرة واحدة في جذر المتصفح ومرر مفتاح Analytics العام بالإضافة إلى نقطة نهاية المُجمِّع:
...
تستدعي جذور قوالب Jami Studio بالفعل `configureTracking()`. يمكن لعمليات نشر القوالب المستضافة تفعيل إعادة التشغيل باستخدام متغيرات بيئة Vite/Netlify، بينما يجب أن يُفضّل مستهلكو المكتبة الصيغة الصريحة `configureTracking({ key, endpoint, sessionReplay })` أعلاه:
...
يتتبع إطار العمل أيضًا `builder connect clicked` من Connect Jami Studio CTAs، ومسارات الاتصال Jami Studio من جانب الخادم لتتبع أحداث دورة الحياة التي بدأت/نجحت/فشلت. يتم استدعاء `configureTracking()` تلقائيًا بواسطة إطار العمل؛ لا تحتاج إلى استدعائه في رمز القالب الخاص بك.
