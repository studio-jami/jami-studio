---
title: "تضمين SDK"
description: "قم بتضمين ملف جانبي Jami Studio في تطبيق SaaS موجود مع سياق الصفحة وأوامر المضيف."
---

# تضمين SDK

تضمين Jami Studio في منتج حالي: احتفظ بتطبيق SaaS الخاص بك وأضف طابعًا متينًا
...
<Diagram id="doc-block-rxyzmb" title={"غشاء التضمين"} summary={"يوفر التطبيق المضيف مصادقة من جانب الخادم وسياق الصفحة المباشرة؛ يقوم Jami Studio بتشغيل السيارة الجانبية المتينة ويصل إلى علامة التبويب المفتوحة من خلال إجراءات العميل وأوامر المضيف."}>
...
يقوم المضيف بتوصيل مسارات خادم Jami Studio إلى تطبيقه الحالي، ويمررها
قام المستخدم بتسجيل الدخول إلى Jami Studio، ويعرض الشريط الجانبي React في المنتج UI.
يستخدم Jami Studio نشر المضيف وجلسة المضيف والإعدادات
...
مصادقة المضيف هي من جانب الخادم. لا تمرر الهوية من المتصفح كمصدر للحقيقة؛ استخدم كائن الطلب/الجلسة الخاص بالمضيف أو رمزًا مميزًا قصير العمر تم التحقق منه من قبل الخادم. إذا لم يكشف المضيف عن رسائل البريد الإلكتروني، قم بإرجاع `userId` المستقر وسيستخدمه Jami Studio كمفتاح المالك.

### عزل قاعدة البيانات

يدير الوضع المضمن جداول Jami Studio في SQL. بالنسبة لمنتج SaaS الناضج، فإن الإعداد الافتراضي الأكثر أمانًا هو **نفس الاستضافة والمصادقة، وقاعدة بيانات/مخطط Jami Studio المخصص**:
...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| **منتقي التطبيقات المضمنة**      | إطلاق تطبيق Jami Studio كامل كإطار iframe مركّز (منتقي الأصول، منشئ النماذج، لوحة الموافقة).                           | `@agent-native/embedding`              |
...
تطبيق Jami Studio كسطح iframe مركّز: منتقي الأصول، ومنشئ الأصول،
...
مركز حتى يتمكن `content.jami.studio` و`assets.jami.studio` من ربط المستخدمين حسب
...
داخل السيارة الجانبية Jami Studio، استخدم مساعدات الإطار لطلب سياق المضيف، أو اكتشاف جلسة المتصفح المباشرة actions، أو تشغيلها، أو مطالبة المضيف بتنفيذ عمل UI. اجتاز دائمًا `hostOrigin` المتوقع في الإنتاج:
...
> قم بتفضيل المكوّن الإضافي المضمن بالبطاريات عندما تريد إدارة Jami Studio
...
يقوم المساعد عن عمد بإرجاع كائنات `{ name, description, parameters, execute }` العادية حتى تتمكن السيارات الجانبية من تكييفها مع استدعاء الوظائف AI SDK أو Anthropic أو OpenAI أو شكل Jami Studio `ActionEntry` دون اقتران SDK بوقت تشغيل واحد.

## شكل المنتج الموصى به

بدء تشغيل iframe أولاً. وهو يعمل مع Jami Studio وتطبيقات SaaS للعملاء وأدوات الإدارة الداخلية دون اقتران دورات الإصدار أو افتراضات CSS/وقت التشغيل.

يجب أن تظل السيارة الجانبية نفسها عبارة عن تطبيق/قالب Jami Studio: actions هي سطح API الخلفي، وحالة التطبيق المدعومة من SQL هي ذاكرة الوكيل، ويمكن لعمليات التكامل مثل Slack أو Telegram التوجيه إلى نفس الدردشة الدائمة. يوفر تضمين SDK الغشاء المباشر بين تلك السيارة الجانبية وصفحة المضيف الحالية.
