---
title: "أسطح الوكيل"
description: "استخدم Jami Studio بشكل مستقل، كدردشة غنية، داخل تطبيق موجود، أو كتطبيق أصلي كامل للوكيل."
...
Jami Studio قابل للتأليف بشكل متعمد. يمكنك استخدام الوكيل بدون الكثير من UI،
...
| **دردشة غنية على Jami Studio** | تريد دردشة مستقلة أو مضمنة مدعومة بحلقة الوكيل المضمنة.                                                | [Chat template](/docs/template-chat), `<AgentChatSurface>`, `<AssistantChat>`               |
| **دردشة غنية مع وكيلك**         | لقد أنشأت الوكيل في مكان آخر وتريد مؤلف Jami Studio والنص وبطاقات الأدوات والأدوات الأصلية.           | `AgentChatRuntime`, `<AssistantChat runtime={runtime}>`                                     |
...
وكيل آخر، أو منتج موجود يتصل بـ Jami Studio.
...
الرمز أو Codex أو Pi أو Cursor أو Mastra أو أوقات التشغيل المماثلة داخل تطبيق Jami Studio.
...
## دردشة غنية على Jami Studio {#rich-chat}
...
وقت التشغيل وتريد دردشة Jami Studio UI حوله. `AgentChatRuntime` هو
الحدود: يقوم وقت التشغيل الخاص بك ببث الأحداث الطبيعية، ويعرض Jami Studio
...
ليست دردشة تطبيق المستخدم النهائي أو نقل A2A، ولا Jami Studio حاليًا
...
يقوم المكون الإضافي للخادم بتوصيل مسارات Jami Studio إلى تطبيقك المضيف ويحلها
...
<Diagram id="doc-block-x4te10" title={"كيف يتم ربط السيارة الجانبية بتطبيق مضيف"} summary={"يقوم البرنامج المساعد بتثبيت مسارات Jami Studio من جانب الخادم؛ يقوم React بتدفق سياق الصفحة الجانبية وأوامر المضيف للخارج."}>
...
<div class="diagram-sidecar">
  ...
  <div class="diagram-panel accent-panel">
    ...
    <div class="diagram-box" data-rough>
      Jami Studio الطرق<br /><small class="diagram-muted"
...
| "أريد وكيل إطار العمل، ولكن يجب أن تكون الدردشة هي UI الرئيسية." | دردشة غنية على Jami Studio |
...
## الخطوات التالية {#related-docs}

- [**Actions**](/docs/actions) — عرّف العملية مرة واحدة؛ كل سطح أعلاه يستدعي العملية نفسها
- [**Native Chat UI**](/docs/native-chat-ui) — اعرض نتائج actions كجداول ومخططات وبطاقات في الدردشة
- [**Generative UI**](/docs/generative-ui) — أنشئ واجهة مستخدم مؤقتة أو دائمة ضمن بيئة معزولة داخل الدردشة
- [**Automation-First Apps**](/docs/pure-agent-apps) — النمط الكامل دون متصفح للوظائف وقوائم الانتظار والبرامج النصية والوكلاء الخارجيين
- [**External Agents**](/docs/external-agents) — صِل المضيفين المتوافقين مع MCP بتطبيق
- [**A2A Protocol**](/docs/a2a-protocol) — استدعِ الوكلاء من تطبيقات Jami Studio أخرى
