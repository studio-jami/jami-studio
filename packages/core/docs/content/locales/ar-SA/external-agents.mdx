...
- **وقت التشغيل الخاص بك خلف دردشة Jami Studio:** راجع [Agent Surfaces](/docs/agent-surfaces#byo-agent) و[Native واجهة الدردشة](/docs/native-chat-ui#byo-agent-runtimes) عندما يقوم وكيل تم إنشاؤه باستخدام إطار عمل آخر بتشغيل `<AssistantChat runtime={...}>`.
...
أضف موصل MCP عن بعد إلى المضيف حيث تريد استخدام Jami Studio.
...
**الاتصال**، تسجيل الدخول باستخدام حساب Jami Studio، الموافقة على نطاقات MCP،
...
أدوات المسح/الاكتشاف، وتسجيل الدخول باستخدام Jami Studio، والموافقة على النطاقات، وتمكينها
الموصل في الدردشة.
...
يقوم تطبيق الطرف الأول skills بتثبيت الإرشادات وموصل MCP المستضاف مع Jami Studio CLI:
...
تحتاج إلى موصل مثل `npx @agent-native/core@latest connect https://assets.jami.studio`.
...
يبحث `reconnect` عن أي إدخال تكوين MCP ينتهي URL بـ `/_agent-native/mcp` للمضيف المحدد والعميل المحدد (المطابقة بواسطة URL بغض النظر عن اسم الموصل)، ثم يقوم بتحديث مادة المصادقة أو استبدالها دون لمس skills المثبت أو إعادة تشغيل تدفق التثبيت الكامل. تمرير التطبيق الأساسي URL (على سبيل المثال، `https://plan.jami.studio`) - يتم استنتاج لاحقة `/_agent-native/mcp`. يتم تحميل المصادقة والأداة لكل عميل، لذا قم بإعادة تشغيل/إعادة تحميل هذا العميل بعد ذلك؛ يحتاج Codex إلى جلسة جديدة قبل ظهور الأدوات المحملة حديثًا.
...
- **لكل رمز** — تم إصداره باستخدام `--full-catalog`، والذي يتضمن مطالبة `catalog_scope: "full"` في JWT. تتجاوز الطلبات اللاحقة عامل التصفية المضغوط لهذا الرمز المميز:
  ...
  ```باش
  npx @agent-native/core@latest Connect https://plan.jami.studio --مخطوطة العميل --الكتالوج الكامل
  ```
...
- قم بتوصيل وكيلك الخاص بـ Dispatch باستخدام `npx @agent-native/core@latest connect https://dispatch.jami.studio`؛ استخدم التطبيق المباشر URL فقط عندما تريد تطبيقًا واحدًا معزولًا.
