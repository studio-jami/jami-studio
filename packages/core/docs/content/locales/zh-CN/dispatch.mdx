...
Dispatch 可以是外部代理的单个 MCP 连接器：在 Claude、ChatGPT、Codex 或 Cursor 中添加一次 `https://dispatch.jami.studio/mcp`，一次授权可到达每个授予的工作区应用程序，而不是每个应用程序一个连接器。请参阅 [External Agents](/docs/external-agents) 了解完整的连接流程、应用程序授权、OAuth 和内联 MCP 应用程序预览。
