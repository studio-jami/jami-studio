...
这与框架的内部浏览器遥测（`trackEvent()` /自动页面浏览量 - 请参阅下面的 [Browser defaults](#browser-defaults)）不同，后者为 Jami Studio 自己的产品分析提供支持。将 `track()` 用于您的应用自己的分析事件，这些事件应到达您配置的提供商。

## 会话重放 {#session-replay}

Jami Studio 应用无需添加第二个分析 SDK，即可接入第一方浏览器会话重放功能。在浏览器根部调用一次 `configureTracking()`，并传入 Analytics 公钥和采集端点：
...
Jami Studio 模板根已经调用了 `configureTracking()`。托管模板部署可以通过 Vite/Netlify 环境变量开启重放，而库使用者则应优先使用上面显式的 `configureTracking({ key, endpoint, sessionReplay })` 形式：
...
浏览器端辅助函数还会尽力地、非阻塞地读取当前 Jami Studio 认证会话。默认情况下，重放仅限已登录用户：启用 `sessionReplay` 后，除非会话解析出用户邮箱地址，否则不会开始录制。已登录用户的重放会包含基于邮箱的 `userId`/`userEmail` 以及 `orgId`。
...
该框架还跟踪来自 Connect Jami Studio CTA 的 `builder connect clicked`，服务器端 Jami Studio 连接路由跟踪已启动/成功/失败的生命周期事件。 `configureTracking()`由框架自动调用；您不需要在自己的模板代码中调用它。
