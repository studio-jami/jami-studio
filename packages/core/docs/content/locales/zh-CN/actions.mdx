...
- **MCP 工具** - 暴露给 Claude、ChatGPT 自定义 MCP 应用、Claude 桌面/代码、光标、Codex 和任何其他 MCP 主机。
- **A2A 工具** — 由其他代理本机应用通过 A2A 调用。
- **CLI 命令** - `pnpm action <name>` 用于脚本和开发循环。

一个定义，七个消费者。这是 [ladder](/docs/what-is-jami#the-ladder) 的第 3 级。
...
<div class="diagram-fanout">
  ...
  <div class="diagram-grid">
    ...
    <div class="diagram-node">
      A2A 工具<br /><small class="diagram-muted">其他 Jami Studio 应用</small>
    </div>
    <div class="diagram-node">
...
<Endpoint id="doc-block-gkhn14" title={"自动挂载的 action 端点"} method="GET" path="/_agent-native/actions/get-lead" summary={"每个 action 都会自动挂载在这里 - 文件名就是 action 名称。"} auth={"会话 cookie；前端调用会携带 `X-Jami Studio-Frontend: 1`"} params={[
  {
    "name": "leadId",
    "in": "query",
    "type": "string",
...
| `"frontend"` | 通过 `useActionMutation` / `useActionQuery` / `callAction` 的浏览器调用（用 `X-Jami Studio-Frontend: 1` 标头标记）。 |
...
保留 `link` 作为 CLI 和非 UI MCP 主机的后备；这也是嵌入的启动目标。嵌入桥 - 已签名的嵌入启动会话、移植与受控帧渲染、`ui/*` 主桥、CSP 和高度限制 - 归 [External Agents](/docs/external-agents#mcp-app-bridge) 所有。
