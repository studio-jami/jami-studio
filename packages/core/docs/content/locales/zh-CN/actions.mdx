...
启用 MCP 后，您的 actions 将显示在框架的 MCP 服务器中，位置为 `/mcp`。默认情况下，每个调用者都会获得一个紧凑的目录 - 面向应用程序的内置程序以及模板声明的应用程序 actions - 并且 `tool-search` 始终存在，因此任何其他工具都可以按需访问。完整的操作界面仅在明确选择加入（`--full-catalog` 代币或 `AGENT_NATIVE_MCP_FULL_CATALOG=1`）时提供，并且 `publicAgent.expose` 在公共界面上选择安全读取/摄取工具。请参阅 [MCP Protocol](/docs/mcp-protocol) 了解目录层、身份验证和 `mcpApp` 资源详细信息。
