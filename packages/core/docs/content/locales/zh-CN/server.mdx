...
应用仅提供 API 并不意味着需要 Express 服务器。Jami Studio 应用已经自带 Nitro/H3 服务器，`defineAction()` 是默认的 API 表面。将操作定义在 `actions/` 中；框架会把每个 action 挂载到 `/_agent-native/actions/<name>`，并带有 schema 验证和请求上下文。
...
聊天，包括 Jami Studio、Anthropic、AI SDK 提供商、用户/应用模型默认值，
