...
description: "用于 scheduled jobs、queues、scripts、integration workers 和外部 agents 的无浏览器 Jami Studio apps。"
---

# Automation-First Apps

Automation-first app 是无浏览器的 Jami Studio app，用于 scheduled jobs、queues、scripts、integration workers 和外部 agents。它不是默认入门路径，但在工作流暂时不需要页面时很有用。它仍然是一个真实的应用程序：actions、sessions、application state、run history、settings、credentials 和共享记录均位于 SQL 中。
