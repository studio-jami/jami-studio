...
Jami Studio Clips Chrome 扩展程序列表为
...
现场演示：[clips.jami.studio](https://clips.jami.studio)。
...
1. **视频存储（必需）。** 通过入门向导连接存储后端。最简单的路径是Jami Studio（测试期间免费，一键式）。对于自托管存储，请设置 `S3_ENDPOINT`、`S3_BUCKET`、`S3_ACCESS_KEY_ID`、`S3_SECRET_ACCESS_KEY` 以及可选的 `S3_REGION` 和 `S3_PUBLIC_BASE_URL`。 Cloudflare R2 和 DigitalOcean Spaces 使用带有 `R2_*` 前缀的相同环境变量。
...
[clips.jami.studio](https://clips.jami.studio) 上托管的 Clips 应用
...
2. **配置生产状态。**设置一个持久化的`DATABASE_URL`，正常
   ...
   视频存储提供商。 Jami Studio Connect是最简单的存储路径；对于
...
6. **连接可选集成。** Google Calendar 为“会议”选项卡提供支持，
   `GEMINI_API_KEY` 或 Jami Studio Connect 支持转录清理和标题，
