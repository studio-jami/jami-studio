...
`reconnect` 查找给定主机和选定客户端的 URL 以 `/mcp` 结尾的任何 MCP 配置条目（通过 URL 进行匹配，无论连接器名称如何），然后刷新或替换身份验证材料，而无需触及已安装的 skills 或重新运行完整安装流程。传递基本应用程序 URL（例如 `https://plan.jami.studio`） - 推断出 `/mcp` 后缀。身份验证和工具加载是针对每个客户端的，因此之后重新启动/重新加载该客户端； Codex 在新加载的工具出现之前需要一个新会话。
