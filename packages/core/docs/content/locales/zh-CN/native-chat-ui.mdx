---
title: "原生聊天 UI"
description: "操作声明的本机聊天渲染器、可重用的 DataTable/DataChart 输出，以及 BYO 代理运行时应如何连接到 Jami Studio 聊天。"
---
...
将标准化事件流式传输到 Jami Studio 的对话 UI 中，同时保持
...
<Diagram id="doc-block-zv5z9u" title={"BYO 运行时保留 Jami Studio 聊天 shell"} summary={"您的外部代理通过连接器传输标准化事件； Jami Studio 保留作曲家、成绩单、工具卡、批准和本机小部件。"}>
...
当您希望内置 Jami Studio 传输作为运行时对象时，请使用：
...
拥有整个外部转录本并且只想要 Jami Studio 的作曲家
字段。
