...
### Jami Studio：制作中的可视化编辑 {#visual-editing-in-production}

[Jami Studio](https://www.jami.studio) 通过提供托管云环境来解决此问题，在该环境中，代理保留在生产中修改应用程序的 UI 的能力。将您的存储库连接到 Jami Studio 并直接提示 UI 更改 - 无需重新部署。

**工作原理：**

1. 将您的代理本机存储库连接到 Jami Studio
2. Jami Studio提供具有代理、可视化编辑和实时协作的云框架
...
- [**Frames**](/docs/frames) — 用于生产 UI 编辑的嵌入式代理面板与 Jami Studio 云框架
