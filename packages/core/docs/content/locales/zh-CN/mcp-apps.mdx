...
在 Jami Studio 应用自己的聊天中，首选 [native chat renderers](/docs/native-chat-ui) 作为第一方小部件，例如表格、图表、键入的结果和批准功能可供性。在 Claude、ChatGPT、Copilot、Cursor 和其他兼容主机中使用 MCP 应用程序进行外部/跨主机内联 UI，并使用操作 `link` 作为通用深层链接回退。
