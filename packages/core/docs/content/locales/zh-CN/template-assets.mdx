...
- **生成图像和视频。** Jami Studio 管理的图像生成在启用后可用，Gemini 负责视频生成以及手动图像回退。
...
现场演示：[assets.jami.studio](https://assets.jami.studio)。
...
1. **安装一次。**这会添加技能说明并一起注册托管的 MCP 连接器：
   ...
   ```bash
   npx skills@最新添加Jami Studio/agent-native --技能资产
   ```

   Vercel/open Skills CLI 仅安装指令文件；它没有
   运行 MCP 连接器设置。需要时使用上面的 Jami Studio CLI 路径
   单命令设置。
