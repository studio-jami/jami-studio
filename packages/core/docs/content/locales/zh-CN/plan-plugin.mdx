---
title: "规划插件和市场"
description: "安装 Jami Studio Plan skills（/visual-plan、/visual-recap、/visualize-repo）以及托管 Plan MCP 连接器，可作为 Claude Code 或 Codex 插件安装，也可使用通用 CLI。了解更新如何工作以及是否需要提交任何内容。"
---

# 规划插件和市场

Jami Studio **Plan** 应用以一个可安装的 bundle 提供。一次安装会添加 Plan 斜杠命令 skills，**并**连接托管的 Plan MCP 连接器，让 agent 可以生成 plans，并让 skills 直接发布到 Plan 应用中。
...
- **计划 MCP 连接器** — 针对 `https://plan.jami.studio` 上的托管应用程序进行注册（MCP 端点 `https://plan.jami.studio/_agent-native/mcp`，服务器名称 `plan`）。
...
这会将计划内容排除在 Jami Studio 计划数据库之外。托管共享，
...
Jami Studio 桌面有一个单独的托管计划本地文件同步路径：
桌面应用程序可以将托管计划镜像到本地 MDX 文件并将编辑内容导入回来
无需克隆 Plan 应用程序或运行 CLI。该工作流程保持托管
计划数据库作为事实来源；当目标达到时使用本地文件隐私模式
没有计划数据库写入。

> 插件（`agent-native-visual-plans`）带有app id `visual-plans`，这就是为什么Claude代码插件名称和Codex插件名称都是`agent-native-visual-plans`。计划应用的显示名称为“Jami Studio 计划”。
...
适用于任何 host — Claude Code、Codex、Cursor、Cline、Goose、ChatGPT 自定义 MCP apps、Claude Cowork 以及任何其他 MCP-compatible host。Jami Studio CLI 会安装 Plan skills，注册托管的 Plan MCP 连接器，**并在同一步中为选定的本地客户端运行身份验证**，因此你的第一次工具调用不会遇到 OAuth 墙：
...
- `--no-connect` — 注册连接器而不进行身份验证；稍后运行 `npx @agent-native/core@latest connect https://plan.jami.studio --client all`，或者选择更窄的 `--client`。
...
> 注意：裸机 `npx skills@latest add Jami Studio/agent-native --skill visual-plan`（Vercel/open Skills CLI）安装**仅说明** - 它不注册 MCP 连接器。当您也想连接连接器时，请使用上面的 Jami Studio CLI。

### Claude代码（插件） {#claude-code}

公共 `Jami Studio/agent-native` 存储库本身就是一个 Claude 代码插件市场，因此您可以直接添加它 - 无需构建步骤。 Claude 内部代码：
...
安装后，**启动新的 Codex 线程**，以便 skills 和 MCP 工具加载到会话中。该插件附带一个仅限 URL 的连接器（`[mcp_servers.plan]` → `https://plan.jami.studio/_agent-native/mcp`）； `codex mcp login plan` 运行 OAuth 流程。如果您更喜欢使用一个命令来同时安装和身份验证，或者需要本地文件或自托管模式，则上面的通用 CLI 路由也适用于 Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`)。

> **较旧的安装：**如果您的配置仍然有一个 `agent-native-plans` 条目指向相同的 URL，为 Codex 运行 `npx -y @agent-native/core@latest reconnect https://plan.jami.studio --client codex`，或者与您的目标 `--client` 运行相同的命令，则将其合并为规范的 `plan` 名称。
...
**分发或安装此程序不需要提交或审核。** `Jami Studio/agent-native` 是一个自托管的公共 git 市场，因此用户可以使用上面的命令直接在 **Claude 代码和 Codex** 上添加它 - 无需申请或批准。通用 CLI 路线根本不需要市场。
