...
- **计划 MCP 连接器** — 针对 `https://plan.jami.studio` 上的托管应用程序进行注册（MCP 端点 `https://plan.jami.studio/mcp`，服务器名称 `plan`）。
...
安装后，**启动新的 Codex 线程**，以便 skills 和 MCP 工具加载到会话中。该插件附带一个仅限 URL 的连接器（`[mcp_servers.plan]` → `https://plan.jami.studio/mcp`）； `codex mcp login plan` 运行 OAuth 流程。如果您更喜欢使用一个命令来同时安装和身份验证，或者需要本地文件或自托管模式，则上面的通用 CLI 路由也适用于 Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`)。
