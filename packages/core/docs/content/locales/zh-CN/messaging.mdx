...
- **Clips 应用程序展开** — 适用于 Slack 的可安装 Jami Studio Clips 使用 `SLACK_CLIENT_ID`、`SLACK_CLIENT_SECRET`、`SLACK_SIGNING_SECRET` 和 `/api/slack/oauth/callback`。每个连接的 Slack 工作空间都会在 `app_secrets` 中获得自己的加密机器人令牌； `SLACK_BOT_TOKEN` 只是传统的单一工作区后备方案。
...
Jami Studio 图标，使 Agents 选择器与产品品牌一致。可以使用应用公开的
...
个人 Slack 私信只接受已验证的 Jami Studio 组织成员，或已关联到该成员的 Slack
