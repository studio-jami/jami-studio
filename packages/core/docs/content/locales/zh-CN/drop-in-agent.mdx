...
将工作交给代理的按钮（而不是运行内联 `llm()` 调用 - [ladder](/docs/what-is-jami#the-ladder) 的反模式）：
...
- **自带代理运行时。**如果您在其他地方构建的代理应该
  在 Jami Studio 保留作曲家、文字记录和工具的同时推动对话
...
当 UI 需要运行代理工具将运行的相同操作（[ladder](/docs/what-is-jami#rung-three) 的梯级 3）时，请使用 `useActionMutation`：
