...
## 完整的 Agent 工作区 {#agent-page}

当完整应用需要一个持久位置来检查和配置代理时，请在 `/agent` 挂载
`AgentTabsPage`。将此路由加入应用导航，并向 `AgentSidebar` 传入
`agentPageHref="/agent"`，这样 Resources 和 Settings 模式就可以链接到完整
页面，而不必重复这些流程。
...
- **Files** — 为个人或组织资源重新托管现有的 `ResourcesPanel`。
- **Connections** — MCP 服务器管理和 A2A 远程代理列表，即此应用的代理可以调用什么。
- **Jobs** — 使用现有暂停/恢复、详情和删除流程的范围化定期作业与自动化。自动化目前仅属于个人。
- **Access** — MCP URL、可用时的 A2A 代理卡片以及共享客户端设置指南；完整连接流程和令牌回退位于 `/mcp/connect`。

页面级 **Personal / Organization** 切换器为选项卡提供范围。成员可以检查组织
数据；在托管组件实施该边界的地方，组织管理仍仅限所有者/管理员。目标是让上下文
可检查、可归因、可治理，并将能力与访问分开。该页面只是对现有组件、操作和
访问检查的薄封装，不是新的管理控制台。目前还没有 grants、范围编辑、撤销
界面或逐迭代 provenance 历史。
...
- [**A2A Protocol**](/docs/a2a-protocol) — 从其他 Agent-Native 应用调用代理
