---
title: "特工表面"
description: "将 Jami Studio 无头使用，作为丰富的聊天，在现有应用程序中，或作为完整的代理本机应用程序。"
search: "无头代理丰富聊天完整应用程序 BYO 代理运行时 AgentChatRuntime 嵌入 actions MCP A2A HTTP CLI"
---

# 特工表面

Jami Studio 是故意可组合的。不用太多就可以使用代理UI，
...
| **Jami Studio 上的丰富聊天** | 您想要由内置代理循环支持的独立或嵌入式聊天。                                     | [Chat template](/docs/template-chat), `<AgentChatSurface>`, `<AssistantChat>`               |
| **与您的代理进行丰富的聊天**  | 您在其他地方构建了代理，并想要 Jami Studio 的编写器、脚本、工具卡和本机小部件。 | `AgentChatRuntime`, `<AssistantChat runtime={runtime}>`                                     |
...
Jami Studio 应用内的代码、Codex、Pi、Cursor、Mastra 或类似运行时。
...
运行时，你想要 Jami Studio 的聊天 UI 围绕它。 `AgentChatRuntime` 是
边界：您的运行时流规范化事件，Jami Studio 呈现
...
不是最终用户应用聊天或 A2A 传输，并且 Jami Studio 目前没有
...
服务器插件将 Jami Studio 路由安装到您的主机应用程序中并解析
主机身份服务器端：
...
<Diagram id="doc-block-oblpnz" title={"Sidecar 如何桥接到主机应用程序"} summary={"该插件在服务器端挂载 Jami Studio 路由； React sidecar 流输入页面上下文并输出主机命令。"}>
...
| “我想要框架的代理，但是聊天应该是主要的UI。”         | Jami Studio 上的丰富聊天 |
...
- [**Actions**](/docs/actions) — 只需定义一次操作；上述每个界面都会调用同一个操作
- [**Native 聊天界面**](/docs/native-chat-ui) — 在聊天中将类型化的 action 结果呈现为表格、图表和卡片
- [**Generative UI**](/docs/generative-ui) — 在聊天中生成临时或持久化的沙盒 UI
- [**Automation-First Apps**](/docs/pure-agent-apps) — 面向作业、队列、脚本和外部代理的完整无浏览器模式
- [**External Agents**](/docs/external-agents) — 将 MCP 兼容主机连接到应用
- [**A2A Protocol**](/docs/a2a-protocol) — 从其他 Jami Studio 应用调用代理
