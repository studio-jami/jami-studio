...
Dispatch 提供了控制平面 actions，它们使用应用程序可直接调用的相同共享存储函数 — 其中 `upsert-workspace-connection` action 会根据提供商目录验证输入，然后委托给本页所示的同一个 `upsertWorkspaceConnection()`：
...
- [**Dispatch**](/docs/dispatch) — 通常负责工作区连接设置和保管库的应用
- [**安全**](/docs/security) — 连接所指向的凭证保管库、机密作用域和访问模型
- [**入门设置**](/docs/onboarding#checking-workspace-connections-in-onboarding) — 从模板设置清单检查连接就绪状态
- [**MCP 客户端**](/docs/mcp-clients) — 在工作区中共享 MCP 服务器凭证的相关可复用配置模式
- [**Brain**](/docs/template-brain) — 基于此模型构建提供商 Sources 页面的参考实现
