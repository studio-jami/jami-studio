...
你不必这样做。每个示例应用还可以作为 `jami.studio` 上的托管应用程序使用 - `mail.jami.studio`、`calendar.jami.studio` 等。免费或付费使用托管版本；仅当您想要更改托管版本未公开的内容时才进行分叉。
...
您将获得一个多选选择器。选择一个应用程序（独立）或多个应用程序（工作区 - 应用程序共享身份验证、品牌、代理配置和数据库）。每个挑选的应用都与您需要的每个文件一起构建到 `apps/<name>/` 中。对于没有浏览器 UI 的自动化应用，而不是应用 UI，请使用 `npx @agent-native/core@latest create my-agent --headless`。
