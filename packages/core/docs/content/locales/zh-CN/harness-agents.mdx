---
title: "安全带特工"
description: "将 Claude Code、Codex、Pi 和其他完整的编码工具作为 Jami Studio 内的嵌入式代理运行，并具有自己的循环、沙箱、本机工具和可恢复的 SQL 支持的会话。"
...
自己的端到端循环，因此 Jami Studio 将其作为会话驱动，而不是单个
模型调用。

<Diagram id="doc-block-1j8ms62" title={"安全带拥有它的环； Jami Studio 驱动会话"} summary={"AgentHarness 底层 creates/resumes 本机会话，将其事件流式传输到正常转录本中，并在轮次之间将恢复状态保留在 SQL 中。"}>
...
Jami Studio可以充当[ACP](https://agentclientprotocol.com)（代理客户端
...
- **Jami Studio 代码/桌面** 在用户计算机上运行 `codex exec`。如果
...
- **`ai-sdk-harness:codex`**加载`@ai-sdk/harness-codex`，它驱动Codex
  ...
  Jami Studio 将本地 Codex CLI auth 文件复制到沙箱中
...
线束拥有长期存在的本机会话状态。 Jami Studio 将其保留在 SQL
因此线程可以在轮流、进程和部署中生存。 `resumeState`
是**不透明** - Jami Studio 存储它并将其返还，但从不检查或
解释它。
...
当您通过 `createSession.tools` 有意设置** Jami Studio actions
...
线束会话流式传输 `AgentHarnessEvent` 值，其中 Jami Studio
...
- 请勿将 Claude 代码、Codex、光标、Mastra 或 Pi 添加为 `AgentEngine`。他们
  ...
  并丢失会话生命周期语义。
- 不要每回合将完整的 Jami Studio 聊天历史记录重播到安全带中。简历
...
- [Jami Studio Code UI](/docs/code-agents-ui) — 可重用的编码工作区表面。
