...
Jami Studio 附带了完整的侧边栏，但侧边栏不是合同。
...
| `createAgentChatAdapter()`           | `@agent-native/core/client` 或 `/client/chat` | 您需要内置 Jami Studio SSE 传输作为低级助手 UI 适配器。    |
