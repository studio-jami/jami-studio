---
title: "एम्बेडिंग SDK"
description: "पेज संदर्भ और होस्ट कमांड के साथ मौजूदा SaaS ऐप में एक Jami Studio साइडकार एम्बेड करें।"
---

# एम्बेडिंग SDK

Jami Studio को मौजूदा उत्पाद में एम्बेड करें: अपना SaaS ऐप रखें, एक टिकाऊ जोड़ें
...
<Diagram id="doc-block-dp2jx4" title={"एम्बेडिंग झिल्ली"} summary={"होस्ट ऐप सर्वर-साइड ऑथ और लाइव पेज संदर्भ प्रदान करता है; Jami Studio टिकाऊ साइडकार चलाता है और क्लाइंट क्रियाओं और होस्ट कमांड के माध्यम से खुले टैब तक पहुंचता है।"}>
...
होस्ट अपने मौजूदा ऐप में Jami Studio सर्वर रूट को माउंट करता है, उसे पास करता है
उपयोगकर्ता ने Jami Studio में लॉग-इन किया, और उत्पाद UI में React साइडबार प्रस्तुत किया।
Jami Studio होस्ट परिनियोजन, होस्ट सत्र और कॉन्फ़िगर का उपयोग करता है
...
होस्ट प्रमाणीकरण सर्वर-साइड है। सत्य के स्रोत के रूप में ब्राउज़र से पहचान को पारित न करें; होस्ट के अनुरोध/सत्र ऑब्जेक्ट या अल्पकालिक सर्वर-सत्यापित टोकन का उपयोग करें। यदि होस्ट ईमेल को उजागर नहीं करता है, तो एक स्थिर `userId` लौटाएं और Jami Studio इसे स्वामी कुंजी के रूप में उपयोग करेगा।

### डेटाबेस अलगाव

एंबेडेड मोड SQL में Jami Studio तालिकाओं का प्रबंधन करता है। एक परिपक्व SaaS उत्पाद के लिए, सबसे सुरक्षित डिफ़ॉल्ट **समान होस्टिंग और प्रमाणीकरण, समर्पित Jami Studio डेटाबेस/स्कीमा** है:
...
होस्ट उत्पाद के मुख्य `DATABASE_URL` का उपयोग समर्थित है, लेकिन इसे एक स्पष्ट विकल्प बनाएं। Jami Studio `settings`, `application_state`, `tools`, `tool_data`, ब्राउज़र-सत्र टेबल, रहस्य, चैट थ्रेड और संबंधित इंडेक्स जैसे फ्रेमवर्क टेबल बनाता है। एक समर्पित DB/स्कीमा तालिका-नाम टकराव से बचाता है, प्रबंधित तालिकाओं के स्वामित्व को स्पष्ट रखता है, और बैकअप/प्रतिधारण नीति के बारे में तर्क करना आसान बनाता है। यदि आप जानबूझकर होस्ट DB साझा करते हैं, तो पहले मौजूदा तालिका नामों की समीक्षा करें और Jami Studio तालिकाओं को फ़्रेमवर्क-स्वामित्व वाली मानें।
...
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| **एंबेडेडऐप पिकर**              | एक केंद्रित आईफ्रेम (एसेट पिकर, फॉर्म बिल्डर, अनुमोदन पैनल) के रूप में एक पूर्ण Jami Studio ऐप लॉन्च करना।                  | `@agent-native/embedding`                   |
...
Jami Studio ऐप एक केंद्रित आईफ्रेम सतह के रूप में: एक परिसंपत्ति पिकर, परिसंपत्ति जनरेटर,
...
हब ताकि `content.jami.studio` और `assets.jami.studio` उपयोगकर्ताओं को लिंक करें
...
Jami Studio साइडकार के अंदर, होस्ट संदर्भ का अनुरोध करने, लाइव ब्राउज़र-सत्र actions की खोज करने, उन्हें चलाने, या होस्ट को UI कार्य करने के लिए कहने के लिए फ़्रेम हेल्पर्स का उपयोग करें। उत्पादन में हमेशा अपेक्षित `hostOrigin` पास करें:
...
बैकएंड actions किसी भी चीज़ के लिए डिफ़ॉल्ट होना चाहिए जो रिफ्रेश, बंद ब्राउज़र, पुनः प्रयास, या एकीकरण-ट्रिगर रन से बचे रहना चाहिए। वे साइडकार ऐप के सामान्य Jami Studio एक्शन/टूल लेयर से संबंधित हैं, जहां एजेंट उन्हें चैट, ऑटोमेशन, Slack/टेलीग्राम/ईमेल इंटीग्रेशन और बैकग्राउंड जॉब्स से कॉल कर सकता है।
...
> जब आप Jami Studio को प्रबंधित करना चाहते हैं तो बैटरी-शामिल प्लगइन को प्राथमिकता दें
...
सहायक जानबूझकर सादे `{ name, description, parameters, execute }` ऑब्जेक्ट लौटाता है ताकि साइडकार उन्हें इस SDK को एक रनटाइम में जोड़े बिना AI SDK, एंथ्रोपिक, OpenAI फ़ंक्शन कॉलिंग, या Jami Studio `ActionEntry` आकार में अनुकूलित कर सकें।

## अनुशंसित उत्पाद आकार

iframe-पहले प्रारंभ करें। यह Jami Studio, ग्राहक SaaS ऐप्स और आंतरिक व्यवस्थापक टूल के लिए रिलीज चक्र या CSS/रनटाइम धारणाओं को जोड़े बिना काम करता है।

साइडकार अभी भी एक Jami Studio ऐप/टेम्पलेट होना चाहिए: actions बैकएंड API सतह है, SQL-समर्थित ऐप स्टेट एजेंट की मेमोरी है, और Slack या टेलीग्राम जैसे एकीकरण उसी टिकाऊ चैट में रूट कर सकते हैं। एम्बेडिंग SDK उस साइडकार और वर्तमान होस्ट पेज के बीच लाइव झिल्ली की आपूर्ति करता है।
