...
- **Jami Studio चैट के पीछे आपका अपना रनटाइम:** [Agent Surfaces](/docs/agent-surfaces#byo-agent) और [Native चैट UI](/docs/native-chat-ui#byo-agent-runtimes) देखें जब किसी अन्य ढांचे के साथ निर्मित एजेंट को `<AssistantChat runtime={...}>` को पावर देना चाहिए।
...
उस होस्ट में एक रिमोट MCP कनेक्टर जोड़ें जहां आप Jami Studio का उपयोग करना चाहते हैं।
...
**कनेक्ट**, अपने Jami Studio खाते से साइन इन करें, MCP दायरे को मंजूरी दें,
...
उपकरणों को स्कैन/खोजें, Jami Studio के साथ साइन इन करें, स्कोप स्वीकृत करें और सक्षम करें
चैट में कनेक्टर.
...
प्रथम-पक्ष ऐप skills निर्देशों और होस्ट किए गए MCP कनेक्टर को Jami Studio CLI के साथ इंस्टॉल करें:
...
`npx @agent-native/core@latest connect https://assets.jami.studio` जैसे कनेक्टर की आवश्यकता है।
...
`reconnect` किसी भी MCP कॉन्फ़िगरेशन प्रविष्टि को ढूंढता है जिसका URL दिए गए होस्ट और चयनित क्लाइंट के लिए `/_agent-native/mcp` में समाप्त होता है (कनेक्टर नाम की परवाह किए बिना URL द्वारा मिलान), फिर आपके स्थापित skills को छूने या पूर्ण इंस्टॉल प्रवाह को फिर से चलाने के बिना प्रामाणिक सामग्री को ताज़ा या प्रतिस्थापित करता है। आधार ऐप URL (जैसे `https://plan.jami.studio`) पास करें - `/_agent-native/mcp` प्रत्यय का अनुमान लगाया गया है। प्रमाणीकरण और टूल लोडिंग प्रति क्लाइंट है, इसलिए बाद में उस क्लाइंट को पुनरारंभ/पुनः लोड करें; नए लोड किए गए टूल प्रदर्शित होने से पहले Codex को एक नए सत्र की आवश्यकता है।
...
यह नीति का सामान्य उदाहरण है। Analytics जानबूझकर `authenticatedReads: "off"` सेट करता है और केवल समीक्षा किए गए `connectorCatalog` actions को उपलब्ध कराता है।
...
`resolveExecutionContext` देने वाला ऐप इस डिफ़ॉल्ट पहचान क्रम को स्पष्ट रूप से बदल देता है। उसके resolver को principal लौटाने से पहले असत्यापित callers, guests, external members और unlinked workspaces को स्वतंत्र रूप से अस्वीकार करना होगा।

एजेंट चलने से पहले सत्यापित Slack DM को Jami Studio संगठन के किसी मौजूदा सदस्य से मिलाया जाता है और workspace/user पहचान लिंक सहेजा जाता है। इससे बने user/org संदर्भ में उस उपयोगकर्ता के संसाधन, निर्देश और skills लोड होते हैं। डिफ़ॉल्ट रूप से सफलतापूर्वक प्रोफ़ाइल मिले ऐसे workspace सदस्य अस्वीकार किए जाते हैं जिनका ईमेल उपलब्ध नहीं है या जो अभी संगठन के सदस्य नहीं हैं। ऐप इन सदस्यों को संगठन-स्कोप वाले अनाम service principal के रूप में चलाने के लिए स्पष्ट रूप से `allowAnonymousOrgScopedSlackDm: true` सेट कर सकते हैं — साझा चैनलों वाली संगठन-व्यापी दृश्यता के उसी स्तर पर, लेकिन कभी भी उपयोगकर्ता-निजी पहुँच के साथ नहीं — साथ में एजेंट को दिखाई देने वाला नोट और व्यक्तिगत पहुँच पाने का तरीका बताने वाली एक बार की Slack सूचना। यह opt-in प्रोफ़ाइल लोड न होने, असत्यापित callers, guests, external (Slack Connect) members या किसी संगठन से न जुड़े workspace को कभी अनुमति नहीं देता; उन्हें विनम्र अस्वीकृति मिलती है। साझा Slack चैनल service principal का उपयोग करते हैं और किसी प्रतिभागी की निजी permissions नहीं लेते। Managed OAuth और generated Slack app manifest दोनों bot scope `users:read.email` मांगते हैं। नया scope देने के लिए मौजूदा installations को reconnect/reinstall करना होगा; पुराने bot-token installations को इसे Slack में manually जोड़ना होगा। इसके बिना DM fail closed होते हैं, जब तक ऐप स्पष्ट रूप से `allowAnonymousOrgScopedSlackDm` सक्षम न करे।
...
- **प्रति टोकन** - `--full-catalog` के साथ टकसाल, जो JWT में `catalog_scope: "full"` दावा एम्बेड करता है। बाद के अनुरोध उस टोकन के लिए कॉम्पैक्ट फ़िल्टर को बायपास करते हैं:
  ...
  ```बैश
  npx @agent-native/core@latest कनेक्ट https://plan.jami.studio --क्लाइंट कोडेक्स --फुल-कैटलॉग
  ```
...
पूर्ण-ऐप एम्बेड ब्राउज़र को MCP बियरर टोकन सौंपने से भी बचते हैं। MCP कॉलर SQL में एक बार एम्बेड टिकट बनाता है; आईफ्रेम लॉन्च रूट इसका उपभोग करता है और एक अल्पकालिक, आईफ्रेम-सुरक्षित ब्राउज़र सत्र कुकी सेट करता है। लैंडिंग URL में एक अस्थायी `__an_embed_token` क्वेरी पैरामीटर होता है जो क्लाइंट के लिए इसे कैप्चर करने, एड्रेस बार से हटाने और तृतीय-पक्ष कुकीज़ अवरुद्ध होने पर इसे समान-मूल `fetch` कॉल में संलग्न करने के लिए पर्याप्त होता है। एंबेड सत्र रूट-स्कोप वाले होते हैं; ऐप फ़ेच में वर्तमान एम्बेडेड लक्ष्य शामिल होता है, और सर्वर खनन किए गए रूट के बाहर टोकन के पुन: उपयोग को अस्वीकार कर देता है। ऐप पेज जानबूझकर `X-Frame-Options` या CSP `frame-ancestors` उत्सर्जित नहीं करते हैं, इसलिए Jami Studio, डिज़ाइन और MCP ऐप होस्ट उन्हें आईफ्रेम कर सकते हैं। क्रॉस-ओरिजिनल पृथक होस्ट के लिए आवश्यकता पड़ने पर ब्राउज़र iframe नेविगेशन COEP/CORP में भी ऑप्ट इन करता है।
...
- `npx @agent-native/core@latest connect https://dispatch.jami.studio` के साथ प्रेषण के लिए अपने स्वयं के एजेंट को कनेक्ट करें; डायरेक्ट ऐप URL का उपयोग केवल तभी करें जब आपको एक अलग ऐप चाहिए।
