...
その必要はありません。すべてのサンプルアプリは、`jami.studio` — `mail.jami.studio`、`calendar.jami.studio` などでホストされるアプリとしても利用できます。ホストされたバージョンを無料または有料で使用します。ホストされたバージョンが公開していないものを変更したい場合にのみフォークしてください。
