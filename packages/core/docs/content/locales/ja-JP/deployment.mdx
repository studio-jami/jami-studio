...
[Jami Studio](https://www.builder.io) は、エージェントが運用環境でアプリの UI を変更できる機能を保持するマネージド クラウド環境を提供することで、この問題を解決します。リポジトリを Jami Studio に接続し、UI の変更を直接要求します。再デプロイは必要ありません。
