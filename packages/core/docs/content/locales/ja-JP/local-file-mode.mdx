...
Codex、Claude コード、Jami Studio サイドバー エージェント、または通常のエージェントでも編集できます
編集者。
...
または Jami Studio デスクトップ ブリッジがそれらを公開します。橋が稼働していない場合、スキル
...
プレーン ホスト ブラウザー フォルダー アクセス。 `content.jami.studio` を開くと、
...
コンテンツをローカルに保存するか、信頼できるローカル ブリッジができるように Jami Studio デスクトップを使用します
