---
title: "エージェント サーフェス"
description: "Jami Studio をヘッドレスで、リッチ チャットとして、既存のアプリ内で、または完全なエージェント ネイティブ アプリケーションとして使用します。"
search: "ヘッドレス エージェント リッチ チャット フル アプリ BYO エージェント ランタイム AgentChatRuntime 埋め込み actions MCP A2A HTTP CLI"
---

# エージェント サーフェス

Jami Studio は意図的に構成可能です。 UI をあまり必要とせずにエージェントを使用できます。
...
| **Jami Studio でのリッチなチャット** | 組み込みエージェント ループを利用したスタンドアロン チャットまたは埋め込みチャットが必要です。                                    | [Chat template](/docs/template-chat), `<AgentChatSurface>`, `<AssistantChat>`               |
| **エージェントでのリッチ チャット**   | 他の場所でエージェントを構築し、Jami Studio のコンポーザー、トランスクリプト、ツール カード、ネイティブ ウィジェットが必要です。 | `AgentChatRuntime`, `<AssistantChat runtime={runtime}>`                                     |
...
別のエージェント、または Jami Studio を呼び出す既存の製品。
...
Jami Studio アプリ内のコード、Codex、Pi、Cursor、Mastra、または同様のランタイム。
...
## Jami Studio での充実したチャット {#rich-chat}
...
ランタイムで、Jami Studio のチャット UI が必要です。 `AgentChatRuntime` は
境界: ランタイムは正規化されたイベントをストリームし、Jami Studio は
...
エンドユーザー アプリのチャットや A2A トランスポートではなく、Jami Studio は現在サポートされていません
...
サーバー プラグインは、Jami Studio ルートをホスト アプリにマウントし、解決します
ホスト ID サーバー側:
...
<Diagram id="doc-block-vsm8e2" title={"サイドカーがホスト アプリにブリッジする方法"} summary={"プラグインは Jami Studio ルーティングをサーバー側にマウントします。 React サイドカーは、ページ コンテキストをストリーム入力し、ホスト コマンドを出力します。"}>
...
<div class="diagram-sidecar">
  ...
  <div class="diagram-panel accent-panel">
    ...
    <div class="diagram-box" data-rough>
      Jami Studio ルート<br /><small class="diagram-muted"
...
| 「フレームワークのエージェントが必要ですが、チャットをメインの UI にする必要があります。」 | Jami Studio での充実したチャット |
...
- [**Actions**](/docs/actions) — 操作を一度定義します。上記のすべてのサーフェスが同じ操作を呼び出します
- [**Native チャット UI**](/docs/native-chat-ui) — 型付き action 結果を表、グラフ、カードとしてチャットに表示します
- [**Generative UI**](/docs/generative-ui) — 一時的または永続化されたサンドボックス UI をチャット内に生成します
- [**Automation-First Apps**](/docs/pure-agent-apps) — ジョブ、キュー、スクリプト、外部エージェント向けの完全なブラウザー不要パターン
- [**External Agents**](/docs/external-agents) — MCP 互換ホストをアプリに接続します
- [**A2A Protocol**](/docs/a2a-protocol) — 他の Jami Studio アプリからエージェントを呼び出します
