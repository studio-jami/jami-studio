---
title: "ネイティブ チャット UI"
description: "アクション宣言されたネイティブ チャット レンダラー、再利用可能な DataTable/DataChart 出力、および BYO エージェント ランタイムが Jami Studio チャットに接続する方法。"
---
...
正規化されたイベントを Jami Studio の会話 UI にストリーミングし、
...
<Diagram id="doc-block-nv1xto" title={"BYO ランタイムは Jami Studio チャット シェルを保持します"} summary={"外部エージェントは、コネクタを介して正規化されたイベントをストリーミングします。 Jami Studio は、コンポーザー、トランスクリプト、ツール カード、承認、およびネイティブ ウィジェットを保持します。"}>
...
組み込みの Jami Studio トランスポートをランタイム オブジェクトとして使用する場合は、次を使用します。
...
外部トランスクリプト全体を所有しており、Jami Studio の作曲者のみを必要としています
フィールド。
