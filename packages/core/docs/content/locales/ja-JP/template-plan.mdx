...
`https://plan.jami.studio/mcp` — クライアント固有のセットアップについては、[MCP Clients](/docs/mcp-clients) を参照してください。
...
- アプリ自体のデフォルト チャット モデルは Sonnet 5 です。いつでも **[設定]** で変更できます。Haiku はより安価な Claude の選択肢で、Luna は GPT-5.6 ファミリーの中でより安価な選択肢です。
...
ホストされたプラン コネクタは `https://plan.jami.studio/mcp` にあります。
