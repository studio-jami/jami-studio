...
Jami Studio には完全なサイドバーが同梱されていますが、サイドバーは契約ではありません。
...
| `createAgentChatAdapter()`           | `@agent-native/core/client` または `/client/chat` | 低レベルのアシスタント UI アダプターとして、組み込みの Jami Studio SSE トランスポートが必要です。            |
...
Jami Studio が作曲家、トランスクリプト、ツール カード、およびツール カードを保持している間の会話
