...
ライブデモ: [mail.jami.studio](https://mail.jami.studio)。

> **Google は警告を表示する場合があります:** ホストされたデモでは、Gmail アクセスに Jami Studio の共有 Google アプリを使用するため、続行する前に Google が確認を求める場合があります。ローカルで実行して、独自の Google OAuth クライアントを使用します。
