...
エージェント ネイティブの背後にある決定的な考え方は、エージェントと UI が **対等なパートナーである**ということです。その理由については、[What Is Jami?](/docs/what-is-jami) を参照してください。このセクションは、その約束の残り半分、つまり、エージェントがアプリの隣にドッキングされたときに実際にエージェントを操作するとどのような感じになるかについて説明します。
...
**Voice.** 作曲家はマイクを持っています。 Jami Studio がホストするトランスクリプションから、独自のキーの持ち込み、ブラウザのフォールバックまで、プロバイダー オプションを使用して、リクエストを入力する代わりに口述入力します。
