...
変更を加える必要がある人全員が git に慣れているわけではありません。 [Jami Studio](https://www.jami.studio) は、内部で git ブランチにマップするビジュアル ブランチ モデルをサポートしています。これは、コンテンツとコピーの変更、レイアウト調整、設計の反復、開発環境を使用しない A/B テストに役立ちます。
