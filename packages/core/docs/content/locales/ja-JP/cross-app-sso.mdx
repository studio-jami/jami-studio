...
`*.jami.studio` でホストされている各アプリは、**独自の個別のユーザー ストア**を使用して独自の展開を実行します。 `mail.jami.studio` と `calendar.jami.studio` は、データベース、セッション テーブル、または Cookie ドメインを共有しません。したがって、「一度サインインすればすべてのアプリを使用」を共有 Cookie にすることはできません。ワークスペースの ID 権限として [Dispatch](/docs/dispatch) が機能する **ID フェデレーション** である必要があります。
...
1. **アプリ → Dispatch（認可）。** アプリ側の `GET /_agent-native/identity/login` ルート（「Jami Studio でサインイン」ボタンのリンク先）がリダイレクトを構築し、ユーザーを ID 機関に送ります。
...
<Endpoint id="cross-app-sso-identity-authorize-ja-jp" title={"ID 承認エンドポイント"} method="GET" path="/_agent-native/identity/authorize" summary={"ディスパッチ (ID 機関) が人間を認証し、署名された ID トークンでリダイレクトします"} auth={"セッションをディスパッチします (ない場合は対話型ログイン)"} params={[
  ...
  },
  {
    ...
    "type": "string",
    "required": true,
    "description": "アプリ コールバック URL。厳格な許可リスト (デフォルトでは `*.jami.studio` または localhost) に対して検証されます。"
  },
  {
    "name": "state",
    "in": "query",
...
- **Set** → アプリには、上記のフローを実行する **「Jami Studio でサインイン」** オプションが表示されます。直接ローカル ログイン (電子メール/パスワード、Google) は引き続き機能します。
...
- **厳格な `redirect_uri` ホワイトリスト。** デフォルトでは、ディスパッチは `*.jami.studio` またはローカルホストにのみリダイレクトされます。任意の、スキーム相対 (`//host`) およびクロスオリジン リダイレクト ターゲットは拒否されるため、権限をオープン リダイレクトまたはトークン抽出オラクルに変えることはできません。
...
あらゆる Dispatch デプロイメントが ID ハブとして機能できます。`dispatch.jami.studio` に限定されません。 Dispatch インスタンスを指すように各クライアント アプリで `AGENT_NATIVE_IDENTITY_HUB_URL` を設定します。
...
- **デフォルト:** `*.jami.studio` および localhost のみ (`DEFAULT_ALLOWED_HOST_SUFFIXES` 定数)。
...
`IDENTITY_SSO_ALLOWED_HOST_SUFFIXES` がない場合、セルフホスト型ディスパッチは `*.jami.studio` 上のアプリにのみトークンを発行できます。 Dispatch デプロイメントで環境変数を設定して、他のドメインのロックを解除します。
...
- アプリから**ログアウト**します。
- ログイン画面に **「Jami Studio でサインイン」** と表示されます。クリックしてください。
