...
アプリが API 専用だからといって、Express サーバーは必要ありません。Jami Studio アプリには Nitro/H3 サーバーがすでに含まれており、`defineAction()` がデフォルトの API サーフェスです。操作は `actions/` に定義します。フレームワークは、各 action をスキーマ検証とリクエスト コンテキスト付きで `/_agent-native/actions/<name>` にマウントします。
...
チャット (Jami Studio、Anthropic、AI SDK プロバイダー、ユーザー/アプリ モデルのデフォルトを含む)
