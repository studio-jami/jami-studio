...
| `PLAN_RECAP_APP_URL`     | `https://plan.jami.studio`                            | 秘密。プラン アプリを別のオリジンで自己ホストする場合のみ。                                                                                                                                                                                                                                 |
...
これをインストールするには、[Jami Studio/agent-native](https://github.com/BuilderIO/agent-native/blob/main/.github/workflows/pr-visual-recap-fork.yml) からリポジトリの `.github/workflows/` ディレクトリに、既存の `pr-visual-recap.yml` と一緒にファイルをコピーします。同じシークレット (`PLAN_RECAP_TOKEN`、`ANTHROPIC_API_KEY`) が適用されます。
...
`Jami Studio/agent-native` モノリポジトリでは、ワークフローは PR ヘッド ソースではなく信頼できるベースブランチ ソースから要約 CLI を実行します。これにより、`packages/core/**` を含む通常のパッケージ変更は、PR で修正された CLI コードを実行せずに要約の対象となり続けます。
...
**再利用可能** オプションは、代わりに約 20 行の薄い呼び出し元を書き込みます。 `uses:` 経由で `Jami Studio/agent-native/.github/workflows/pr-visual-recap-reusable.yml` に委任します。すべての呼び出し元は、ワークフローの実行時に最新のロジックを自動的に取得し、ローカル更新は必要ありません。
...
# シン呼び出し元 — 完全なワークフロー ロジックは Jami Studio/agent-native にあります。
