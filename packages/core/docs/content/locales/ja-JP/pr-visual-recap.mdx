...
プランを対話的にインストールすると、Jami Studio CLI が追加するかどうかを尋ねます
...
`recap doctor` は不正な形式または未対応の JSON を拒否し、構成されたすべてのラベルに一致するオンライン ランナーが GitHub にない場合に報告します。ランナーの登録、インストール、アップグレード、容量、保守は引き続き利用者の責任です。Jami Studio はラベルを選択しますが、ランナーを提供または運用しません。
...
要約コンテンツを Jami Studio 計画データベースに送信せずに要約するには、
