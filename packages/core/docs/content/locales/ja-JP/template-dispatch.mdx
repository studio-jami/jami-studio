...
ライブデモ: [dispatch.jami.studio](https://dispatch.jami.studio)。
...
- **1 つの MCP コネクタを公開します。** 追加
  Claude、ChatGPT の `https://dispatch.jami.studio/_agent-native/mcp`
...
1. **設定**を開きます。
2. **LLM** で、Jami Studio に接続するか、`ANTHROPIC_API_KEY` などの独自のプロバイダー キーを追加します。
