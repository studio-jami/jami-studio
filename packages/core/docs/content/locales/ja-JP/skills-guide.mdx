...
Jami Studio CLI をローカル エージェントのデフォルトのドキュメント パスとして保持します。
は、MCP コネクタも登録します。 `Jami Studio/agent-native` は本物の GitHub
...
場所。新しくコピーされた Jami Studio skills には `agent-native-skill.json` が含まれます
将来のステータス出力でソースとハッシュを識別できるようにするためのマーカー。

生成された Jami Studio アプリとワークスペースには、フレームワークが提供するものも含まれます
