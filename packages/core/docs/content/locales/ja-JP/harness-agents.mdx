---
title: "ハーネス エージェント"
description: "独自のループ、サンドボックス、ネイティブ ツール、および再開可能な SQL バックアップ セッションを使用して、Claude コード、Codex、Pi、およびその他の完全なコーディング ハーネスを Jami Studio 内の組み込みエージェントとして実行します。"
...
> Codex, Pi) をエージェントとして Jami Studio に入力します。アプリを構築しますか?
...
承認モデルとサンドボックスの動作。 Jami Studio は
...
独自のループをエンドツーエンドで実行するため、Jami Studio はそれを単一ではなくセッションとして駆動します
モデル呼び出し。

<Diagram id="doc-block-1hwvd6l" title={"ハーネスはそのループを所有します。 Jami Studio がセッションを駆動します"} summary={"AgentHarness 基板 creates/resumes はネイティブ セッションであり、そのイベントを通常のトランスクリプトにストリーミングし、ターン間で SQL のresumeState を保持します。"}>
...
| Claude-Code/Codex スタイル **コーディング ワークスペース UI** をレンダリングします              | [Jami Studio Code UI](/docs/code-agents-ui) |
...
隣接するサーフェス: Jami Studio のチャットの背後に別の場所に構築したエージェントを配置します
...
Jami Studio は [ACP](https://agentclientprotocol.com) (エージェント クライアント) として機能できます
...
- **Jami Studio コード / デスクトップ** はユーザーのマシン上で `codex exec` を実行します。もし
...
- **`ai-sdk-harness:codex`** は `@ai-sdk/harness-codex` をロードし、Codex を駆動します
  ...
  Jami Studio は、ローカル Codex CLI 認証ファイルをサンドボックスにコピーしてから、
...
ハーネスは、存続期間の長いネイティブ セッション状態を所有します。 Jami Studio は SQL に永続化します
そのため、スレッドはターン、プロセス、デプロイを経ても存続できます。 `resumeState`
は **不透明** — Jami Studio は保管して返却しますが、検査や
それを解釈します。
...
Jami Studio actions から `createSession.tools` までの意図的なセット**
...
ハーネス セッションは `AgentHarnessEvent` 値をストリーミングします。この値は Jami Studio
...
- 毎ターン、完全な Jami Studio チャット履歴をハーネスに再生しないでください。再開
...
- [Jami Studio Code UI](/docs/code-agents-ui) — 再利用可能なコーディング ワークスペース サーフェス。
