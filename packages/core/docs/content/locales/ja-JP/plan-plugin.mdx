...
> 注: ベア `npx skills@latest add Jami Studio/agent-native --skill visual-plan` (Vercel/open Skills CLI) は **手順のみ** をインストールします。MCP コネクタは登録されません。コネクタも配線したい場合は、上記の Agent-Native CLI を使用してください。
