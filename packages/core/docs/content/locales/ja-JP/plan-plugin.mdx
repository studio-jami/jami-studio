...
description: "Jami Studio Plan skills（/visual-plan、/visual-recap、/visualize-repo）とホスト型 Plan MCP コネクタを、Claude Code または Codex プラグインとして、またはユニバーサル CLI でインストールします。更新の仕組みと、何かを送信する必要があるかどうか。"
---

# プラグインとマーケットプレイスの計画

Jami Studio **Plan** アプリは、1 つのインストール可能なバンドルとして提供されます。1 回のインストールで Plan の slash-command skills が追加され、ホスト型 Plan MCP コネクタも接続されるため、agent は plans を生成でき、skills はそれらを Plan アプリに直接公開できます。
...
- **プラン MCP コネクタ** — `https://plan.jami.studio` (MCP エンドポイント `https://plan.jami.studio/_agent-native/mcp`、サーバー名 `plan`) でホストされているアプリに対して登録されています。
...
これにより、計画の内容が Jami Studio 計画データベースに保存されなくなります。ホスト型共有、
...
Jami Studio デスクトップには、ホスト型プラン用の別のローカル ファイル同期パスがあります:
...
> プラグイン (`agent-native-visual-plans`) にはアプリ ID `visual-plans` が含まれているため、Claude コード プラグイン名と Codex プラグイン名は両方とも `agent-native-visual-plans` です。プラン アプリの表示名は「Jami Studio プラン」です。
...
Claude Code、Codex、Cursor、Cline、Goose、ChatGPT custom MCP apps、Claude Cowork、その他 MCP-compatible な任意の host で動作します。Jami Studio CLI は Plan skills をインストールし、ホスト型 Plan MCP コネクタを登録し、**選択したローカル client の認証を同じ手順で実行する**ため、最初の tool call が OAuth の壁に当たりません。
...
- `--no-connect` — 認証せずにコネクタを登録します。後で `npx @agent-native/core@latest connect https://plan.jami.studio --client all` を実行するか、より狭い `--client` を選択してください。
...
> 注: ベア `npx skills@latest add Jami Studio/agent-native --skill visual-plan` (Vercel/open Skills CLI) は **手順のみ** をインストールします。MCP コネクタは登録されません。コネクタも配線したい場合は、上記の Jami Studio CLI を使用してください。

### Claude コード (プラグイン) {#claude-code}

パブリック `Jami Studio/agent-native` リポジトリ自体は Claude コード プラグイン マーケットプレイスであるため、ビルド手順なしで直接追加します。 Claude コード内:
...
**これを配布またはインストールするには提出やレビューは必要ありません。** `Jami Studio/agent-native` は自己ホスト型のパブリック git マーケットプレイスであるため、ユーザーは **Claude コードと Codex の両方**に上記のコマンドを使用して直接追加します。申請や承認は必要ありません。ユニバーサル CLI ルートにはマーケットプレイスはまったく必要ありません。
