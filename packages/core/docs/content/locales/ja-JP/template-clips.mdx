...
Jami Studio Clips Chrome 拡張機能のリストは次のとおりです
...
ライブデモ: [clips.jami.studio](https://clips.jami.studio)。
...
1. **ビデオ ストレージ (必須)。** オンボーディング ウィザードを通じてストレージ バックエンドに接続します。最も簡単なパスは Jami Studio (ベータ期間中は無料、ワンクリック) です。セルフホスト型ストレージの場合は、`S3_ENDPOINT`、`S3_BUCKET`、`S3_ACCESS_KEY_ID`、`S3_SECRET_ACCESS_KEY`、およびオプションで `S3_REGION` および `S3_PUBLIC_BASE_URL` を設定します。 Cloudflare R2 と DigitalOcean Spaces は、`R2_*` プレフィックスが付いた同じ環境変数を使用します。
...
[clips.jami.studio](https://clips.jami.studio) でホストされている Clips アプリ
...
2. **運用状態を構成します。** 永続的な `DATABASE_URL` (通常) を設定します。
   ...
   ビデオストレージプロバイダー。 Jami Studio Connect は最も簡単なストレージ パスです。
...
6. **オプションの統合を接続します。** Google Calendar は [会議] タブを強化します。
   `GEMINI_API_KEY` または Jami Studio Connect はトランスクリプトのクリーンアップとタイトルを強化します。
