...
설치하려면 [studio-jami/jami-studio](https://github.com/studio-jami/jami-studio/blob/main/.github/workflows/pr-visual-recap-fork.yml)의 파일을 기존 `pr-visual-recap.yml`와 함께 저장소의 `.github/workflows/` 디렉터리에 복사하세요. 동일한 비밀(`PLAN_RECAP_TOKEN`, `ANTHROPIC_API_KEY`)이 적용됩니다.
...
`studio-jami/jami-studio` 모노레포에서 워크플로는 PR 헤드 소스 대신 신뢰할 수 있는 기본 분기 소스에서 요약 CLI를 실행합니다. 이는 PR 수정 CLI 코드를 실행하지 않고도 요약에 적합하도록 `packages/core/**`를 포함한 일반적인 패키지 변경 사항을 유지합니다.
...
**재사용 가능** 옵션은 대신 ~20줄의 얇은 호출자를 작성합니다. `uses:`를 통해 `studio-jami/jami-studio/.github/workflows/pr-visual-recap-reusable.yml`에 위임합니다. 모든 호출자는 워크플로가 실행될 때 로컬 업데이트 없이 자동으로 최신 로직을 선택합니다.
...
# 씬 호출자 - 전체 작업 흐름 논리가 studio-jami/jami-studio에 있습니다.
