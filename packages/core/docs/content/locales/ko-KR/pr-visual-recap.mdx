...
대화식으로 Plans를 설치하면 Jami Studio CLI가 추가 여부를 묻습니다.
...
`recap doctor`는 잘못된 형식이나 지원되지 않는 JSON을 거부하며 구성된 모든 레이블과 일치하는 온라인 러너를 GitHub에서 찾을 수 없을 때 이를 보고합니다. 러너 등록, 설치, 업그레이드, 용량 및 유지 관리는 계속 사용자의 책임입니다. Jami Studio는 레이블을 선택할 뿐 러너를 제공하거나 운영하지 않습니다.
...
요약 내용을 Jami Studio 계획 데이터베이스로 보내지 않고 요약하고
