...
`sendToAgentChat()`는 내부적으로 `agentNative.submitChat`로 표시되는 제출된 채팅 경로에 대해 지원되는 브라우저 래퍼입니다. 래퍼가 로컬 사이드바, Jami Studio/프레임 라우팅, MCP 앱 호스트 라우팅, 탭 ID 및 코드 요청 라우팅을 처리하므로 앱 UI는 `agentNative.submitChat`를 직접 게시하는 대신 래퍼를 호출해야 합니다.
