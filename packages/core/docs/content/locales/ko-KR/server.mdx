...
앱이 API 전용이라고 해서 Express 서버가 필요한 것은 아닙니다. Jami Studio 앱에는 이미 Nitro/H3 서버가 포함되어 있으며, `defineAction()`이 기본 API 표면입니다. 작업은 `actions/`에 정의하세요. 프레임워크는 각 action을 schema validation 및 request context와 함께 `/_agent-native/actions/<name>`에 마운트합니다.
...
Jami Studio, Anthropic, AI SDK 제공자, 사용자/앱 모델 기본값을 포함한 채팅
