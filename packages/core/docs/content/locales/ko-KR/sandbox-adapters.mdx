...
Jami Studio에는 좁은 뒤에 있는 문제를 고려하는 두 개의 어댑터 이음새가 있습니다.
교체 가능한 인터페이스:
...
| Claude-Code/Codex 스타일 **코딩 작업 공간 UI** 렌더링                    | [Jami Studio Code UI](/docs/code-agents-ui) |
...
ID별로 내장 어댑터를 선택합니다. `local`(기본값)과 `background`가 모두 연결되어 있습니다. 알 수 없는 값은 실행에 실패하지 않고 `local`로 대체됩니다.

```bash
AGENT_NATIVE_SANDBOX=local        # 기본값 - 명시적
AGENT_NATIVE_SANDBOX=background   # 모든 run-code 호출을 내구성 큐에 추가
```

`background`는 원시 코드를 `sandbox_executions`에 저장하고 `status: "queued"`와 `executionId`를 즉시 반환합니다. 서명된 self-dispatch는 새로운 serverless 호출을 시작하고, 장기 실행 Node는 프로세스 내에서 분리된 작업을 시작합니다. 상태 polling과 주기적 sweep은 누락된 dispatch 또는 만료된 lease를 다시 구동합니다. 영구 저장된 결과는 나중에 실행을 polling하여 가져옵니다.

`BackgroundQueueAdapter`는 내구성 큐 마커이지 executor가 아닙니다. 큐에 있는 작업은 사용 가능한 경우 등록된 non-queued execution adapter를 사용하고, 그렇지 않으면 로컬 child process로 대체됩니다. `background`만 선택해도 원격 또는 컨테이너 격리가 제공되지 않으며 edge 런타임이 `node:child_process`와 호환되는 것도 아닙니다.
...
내장 `background` 어댑터는 이미 내구성 큐와 새로운 실행 예산을 제공하지만 실제 실행은 계속 non-queued adapter 또는 로컬 child-process fallback을 사용합니다. 별도로 등록한 원격 또는 격리 어댑터(Docker나 Vercel-Sandbox 스타일 실행기)는 다른 실행 경계를 제공하고 호출별 백그라운드 작업도 실행할 수 있습니다.

이러한 어댑터는 다음을 수행합니다.

1. out-of-process 런타임에 대해 `SandboxAdapter.run`를 구현합니다.
2. 루프백 브리지를 터널링하거나 브리지 호출을 상위 프로세스로 프록시합니다.
3. 제공된 내구성 큐 수명 주기와 독립적으로 원격 또는 컨테이너 격리를 제공합니다.

`registerSandboxAdapter()`를 통해 프로그래밍 방식으로 등록하세요. 에이전트 루프와 `run-code.ts`를 변경할 필요가 없습니다.
