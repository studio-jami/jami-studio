...
### Jami Studio: 프로덕션에서의 시각적 편집 {#visual-editing-in-production}

[Jami Studio](https://www.jami.studio)는 에이전트가 프로덕션에서 앱의 UI를 수정할 수 있는 기능을 유지하는 관리형 클라우드 환경을 제공하여 이 문제를 해결합니다. 저장소를 Jami Studio에 연결하고 UI 변경 사항을 직접 묻는 메시지를 표시하세요. 재배포가 필요하지 않습니다.

**작동 방식:**

1. 에이전트 기본 저장소를 Jami Studio에 연결
2. Jami Studio는 에이전트, 시각적 편집 및 실시간 협업 기능을 갖춘 클라우드 프레임을 제공합니다
...
- [**Frames**](/docs/frames) — 프로덕션 UI 편집을 위한 내장 에이전트 패널과 Jami Studio 클라우드 프레임의 차이입니다
