...
Jami Studio 앱의 자체 채팅 내에서 표, 차트, 입력된 결과 및 승인 어포던스와 같은 자사 위젯에 대해 [native chat renderers](/docs/native-chat-ui)를 선호합니다. Claude, ChatGPT, Copilot, Cursor 및 기타 호환 호스트의 외부/교차 호스트 인라인 UI용 MCP 앱을 범용 딥 링크 폴백으로 `link` 작업과 함께 사용하세요.
