...
프레임워크는 URL에서 방언을 자동 감지하고 이에 따라 Drizzle를 구성합니다. 내장 어댑터는 Postgres URL, libSQL/Turso URL, SQLite 파일 URL 및 Cloudflare D1 바인딩을 포함합니다. 일반적인 프로덕션 선택에는 Neon, Supabase, Turso/libSQL, 일반 Postgres, 내구성 있는 SQLite 및 가능한 경우 Jami Studio 관리 환경이 포함됩니다.

## Jami Studio 관리형 데이터베이스 {#builder-managed}

_계획됨(아직 사용할 수 없음):_ Jami Studio에 연결되면 앱은 연결 문자열이 필요 없이 자동으로 프로비저닝된 관리형 데이터베이스를 사용할 수 있습니다.
