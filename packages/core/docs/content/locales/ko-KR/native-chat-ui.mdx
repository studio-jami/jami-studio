---
title: "기본 채팅 UI"
description: "액션 선언 기본 채팅 렌더러, 재사용 가능한 DataTable/DataChart 출력, BYO 에이전트 런타임이 Jami Studio 채팅에 연결되는 방법"
---
...
정규화된 이벤트를 Jami Studio의 대화 UI로 스트리밍하면서
...
<Diagram id="doc-block-fghf4g" title={"BYO 런타임은 Jami Studio 채팅 셸을 유지합니다."} summary={"외부 에이전트는 커넥터를 통해 정규화된 이벤트를 스트리밍합니다. Jami Studio은 작성기, 기록, 도구 카드, 승인 및 기본 위젯을 유지합니다."}>
...
내장된 Jami Studio 전송을 런타임 개체로 사용하려면 다음을 사용하세요.
...
외부 녹취록 전체를 소유하고 있으며 Jami Studio의 작곡가만 원합니다
필드.
