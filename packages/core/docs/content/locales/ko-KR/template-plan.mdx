---
title: "시각적 계획"
description: "Jami Studio Plans는 코딩 에이전트의 계획을 구조화되고 검토 가능한 문서(다이어그램, 와이어프레임, 주석이 달린 코드, 주석 및 공유 링크)로 바꿔줍니다. CLI에서 한 번 설치하십시오. 게스트로 편집과 공유하고 저장하거나 공유하려는 경우에만 로그인한 리뷰어."
---
...
Jami Studio 계획은 코딩 에이전트를 위한 시각적 계획 모드입니다. 평범해졌습니다
...
`https://plan.jami.studio/_agent-native/mcp` — 클라이언트별 설정은 [MCP Clients](/docs/mcp-clients)를 참조하세요.
...
커넥터이므로 단일 명령 설정을 원할 경우 Jami Studio CLI 경로를 사용하세요.

> **일회 설치 플러그인을 선호하십니까?** Claude 코드 및 Codex가 추가할 수 있습니다
> `Jami Studio/agent-native`는 다음을 번들로 제공하는 플러그인 마켓플레이스입니다.
...
그리고 VS에 대한 기존 Jami Studio MCP 연결 흐름을 실행하는 명령이 포함되어 있습니다.
...
호스팅된 Plans 커넥터는 `https://plan.jami.studio/_agent-native/mcp`에 있습니다.
...
로컬 파일 모드는 계획 또는 요약 내용이 Jami Studio로 이동하는 것을 방지합니다.
...
- [**Skills**](/docs/skills-guide) — Jami Studio가 skills를 설치하는 방법
