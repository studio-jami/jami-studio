...
`https://plan.jami.studio/mcp` — 클라이언트별 설정은 [MCP Clients](/docs/mcp-clients)를 참조하세요.
...
- 앱 자체의 기본 채팅 모델은 Sonnet 5이며, 언제든지 **설정**에서 변경할 수 있습니다. Haiku는 더 저렴한 Claude 옵션이고, Luna는 GPT-5.6 계열에서 더 저렴한 옵션입니다.
...
호스팅된 Plans 커넥터는 `https://plan.jami.studio/mcp`에 있습니다.
