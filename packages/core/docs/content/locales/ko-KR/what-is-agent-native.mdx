...
이것이 바로 Jami Studio 경험을 위해 설계된 것입니다. 대부분의 제품이 거기에 도달하지 못하는 이유는 다음과 같습니다.
...
1. [jami.studio/templates](/templates)에서 템플릿을 선택하세요
2. 호스팅 앱으로 즉시 사용하세요(예: mail.jami.studio)
3. 맞춤 설정을 원할 때 포크하세요 — "Stripe 계정 연결", "코호트 차트 추가"
4. 에이전트는 귀하의 요구에 맞게 코드를 수정합니다.
5. 포크를 자신의 도메인에 배포하거나 jami.studio을 유지하세요
