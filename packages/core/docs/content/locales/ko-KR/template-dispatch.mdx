...
라이브 데모: [dispatch.jami.studio](https://dispatch.jami.studio).
...
- **MCP 커넥터 1개를 노출합니다.** 추가
  Claude, ChatGPT의 `https://dispatch.jami.studio/_agent-native/mcp`,
...
1. **설정**을 엽니다.
2. **LLM**에서 Jami Studio를 연결하거나 `ANTHROPIC_API_KEY`와 같은 자체 공급자 키를 추가합니다.
