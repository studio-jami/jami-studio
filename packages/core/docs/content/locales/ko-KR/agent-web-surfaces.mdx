...
공개 에이전트 웹을 사용하면 에이전트가 공개 Jami Studio 경로를 쉽게 크롤링하고, 읽고, 인용하고, 호출할 수 있습니다. 목표는 모든 앱 엔드포인트를 공개하는 것이 아닙니다. 목표는 이미 공개된 페이지에 대해 깨끗한 공개 표면을 게시하는 동시에 개인 데이터 및 도구 액세스를 명시적인 제어로 유지하는 것입니다.
