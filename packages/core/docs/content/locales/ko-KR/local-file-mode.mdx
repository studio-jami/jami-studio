...
Codex, Claude 코드, Jami Studio 사이드바 에이전트 또는 일반 사용자로도 편집 가능
편집자.
...
또는 Jami Studio 데스크탑 브리지가 이를 노출합니다. 다리가 달리고 있지 않으면 스킬
...
일반 호스팅 브라우저 폴더 액세스. `content.jami.studio`를 열면
...
신뢰할 수 있는 로컬 브리지에서 콘텐츠를 로컬로 전송하거나 Jami Studio 데스크탑을 사용하여
