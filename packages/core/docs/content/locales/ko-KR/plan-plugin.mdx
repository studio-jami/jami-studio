...
> 참고: 베어 `npx skills@latest add studio-jami/jami-studio --skill visual-plan`(Vercel/open Skills CLI)는 **지침만** 설치하며 MCP 커넥터를 등록하지 않습니다. 커넥터도 배선하려면 위의 Jami Studio CLI를 사용하세요.

### Claude 코드(플러그인) {#claude-code}

공개 `studio-jami/jami-studio` 저장소는 그 자체로 Claude 코드 플러그인 마켓플레이스이므로 빌드 단계 없이 직접 추가할 수 있습니다. Claude 내부 코드:
...
**이것을 배포하거나 설치하는 데 제출이나 검토가 필요하지 않습니다.** `studio-jami/jami-studio`는 자체 호스팅 공개 Git 마켓플레이스이므로 사용자는 **Claude 코드 및 Codex**에서 위 명령을 사용하여 직접 추가할 수 있으며 신청이나 승인이 필요하지 않습니다. 범용 CLI 경로에는 마켓플레이스가 전혀 필요하지 않습니다.
