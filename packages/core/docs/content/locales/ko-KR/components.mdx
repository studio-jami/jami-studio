...
Jami Studio는 전체 사이드바를 제공하지만 사이드바는 계약이 아닙니다.
...
| `createAgentChatAdapter()`           | `@agent-native/core/client` 또는 `/client/chat` | 저수준 보조 UI 어댑터로 내장된 Jami Studio SSE 전송이 필요합니다.                 |
...
Jami Studio가 작곡가, 대본, 도구 카드를 보관하는 동안 대화
