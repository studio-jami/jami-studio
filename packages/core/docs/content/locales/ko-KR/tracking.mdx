...
이는 Jami Studio의 자체 제품 분석을 지원하는 프레임워크의 내부 브라우저 원격 측정(`trackEvent()` / 자동 페이지뷰 - 아래 [Browser defaults](#browser-defaults) 참조)과 다릅니다. 구성된 제공업체에 도달해야 하는 앱 자체 분석 이벤트에 `track()`를 사용하세요.

## 세션 리플레이 {#session-replay}

Jami Studio 앱은 두 번째 분석 SDK를 추가하지 않고도 퍼스트파티 브라우저 세션 리플레이를 사용하도록 설정할 수 있습니다. 브라우저 루트에서 `configureTracking()`을 한 번 호출하고 Analytics 공개 키와 수집기 엔드포인트를 전달하세요.
...
Jami Studio 템플릿 루트는 이미 `configureTracking()`을 호출합니다. 호스팅된 템플릿 배포는 Vite/Netlify 환경 변수로 리플레이를 켤 수 있으며, 라이브러리 소비자는 위의 명시적인 `configureTracking({ key, endpoint, sessionReplay })` 형태를 사용하는 것이 좋습니다.
...
브라우저 도우미는 또한 현재 Jami Studio 인증 세션에 대해 최선을 다하는 논블로킹 읽기를 수행합니다. 리플레이는 기본적으로 로그인한 사용자에게만 적용됩니다. `sessionReplay`가 활성화되어 있어도 세션이 사용자 이메일 주소로 확인되지 않으면 녹화가 시작되지 않습니다. 로그인한 사용자의 리플레이에는 이메일 기반 `userId`/`userEmail`과 `orgId`가 포함됩니다.
...
프레임워크는 Connect Jami Studio CTA에서 `builder connect clicked`도 추적하고, 서버측 Jami Studio 연결 경로는 시작/성공/실패한 수명 주기 이벤트를 추적합니다. `configureTracking()`는 프레임워크에 의해 자동으로 호출됩니다. 자신의 템플릿 코드에서 호출할 필요가 없습니다.
