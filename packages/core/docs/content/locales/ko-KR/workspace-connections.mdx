...
Dispatch는 앱이 직접 호출할 수 있는 것과 동일한 공유 저장소 함수를 사용하는 제어 평면 actions를 제공합니다. `upsert-workspace-connection` action은 공급자 카탈로그를 기준으로 입력을 검증한 다음 이 페이지에 나온 동일한 `upsertWorkspaceConnection()`에 위임합니다.
...
- [**Dispatch**](/docs/dispatch) — 일반적으로 작업 공간 연결 설정과 Vault를 관리하는 앱
- [**보안**](/docs/security) — 연결이 가리키는 자격 증명 Vault, 시크릿 범위, 접근 모델
- [**온보딩**](/docs/onboarding#checking-workspace-connections-in-onboarding) — 템플릿 설정 체크리스트에서 연결 준비 상태를 확인하는 방법
- [**MCP 클라이언트**](/docs/mcp-clients) — 작업 공간 전체에서 MCP 서버 자격 증명을 공유하는 관련 재사용 가능 구성 패턴
- [**Brain**](/docs/template-brain) — 이 모델을 기반으로 구축된 공급자 Sources 페이지의 참조 구현
