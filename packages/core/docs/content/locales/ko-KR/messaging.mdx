...
- **Clips 앱이 펼쳐집니다** — 설치 가능한 Jami Studio Slack용 클립은 `SLACK_CLIENT_ID`, `SLACK_CLIENT_SECRET`, `SLACK_SIGNING_SECRET` 및 `/api/slack/oauth/callback`를 사용합니다. 연결된 각 Slack 작업 공간은 `app_secrets`에서 자체 암호화된 봇 토큰을 얻습니다. `SLACK_BOT_TOKEN`는 레거시 단일 작업공간 대체일 뿐입니다.
...
**Basic Information → Display Information**에서 Jami Studio 아이콘을 업로드하세요. 앱의 공개
...
개인 Slack DM은 확인된 Jami Studio 조직 구성원이나 이미 연결된 Slack 신원에서만
