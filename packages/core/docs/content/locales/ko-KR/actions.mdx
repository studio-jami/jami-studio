...
하나의 정의, 7명의 소비자. 이것은 [ladder](/docs/what-is-jami#the-ladder)의 3번 렁입니다.
...
<div class="diagram-fanout">
  ...
  <div class="diagram-grid">
    ...
    <div class="diagram-node">
      A2A 도구<br /><small class="diagram-muted">다른 Jami Studio 앱</small>
    </div>
    <div class="diagram-node">
...
<Endpoint id="doc-block-268kyf" title={"자동 마운트된 action 엔드포인트"} method="GET" path="/_agent-native/actions/get-lead" summary={"모든 action은 여기에 자동으로 마운트됩니다. 파일 이름이 action 이름입니다."} auth={"세션 쿠키; 프론트엔드 호출에는 `X-Jami Studio-Frontend: 1`이 포함됩니다"} params={[
  {
    "name": "leadId",
    "in": "query",
    "type": "string",
...
| `"frontend"` | `useActionMutation` / `useActionQuery` / `callAction`를 통한 브라우저 호출(`X-Jami Studio-Frontend: 1` 헤더 태그가 지정됨)      |
