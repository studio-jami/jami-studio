---
title: "하네스 에이전트"
description: "자체 루프, 샌드박스, 기본 도구 및 재개 가능한 SQL 지원 세션을 사용하여 Claude 코드, Codex, Pi 및 기타 전체 코딩 하네스를 Jami Studio 내부에 내장된 에이전트로 실행하세요."
...
자체 루프가 엔드투엔드이므로 Jami Studio는 이를 단일이 아닌 세션으로 구동합니다
모델 통화.

<Diagram id="doc-block-15vqyqp" title={"하네스는 루프를 소유합니다. Jami Studio이 세션을 구동합니다."} summary={"AgentHarness 기판은 기본 세션을 creates/resumes하고 해당 이벤트를 일반 기록으로 스트리밍하며 턴 사이에 SQL에서 이력서 상태를 유지합니다."}>
...
인접한 표면: Jami Studio 채팅 뒤의 다른 곳에 구축한 에이전트 배치
...
Jami Studio는 [ACP](https://agentclientprotocol.com)(에이전트 클라이언트) 역할을 할 수 있습니다.
...
- **Jami Studio 코드 / 데스크톱**은 사용자 컴퓨터에서 `codex exec`를 실행합니다. 만일
...
- **`ai-sdk-harness:codex`**는 Codex를 구동하는 `@ai-sdk/harness-codex`를 로드합니다.
  ...
  Jami Studio는 로컬 Codex CLI 인증 파일을 샌드박스에 복사합니다.
...
하네스는 수명이 긴 기본 세션 상태를 소유합니다. Jami Studio는 SQL에서 이를 유지합니다
따라서 스레드는 차례, 프로세스 및 배포 전반에 걸쳐 생존할 수 있습니다. `resumeState`
**불투명** — Jami Studio는 이를 저장하고 돌려주지만 결코 검사하거나
해석합니다.
...
Jami Studio actions부터 `createSession.tools`까지의 의도적인 세트**
...
하네스 세션은 `AgentHarnessEvent` 값을 스트리밍하며, 이는 Jami Studio
...
- Claude 코드, Codex, 커서, Mastra 또는 Pi를 `AgentEngine`로 추가하지 마세요. 그들은
  ...
  세션 수명 주기 의미가 손실됩니다.
- 매 턴마다 전체 Jami Studio 채팅 기록을 하네스에 재생하지 마세요. 이력서
...
- [Jami Studio Code UI](/docs/code-agents-ui) — 재사용 가능한 코딩 작업 공간
