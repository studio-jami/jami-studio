...
- **이미지 및 비디오 생성.** 활성화된 경우 Jami Studio 관리 이미지 생성을 사용할 수 있으며 Gemini는 비디오 생성과 수동 이미지 대체 기능을 제공합니다.
...
라이브 데모: [assets.jami.studio](https://assets.jami.studio).
...
1. **한 번 설치합니다.** 기술 지침이 추가되고 호스팅된 MCP 커넥터가 함께 등록됩니다.
   ...
   ````배쉬
   npx skills@latest Jami Studio/agent-native --skill 자산 추가
   ```

   Vercel/open Skills CLI는 지침 파일만 설치합니다. 그렇지 않습니다
   MCP 커넥터 설정을 실행하세요. 원하는 경우 위의 Jami Studio CLI 경로를 사용하세요
   단일 명령 설정
