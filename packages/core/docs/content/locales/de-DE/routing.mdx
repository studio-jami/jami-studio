...
Jami Studio Apps verwenden **React Router v8** mit dateibasiertem Routing über `flatRoutes()` von `@react-router/fs-routes`. Jede Datei in `app/routes/` wird zu einem URL. Vorlagen verwenden die Punktnotationskonvention – Punkte trennen URL-Segmente innerhalb eines einzelnen Dateinamens.
