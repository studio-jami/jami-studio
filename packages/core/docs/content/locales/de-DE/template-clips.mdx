...
Die Liste der Jami Studio Clips Chrome-Erweiterungen lautet
...
1. **Videospeicher (erforderlich).** Verbinden Sie ein Speicher-Backend über den Onboarding-Assistenten. Der einfachste Weg ist Jami Studio (kostenlos während der Beta, ein Klick). Für selbst gehosteten Speicher stellen Sie `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` und optional `S3_REGION` und `S3_PUBLIC_BASE_URL` ein. Cloudflare R2 und DigitalOcean Spaces verwenden dieselben Umgebungsvariablen mit dem Präfix `R2_*`.
...
Die gehostete Clips-App unter [clips.jami.studio](https://clips.jami.studio)
...
2. **Konfigurieren Sie den Produktionsstatus.** Legen Sie einen dauerhaften `DATABASE_URL` fest, den Normalen
   ...
   Videospeicheranbieter. Jami Studio Connect ist der einfachste Speicherpfad; für
...
6. **Verbinden Sie optionale Integrationen.** Google Calendar unterstützt die Registerkarte „Besprechungen“,
   `GEMINI_API_KEY` oder Jami Studio Connect ermöglicht die Bereinigung von Transkripten und Titeln
