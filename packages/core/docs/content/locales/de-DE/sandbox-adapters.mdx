...
Jami Studio verfügt über zwei Verbindungsnähte, die ein Problem hinter einer schmalen Naht verdrängen,
...
| Rendern Sie einen **Codierungsarbeitsbereich UI** im Claude-Code/Codex-Stil        | [Jami Studio Code UI](/docs/code-agents-ui) |
...
<FileTree
  ...
  entries={[
    ...
    {
      path: "background.ts",
      note: "Marker für die dauerhafte Warteschlange und Lebenszyklus der Hintergrundausführung",
    },
    {
...
Wählt einen integrierten Adapter anhand der ID aus. Sowohl `local` (Standard) als auch `background` sind eingebunden; unbekannte Werte fallen auf `local` zurück, statt die Ausführung fehlschlagen zu lassen.

```bash
AGENT_NATIVE_SANDBOX=local        # die Standardeinstellung – explizit
AGENT_NATIVE_SANDBOX=background   # jeden run-code-Aufruf dauerhaft einreihen
```

`background` speichert den Rohcode in `sandbox_executions` und gibt sofort eine `executionId` mit `status: "queued"` zurück. Ein signierter Self-Dispatch startet einen neuen Serverless-Aufruf; langlebiges Node startet abgekoppelte Arbeit im Prozess; Statusabfragen und der periodische Sweep treiben verlorene Dispatches oder abgelaufene Leases erneut an. Das gespeicherte Ergebnis wird später durch Abfragen der Ausführung gelesen.

`BackgroundQueueAdapter` ist ein Marker für die dauerhafte Warteschlange, nicht der Executor. Eingereihte Arbeit verwendet einen registrierten, nicht warteschlangenbasierten Ausführungsadapter, falls vorhanden; andernfalls fällt sie auf den lokalen Child-Prozess zurück. `background` allein bietet keine Remote- oder Container-Isolation und macht eine Edge-Laufzeit nicht mit `node:child_process` kompatibel.
...
Der integrierte Adapter `background` bietet bereits dauerhafte Einreihung und ein neues Ausführungsbudget, während die eigentliche Ausführung weiterhin einen nicht warteschlangenbasierten Adapter oder den lokalen Child-Prozess als Fallback verwendet. Ein separat registrierter Remote- oder isolierter Adapter (Docker oder ein Vercel-Sandbox-artiger Runner) kann eine andere Ausführungsgrenze bereitstellen und auch Hintergrundarbeit pro Aufruf ausführen.

Ein solcher Adapter würde:

1. `SandboxAdapter.run` gegen eine Out-of-Process-Laufzeit implementieren.
2. Die Loopback-Brücke tunneln oder Bridge-Aufrufe zum übergeordneten Prozess zurückleiten.
3. Remote- oder Container-Isolation unabhängig vom ausgelieferten dauerhaften Warteschlangenlebenszyklus bereitstellen.

Registrieren Sie ihn programmgesteuert über `registerSandboxAdapter()`. Die Agentenschleife und `run-code.ts` müssen sich nicht ändern.
