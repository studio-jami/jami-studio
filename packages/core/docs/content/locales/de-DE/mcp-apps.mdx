  ...
  annotations={[
    {
      ...
      note: "`mcpApp.resource` kündigt eine Inline-Benutzeroberfläche für Hosts an, die die Apps-Erweiterung MCP unterstützen. Behalten Sie auch den `link`-Jami Studio der Aktion bei – nur CLI-Hosts und ältere Hosts ignorieren die UI-Metadaten und benötigen weiterhin den Deep Link.",
    },
    {
      lines: "7",
...
- [Actions](/docs/actions) – `defineAction`, der `link`-Jami Studio, `publicAgent`.
