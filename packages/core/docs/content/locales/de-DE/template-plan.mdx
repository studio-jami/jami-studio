...
Jami Studio Plans ist ein visueller Planmodus für Codierungsagenten. Es wird zu einem gewöhnlichen
...
Verwenden Sie Jami Studio CLI. Dies ist das empfohlene Setup, da es das
...
`https://plan.jami.studio/_agent-native/mcp` – siehe [MCP Clients](/docs/mcp-clients) für kundenspezifische Einrichtung.
...
-Anschluss, verwenden Sie also den Pfad Jami Studio CLI, wenn Sie die Ein-Befehl-Einrichtung wünschen.

> **Bevorzugen Sie ein One-Install-Plugin?** Claude-Code und Codex können hinzugefügt werden
> `Jami Studio/agent-native` direkt als Plugin-Marktplatz, der die
...
und enthält einen Befehl zum Ausführen des vorhandenen Jami Studio MCP-Verbindungsablaufs für VS
...
Der gehostete Plan-Connector befindet sich unter `https://plan.jami.studio/_agent-native/mcp`.
...
Der Modus „Lokale Dateien“ verhindert, dass Plan- oder Zusammenfassungsinhalte an den Jami Studio gesendet werden.
...
Jami Studio Desktop bietet gehosteten Plänen außerdem eine native lokale Ordnerbrücke. Dies
...
Öffnen Sie einen Plan in Jami Studio Desktop und verwenden Sie im Planmenü **Lokale Dateien** actions
dann:
...
`npx -y @agent-native/core@latest reconnect https://plan.jami.studio --client codex` für Codex, oder führen Sie`/mcp` → **Authentifizieren** auf einem OAuth-fähigen Host erneut aus. Starten Sie ein
...
- [**Skills**](/docs/skills-guide) – wie Jami Studio skills installiert
