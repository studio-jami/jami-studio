...
- `view` – die aktuelle Seite/der aktuelle Abschnitt, z. B. „Posteingang“, „Formular-Jami Studio“ oder „Dashboard“
...
`sendToAgentChat()` ist der unterstützte Browser-Wrapper für den Pfad des übermittelten Chats, der intern manchmal als `agentNative.submitChat` angezeigt wird. Die App UI sollte den Wrapper aufrufen, anstatt `agentNative.submitChat` direkt zu veröffentlichen, da der Wrapper lokale Seitenleisten, Jami Studio/Frame-Routing, MCP-App-Host-Routing, Tab-IDs und Code-Anfrage-Routing verarbeitet.
