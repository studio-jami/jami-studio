...
description: "Führen Sie Claude Code, Codex, Pi und andere vollständige Codierungssysteme als eingebettete Agenten in Jami Studio aus, mit eigener Schleife, Sandbox, nativen Tools und wiederaufnehmbaren, von SQL unterstützten Sitzungen."
...
> Codex, Pi) als Agent in Jami Studio ein. Eine App erstellen? Beginnen Sie mit
...
Genehmigungsmodell und Sandbox-Verhalten. Jami Studio führt diese über den
...
Eigene End-to-End-Schleife, daher fährt Jami Studio sie als Sitzung und nicht als einzelne
Modellaufruf.

<Diagram id="doc-block-q2hd5a" title={"Ein Geschirr besitzt seine Schlaufe; Jami Studio steuert die Sitzung"} summary={"Das AgentHarness-Substrat creates/resumes der nativen Sitzung, streamt seine Ereignisse in das normale Transkript und behält zwischen den Runden den resumeState in SQL bei."}>
...
Angrenzende Oberflächen: Platzieren Sie einen Agenten, den Sie an anderer Stelle erstellt haben, hinter dem Chat von Jami Studio
...
Jami Studio kann als [ACP](https://agentclientprotocol.com) (Agent Client) fungieren
...
- **Jami Studio Code / Desktop** führt `codex exec` auf dem Computer des Benutzers aus. Wenn
...
- **`ai-sdk-harness:codex`** lädt `@ai-sdk/harness-codex`, das Codex antreibt
  ...
  Jami Studio kopiert die lokale Codex CLI-Authentifizierungsdatei vor dem
...
Ein Harness besitzt einen langlebigen nativen Sitzungsstatus. Jami Studio behält es in SQL
damit ein Thread über Runden, Prozesse und Bereitstellungen hinweg überleben kann. Der `resumeState`
ist **undurchsichtig** – Jami Studio speichert es und gibt es zurück, inspiziert es jedoch nie oder
interpretiert es.
...
absichtliche Festlegung** von Jami Studio actions bis `createSession.tools`, wenn Sie
...
Eine Harness-Sitzung streamt `AgentHarnessEvent`-Werte, die Jami Studio
...
- Spielen Sie nicht jede Runde den gesamten Jami Studio-Chatverlauf in einem Geschirr ab. Fortsetzen
...
- [Jami Studio Code UI](/docs/code-agents-ui) – die wiederverwendbare Codierungsarbeitsbereichsoberfläche.
