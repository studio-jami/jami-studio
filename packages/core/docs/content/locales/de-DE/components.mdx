...
Jami Studio liefert eine vollständige Seitenleiste, aber die Seitenleiste ist nicht der Vertrag. Die
...
| `createAgentChatAdapter()`           | `@agent-native/core/client` oder `/client/chat` | Sie benötigen den integrierten Jami Studio SSE-Transport als Low-Level-Assistent-UI-Adapter.                                          |
...
Konversation, während Jami Studio den Komponisten, das Transkript, die Werkzeugkarten usw. behält.
