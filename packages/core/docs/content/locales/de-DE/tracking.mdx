...
Dies unterscheidet sich von der internen Browser-Telemetrie des Frameworks (`trackEvent()` / automatische Seitenaufrufe – siehe [Browser defaults](#browser-defaults) unten), die die eigene Produktanalyse von Jami Studio unterstützt. Verwenden Sie `track()` für die eigenen Analyseereignisse Ihrer App, die Ihre konfigurierten Anbieter erreichen sollen.

## Session Replay {#session-replay}

Jami Studio-Apps können sich für erstklassiges browserseitiges Session Replay entscheiden, ohne ein zweites Analytics-SDK hinzuzufügen. Rufen Sie `configureTracking()` einmal im Browser-Root auf und übergeben Sie den öffentlichen Analytics-Schlüssel sowie den Collector-Endpunkt:
...
Jami Studio-Template-Roots rufen `configureTracking()` bereits auf. Gehostete Template-Deployments können Replay über Vite-/Netlify-Umgebungsvariablen aktivieren, während Bibliotheks-Konsumenten die explizite Form `configureTracking({ key, endpoint, sessionReplay })` von oben bevorzugen sollten:
...
Das Framework verfolgt auch `builder connect clicked` von Connect Jami Studio-CTAs und die serverseitigen Jami Studio-Connect-Routen verfolgen gestartete/erfolgreiche/fehlgeschlagene Lebenszyklusereignisse. `configureTracking()` wird vom Framework automatisch aufgerufen; Sie müssen es nicht in Ihrem eigenen Vorlagencode aufrufen.
