...
Eine Definition, sieben Verbraucher. Dies ist Sprosse 3 des [ladder](/docs/what-is-jami#the-ladder).
...
<Endpoint id="doc-block-23t11k" title="Automatisch gemounteter Action-Endpunkt" method="GET" path="/_agent-native/actions/get-lead" summary="Jede Action wird hier automatisch gemountet - der Dateiname ist der Action-Name." auth={"Session-Cookie; Frontend-Aufrufe tragen `X-Jami Studio-Frontend: 1`"} params={[
  {
    "name": "leadId",
    "in": "query",
    "type": "string",
...
| `"frontend"` | Ein Browseraufruf über `useActionMutation` / `useActionQuery` / `callAction` (getaggt mit dem `X-Jami Studio-Frontend: 1`-Header).                        |
...
Für UI-fähige MCP-Hosts kann eine Aktion eine optionale MCP-Apps-Ressource über das `mcpApp`-Feld (plus ein passendes `link`) deklarieren, sodass fähige Hosts das Ergebnis inline rendern. Wenn `link` und `mcpApp` auf die gleiche Route zeigen sollen, erstellt `embedRoute()` beide aus einem reinen Pfad-Jami Studio:
