...
- **Clips-App entfaltet sich** – installierbare Jami Studio-Clips für Slack verwenden `SLACK_CLIENT_ID`, `SLACK_CLIENT_SECRET`, `SLACK_SIGNING_SECRET` und `/api/slack/oauth/callback`. Jeder verbundene Slack-Arbeitsbereich erhält seinen eigenen verschlüsselten Bot-Token in `app_secrets`; `SLACK_BOT_TOKEN` ist nur ein Legacy-Fallback für einen einzelnen Arbeitsbereich.
...
Information → Display Information** das Jami Studio-Symbol hoch, damit die
...
Jami Studio-Organisation oder von bereits verknüpften Slack-Identitäten
