...
> Hinweis: Der nackte `npx skills@latest add studio-jami/jami-studio --skill visual-plan` (Vercel/open Skills CLI) installiert **nur Anweisungen** – der MCP-Anschluss wird nicht registriert. Verwenden Sie den oben genannten Jami Studio CLI, wenn auch der Stecker verkabelt sein soll.

### Claude-Code (Plugin) {#claude-code}

Das öffentliche `studio-jami/jami-studio`-Repo ist selbst ein Claude-Code-Plugin-Marktplatz, Sie fügen es also direkt hinzu – kein Build-Schritt. Im Claude-Code:
