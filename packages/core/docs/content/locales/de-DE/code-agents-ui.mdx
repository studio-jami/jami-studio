...
description: "Erstellen und passen Sie Jami Studio-Codeoberflächen mit dem gemeinsam genutzten UI-Paket, der Desktop-Host-Bridge und dem CLI-Run-Store an."
---
...
| ---------------------------------------------------------------------------------- | -------------------------------------- |
| Rendern Sie einen **Codierungsarbeitsbereich UI** im Claude-Code/Codex-Stil        | **Jami Studio Code UI** (diese Seite) |
...
Jami Studio Code ist die Jami Studio-Codierungsoberfläche: ein lokaler Arbeitsbereich im Claude Code/Codex-Stil für Codierungssitzungen, Slash-Befehle, Migrationen, Audits, Transkripte, Laufkontrollen und Nachverfolgungen. Ein einfacher `npx @agent-native/core@latest`-Befehl öffnet diesen Arbeitsbereich; `npx @agent-native/core@latest code` ist der explizite Unterbefehl für dasselbe Erlebnis.
...
Hosts können Ausführungsquellen in derselben Liste mischen. Lokale Jami Studio-Codesitzungen
...
Jami Studio-Code kann ein lokales Codex CLI-Login anstelle eines OpenAI API-Schlüssels verwenden.
...
Der automatische Modus ist die Standardeinstellung für lokale Jami Studio Code-Sitzungen. Verwenden Sie den Planmodus für
...
Lokale Jami Studio-Codeausführungen werden gespeichert unter:
...
Jami Studio-Code verwendet dasselbe `AgentComposerFrame` + `PromptComposer` /
...
Der Sidebar-Entwicklungsagent und der Jami Studio-Code verwenden beide dasselbe Minimum
