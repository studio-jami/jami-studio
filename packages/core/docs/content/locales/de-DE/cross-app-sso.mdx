...
Jede gehostete App bei `*.jami.studio` führt ihre eigene Bereitstellung mit ihrem **eigenen separaten Benutzerspeicher** aus. `mail.jami.studio` und `calendar.jami.studio` nutzen keine gemeinsame Datenbank, Sitzungstabelle oder Cookie-Domäne. „Einmal anmelden, jede App verwenden“ kann also kein gemeinsam genutztes Cookie sein – es muss eine **Identitätsföderation** sein, wobei [Dispatch](/docs/dispatch) als Identitätsautorität für den Arbeitsbereich fungiert.
...
1. **App → Dispatch (Autorisierung).** Die app-seitige Route `GET /_agent-native/identity/login` (auf die die Schaltfläche „Mit Jami Studio anmelden“ verweist) erstellt die Weiterleitung und sendet den Benutzer an die Identitätsautorität:
...
<Endpoint id="cross-app-sso-identity-authorize-de-de" title="Identity Authorize Endpoint" method="GET" path="/_agent-native/identity/authorize" summary={"Dispatch (Identitätsbehörde) authentifiziert den Menschen und leitet ihn mit einem signierten Identitätstoken zurück"} auth="Sitzung weiterleiten (interaktive Anmeldung, falls keine)" params={[
  ...
  },
  {
    ...
    "type": "string",
    "required": true,
    "description": "App-Rückruf URL. Validiert anhand einer strengen Zulassungsliste (standardmäßig `*.jami.studio` oder localhost)."
  },
  {
    "name": "state",
    "in": "query",
...
- **Set** → Die App zeigt die Option **„Mit Jami Studio anmelden“** an, die den obigen Ablauf ausführt. Daneben funktioniert weiterhin die direkte lokale Anmeldung (E-Mail/Passwort, Google).
...
- **Strikte `redirect_uri`-Zulassungsliste.** Dispatch leitet standardmäßig immer nur zu `*.jami.studio` oder localhost weiter. Beliebige, schemarelative (`//host`) und Cross-Origin-Redirect-Ziele werden abgelehnt, sodass die Autorität nicht in ein Open-Redirect- oder Token-Exfiltration-Orakel umgewandelt werden kann.
...
Jede Dispatch-Bereitstellung kann als Identitäts-Hub dienen – Sie sind nicht auf `dispatch.jami.studio` beschränkt. Legen Sie `AGENT_NATIVE_IDENTITY_HUB_URL` in jeder Client-App so fest, dass es auf Ihre Dispatch-Instanz verweist:
...
- **Standard:** Nur `*.jami.studio` und localhost (die `DEFAULT_ALLOWED_HOST_SUFFIXES`-Konstante).
...
Ohne `IDENTITY_SSO_ALLOWED_HOST_SUFFIXES` kann ein selbstgehosteter Dispatch nur Token für Apps auf `*.jami.studio` ausgeben. Legen Sie die Umgebungsvariable in Ihrer Dispatch-Bereitstellung fest, um andere Domänen freizuschalten.
...
- Der Anmeldebildschirm zeigt jetzt **„Mit Jami Studio anmelden“** an. Klicken Sie darauf.
