...
| `clips`        | Suche, Import, Besprechungen            | Gehirn, Clips                                    |
...
Dispatch stellt Steuerungsebenen-Aktionen bereit, die dieselben gemeinsam genutzten Store-Funktionen verwenden, die eine App direkt aufrufen könnte – die Aktion `upsert-workspace-connection` validiert Eingaben anhand des Anbieterkatalogs und delegiert dann an dieselbe `upsertWorkspaceConnection()`-Funktion, die auf dieser Seite gezeigt wird:
...
- [**Dispatch**](/docs/dispatch) – die App, die normalerweise die Einrichtung von Workspace-Verbindungen und den Tresor verwaltet
- [**Sicherheit**](/docs/security) – der Anmeldeinformationstresor, die Geheimnisbereiche und das Zugriffsmodell, auf das Verbindungen verweisen
- [**Onboarding**](/docs/onboarding#checking-workspace-connections-in-onboarding) – Verbindungsbereitschaft in der Setup-Checkliste einer Vorlage prüfen
- [**MCP-Clients**](/docs/mcp-clients) – MCP-Server-Anmeldeinformationen als verwandtes wiederverwendbares Konfigurationsmuster im Workspace teilen
- [**Brain**](/docs/template-brain) – die Referenzimplementierung einer Anbieter-Quellseite auf Grundlage dieses Modells
