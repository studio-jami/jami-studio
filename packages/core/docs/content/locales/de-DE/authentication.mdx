...
Jami Studio Apps verwenden [Better Auth](https://better-auth.com) für die Authentifizierung mit einem Account-First-Design. Benutzer erstellen beim ersten Besuch ein Konto und erhalten vom ersten Tag an eine echte Identität.
...
| Vom Erstanbieter gehostet (`*.jami.studio`)  | Isolierter Namespace pro App (jede hat ihre eigene Authentifizierungsdatenbank); `COOKIE_DOMAIN=.jami.studio` wird standardmäßig ignoriert |
