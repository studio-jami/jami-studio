...
description: "Run Claude Code, Codex, Pi, and other full coding harnesses as embedded agents inside Jami Studio, with their own loop, sandbox, native tools, and resumable SQL-backed sessions."
...
Codex, Pi) into Jami Studio as the agent. Building an app? Start with
...
approval model, and sandbox behavior. Jami Studio runs these through the
...
own loop end to end, so Jami Studio drives it as a session, not as a single
model call.

<Diagram id="doc-block-ulh6ba" title={"A harness owns its loop; Jami Studio drives the session"} summary={"The AgentHarness substrate creates/resumes the native session, streams its events into the normal transcript, and persists resumeState in SQL between turns."}>
...
| Render a Claude-Code/Codex-style **coding workspace UI**                   | [Jami Studio Code UI](/docs/code-agents-ui) |
...
Adjacent surfaces: put an agent you built elsewhere behind Jami Studio's chat
...
Jami Studio can act as an [ACP](https://agentclientprotocol.com) (Agent Client
...
- **Jami Studio Code / Desktop** runs `codex exec` on the user's machine. If
...
- **`ai-sdk-harness:codex`** loads `@ai-sdk/harness-codex`, which drives Codex
  ...
  Jami Studio copies the local Codex CLI auth file into the sandbox before the
...
A harness owns long-lived native session state. Jami Studio persists it in SQL
so a thread can survive across turns, processes, and deploys. The `resumeState`
is **opaque** — Jami Studio stores it and hands it back, but never inspects or
interprets it.
...
intentional set** of Jami Studio actions through `createSession.tools` when you
...
A harness session streams `AgentHarnessEvent` values, which Jami Studio
...
- Don't replay full Jami Studio chat history into a harness each turn. Resume
...
- [Jami Studio Code UI](/docs/code-agents-ui) — the reusable coding workspace surface.
