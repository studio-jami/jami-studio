---
title: "Jami Studio Code UI"
description: "Build and customize Jami Studio Code surfaces with the shared UI package, Desktop host bridge, and CLI run store."
---
...
| You want to…                                                               | Use                                    |
| -------------------------------------------------------------------------- | -------------------------------------- |
| Render a Claude-Code/Codex-style **coding workspace UI**                   | **Jami Studio Code UI** (this page)   |
...
Jami Studio Code is the Jami Studio coding surface: a local Claude Code/Codex-style workspace for coding sessions, slash commands, migrations, audits, transcripts, run controls, and follow-ups. A bare `npx @agent-native/core@latest` command opens this workspace; `npx @agent-native/core@latest code` is the explicit subcommand for the same experience.
...
The current split is intentionally converging: the standard agent sidebar and Agent Teams run on the core `run-manager` lifecycle, while Jami Studio Code uses local long-running sessions backed by the file-based Code run store and the shared background-run controller vocabulary.
...
Hosts can mix run sources in the same list. Local Jami Studio Code sessions
...
Jami Studio Code can use a local Codex CLI login instead of an OpenAI API key.
...
Auto mode is the default for local Jami Studio Code sessions. Use Plan mode for
...
Local Jami Studio Code runs are stored at:
...
Jami Studio Code uses the same `AgentComposerFrame` + `PromptComposer` /
...
The sidebar development agent and Jami Studio Code both use the same minimal
...
Jami Studio Code treats migration as a capability, not a separate app category. `/migrate` can be a built-in goal, a project command, or a custom instruction pack on top of the same host contract.

### Migrating to Jami Studio with `/migrate` {#migrate}

`/migrate` is the built-in goal for moving an existing app, URL, or described product into Jami Studio. It is a slash goal in the Code workspace — not a separate template to scaffold and not a one-off product — so it shares the same session store, transcript, run controls, and Desktop hub as every other Code session, and you can resume, attach to, inspect, and stop it the same way.
...
When the host implements `listCodePacks`, the shared UI shows project commands and skills in the rail. Command rows insert `/<command>`, and skill rows insert a focused “Use the `<skill>` skill…” prompt so the rail stays actionable. The built-in slash goals `/migrate` and `/audit` stay reserved for the global Jami Studio Code controls, as do run-control names such as `status` and `resume` — those are subcommands invoked without a slash (`npx @agent-native/core@latest code status`, `npx @agent-native/core@latest code resume`), not slash goals.
