...
You don't need to build Jami from scratch. The agent chat, workspace tab, CLI terminal, voice input, and all the related infrastructure ship as a handful of React components you drop into any app.
...
A button that hands work off to the agent (instead of running an inline `llm()` call — the anti-pattern from the [ladder](/docs/what-is-jami#the-ladder)):
...
- **Bring your own agent runtime.** If an agent you built elsewhere should
  power the conversation while Jami Studio keeps the composer, transcript, tool
...
When the UI needs to run the same operation an agent tool would run — rung 3 of the [ladder](/docs/what-is-jami#rung-three) — use `useActionMutation`:
