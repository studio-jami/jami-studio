...
Jami Studio ships a full sidebar, but the sidebar is not the contract. The
...
| `createAgentChatAdapter()`           | `@agent-native/core/client` or `/client/chat` | You need the built-in Jami Studio SSE transport as a low-level assistant-ui adapter.            |
...
conversation while Jami Studio keeps the composer, transcript, tool cards, and
