---
title: "Key Concepts"
description: "How Jami Studio apps work: actions first, SQL database, app-agent loop, optional UI, polling sync, external-agent entry points, context awareness, and portability."
---

# Key Concepts

How Jami Studio apps work under the hood — the principles and the architecture. This page is the contract; for the vision and the case for building this way, see [What Is Jami?](/docs/what-is-jami).

## The architecture {#the-architecture}

Every Jami Studio app is three things working together:
...
Every Jami Studio app has the same agent building blocks, regardless of whether
...
Every user-facing feature should update all applicable areas. Skipping an applicable area breaks the machine-interface contract; forcing a screen onto an automation that no human needs to browse is also a smell.
...
Jami Studio apps include an embedded agent panel that provides the AI agent alongside the app UI. This is what makes the architecture work: the agent needs a computer (database, browser, code execution), and the app needs the agent for AI work.
...
- [**What Is Jami?**](/docs/what-is-jami) — the vision and philosophy behind these rules
