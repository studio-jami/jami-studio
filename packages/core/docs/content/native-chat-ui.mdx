...
description: "Action-declared native chat renderers, reusable DataTable/DataChart outputs, and how BYO agent runtimes should connect to Jami Studio chat."
---
...
stream normalized events into Jami Studio's conversation UI while keeping the
...
<Diagram id="doc-block-784bg9" title="BYO runtime keeps the Jami Studio chat shell" summary={"Your external agent streams normalized events through a connector; Jami Studio keeps the composer, transcript, tool cards, approvals, and native widgets."}>
...
When you want the built-in Jami Studio transport as a runtime object, use:
...
owns the entire external transcript and only wants Jami Studio's composer
field.
