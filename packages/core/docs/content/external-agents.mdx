...
description: "Connect Claude, ChatGPT, Codex, Cursor, Claude Cowork, or any MCP-compatible host to a hosted Jami Studio app — then round-trip artifacts back into the running UI with MCP Apps and deep links."
...
**This page: connect an external agent or MCP host to your app.** Use it when Claude, ChatGPT, Codex, Cursor, Claude Cowork, or another MCP-compatible host should drive a hosted Jami Studio app and round-trip the result back into the running UI.
...
- **External MCP host:** use this page when Claude, ChatGPT, Codex, Cursor, OpenCode, GitHub Copilot / VS Code, or another MCP-compatible host should call your hosted Jami Studio app.
- **Your own runtime behind Jami Studio chat:** see [Agent Surfaces](/docs/agent-surfaces#byo-agent) and [Native Chat UI](/docs/native-chat-ui#byo-agent-runtimes) when an agent built with another framework should power `<AssistantChat runtime={...}>`.
- **Your app consuming MCP tools:** see [MCP Clients](/docs/mcp-clients) when a Jami Studio app needs to call tools exposed by another MCP server.
- **Another app or agent via A2A:** use [Agent Mentions](/docs/agent-mentions) and [A2A](/docs/a2a-protocol) when Jami Studio apps should discover and delegate to each other.
- **Local custom sub-agents:** use [Workspace](/docs/workspace) when you want custom agent profiles inside the Jami Studio workspace itself.

## Easy setup {#easy-setup}

Add one remote MCP connector to the host where you want to use Jami Studio.
...
**Connect**, sign in with your Jami Studio account, approve the MCP scopes,
...
scan/discover tools, sign in with Jami Studio, approve the scopes, and enable
...
```text
Use Jami Studio Analytics to generate a weekly conversion-rate bar chart and show it inline.
```

```text
Use Jami Studio Mail to draft a short follow-up email to me, but do not send it.
```
...
First-party app skills install the instructions and the hosted MCP connector together with the Jami Studio CLI:
...
need a connector such as `npx @agent-native/core@latest connect https://assets.jami.studio`.
...
Hosted Jami Studio apps also support the standard remote MCP OAuth flow. For clients that implement MCP OAuth, add the remote HTTP server URL with no static headers:
...
matched to an existing Jami Studio organization member and persisted as a
...
- **Per token** — mint with `--full-catalog`, which embeds a `catalog_scope: "full"` claim in the JWT. Subsequent requests bypass the compact filter for that token:
  ...
  ```bash
  npx @agent-native/core@latest connect https://plan.jami.studio --client codex --full-catalog
  ```
...
→ Open draft in Mail → https://mail.jami.studio/_agent-native/open?app=mail&view=inbox&compose=…
```
...
Jami Studio apps also speak the official MCP Apps extension. When any action
...
// .mcp.json
{
  "mcpServers": {
    "analytics": {
      "type": "http",
      "url": "https://analytics.jami.studio/_agent-native/mcp",
...
- Connect your own agent to Dispatch with `npx @agent-native/core@latest connect https://dispatch.jami.studio`; use a direct app URL only when you want one isolated app.
