---
title: "Visual Plans"
description: "Jami Studio Plans turns your coding agent's plan into a structured, reviewable document — diagrams, wireframes, annotated code, comments, and share links. Install once from the CLI; reviewers you share with edit as a guest and sign in only to save or share."
---
...
Jami Studio Plans is visual plan mode for coding agents. It turns an ordinary
...
Use the Jami Studio CLI. This is the recommended setup because it installs the
...
`https://plan.jami.studio/_agent-native/mcp` — see [MCP Clients](/docs/mcp-clients) for client-specific setup.
...
connector, so use the Jami Studio CLI path when you want the one-command setup.

<Callout tone="info">

**Prefer a one-install plugin?** Claude Code and Codex can add
`studio-jami/jami-studio` directly as a plugin marketplace, which bundles the
...
and includes a command to run the existing Jami Studio MCP connect flow for VS
...
- **Viewing** a public or shared plan works for anyone with the link — no account
  required.
- **Commenting** on a shared plan requires a Jami Studio account.
...
The hosted Plans connector lives at `https://plan.jami.studio/_agent-native/mcp`.
...
Local-files mode prevents plan or recap content from going to the Jami Studio
...
Jami Studio Desktop also gives hosted Plans a native local-folder bridge. This
...
Open a plan in Jami Studio Desktop, use the plan menu's **Local files** actions,
then:
...
- [**Skills**](/docs/skills-guide) — how Jami Studio installs skills
