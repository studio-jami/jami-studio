...
This is deliberately narrower than a general-purpose HTTP client or a chat integration. Jami Studio's automation connector runtime supports exactly two directions:
...
n8n is runtime-backed through configured Webhook workflows. Jami Studio does **not** host or provision n8n.

Configure an n8n Webhook node with an explicit URL and one of n8n's supported authentication modes (Header, Basic, or JWT), then register that static URL/origin and the matching Jami Studio secret reference. n8n supports immediate responses, final-node responses, and responses via a Respond to Webhook node; model the selected behavior as asynchronous or synchronous in the workflow definition.

For callbacks from n8n into Jami Studio, configure a Jami Studio callback URL and a shared-secret or HMAC adapter. Include a stable event ID header whenever n8n can retry delivery.
...
Zapier MCP is a separately configured remote MCP connection with a Zapier account and its own authorization flow. It is not interchangeable with a Jami Studio workflow callback credential.
