...
With one config file, every Jami Studio app in your workspace gains access to tools provided by MCP servers on your machine: `claude-in-chrome` for browser automation, `@modelcontextprotocol/server-filesystem` for reading files, `@playwright/mcp` for browser testing, and anything else that speaks MCP.
...
Jami Studio includes local-development toggles for common stdio MCP servers.
...
This means users who've installed the claude-in-chrome extension get browser control across every Jami Studio app they open with no config changes.
...
If your workspace runs multiple Jami Studio apps (e.g. dispatch + mail + clips), share MCP servers across all of them without per-app copy-pasting of URLs and bearer tokens. See [Multi-App Workspace](/docs/multi-app-workspace) for the full deployment picture.
...
Jami Studio will always boot; broken MCP configuration just means fewer tools.
