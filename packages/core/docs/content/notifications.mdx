...
const rows = await listNotifications("steve@jami.studio", {
  unreadOnly: true,
  limit: 50,
});
const unread = await countUnread("steve@jami.studio");
await markNotificationRead(rows[0].id, "steve@jami.studio");
await markAllNotificationsRead("steve@jami.studio");
await deleteNotification(rows[0].id, "steve@jami.studio");
```
