...
1. In **Settings → Messaging**, select **Agent manifest** to download the
   ...
   Display Information**, upload the Jami Studio icon so the Agents picker
...
- **Personal and channel identity.** A personal Slack DM is accepted only for a
  verified Jami Studio organization member or a Slack identity already linked
...
- **Identity** — personal DMs require a verified Jami Studio organization member or an already linked Slack identity. Configured channels use a separate service principal, policy, and budget.
...
If you're running multiple Jami Studio apps (mail, calendar, analytics, etc.), the recommended pattern is to set up messaging on **[Dispatch](/docs/dispatch)** (see also the [template reference](/docs/template-dispatch)) and let it route work to your domain apps over [A2A](/docs/a2a-protocol).
...
Each external conversation maps to a persistent thread in the app's database:
