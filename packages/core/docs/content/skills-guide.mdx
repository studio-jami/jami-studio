---
title: "Skills Guide"
description: "How skills work in Jami Studio: framework skills, domain skills, and creating custom skills."
---
...
App-backed skills package a Jami Studio app as a skill marketplace artifact. The bundle can include agent instructions, exported skills, MCP connector metadata, hosted/local launch instructions, and UI surfaces such as MCP Apps.
...
The Jami Studio runtime reads skills from `.agents/skills/`. Claude Code reads skills from `.claude/skills/` independently. To make a skill available to your coding agent but hidden from the runtime agent:
...
Keep the Jami Studio CLI as the default docs path for local agents because it
also registers the MCP connector. `studio-jami/jami-studio` is a real GitHub
...
place. Newly copied Jami Studio skills include an `agent-native-skill.json`
marker so future status output can identify the source and hash.

Generated Jami Studio apps and workspaces also include framework-provided
