import enUS from "./en-US";

const zhCN = {
  language: {
    label: "语言",
    system: "系统",
    systemDescription: "使用浏览器语言",
    suggestionTitle: "要用{{language}}阅读此页面吗？",
    suggestionDescription: "你的浏览器语言是{{language}}。",
    suggestionSwitch: "切换到{{language}}",
    suggestionKeepEnglish: "继续使用英文",
  },
  header: {
    docs: "文档",
    templates: "应用",
    skills: "技能",
    searchAria: "搜索文档",
    searchPlaceholder: "搜索文档...",
    askAssistant: "询问 AI 助手",
    toggleNavigation: "切换导航菜单",
  },
  footer: {
    download: "下载",
    brand: "品牌",
    privacy: "隐私",
    terms: "条款",
  },
  feedback: {
    label: "反馈",
    placeholder: "告诉我们怎样能让这些文档更好。",
  },
  demoVideo: {
    visualPlanningAria: "Jami Studio 可视化规划演示",
  },
  docBlocks: {
    blockLabel: "{{alias}} 区块",
    unknownBlockType: "未知区块类型",
  },
  theme: {
    light: "浅色",
    dark: "深色",
    toggle: "切换主题",
    label: "主题：{{theme}}",
  },
  docs: {
    navigateAria: "浏览文档",
    onThisPage: "本页内容",
    copyMarkdown: "将文档复制为 Markdown",
    copiedMarkdown: "已复制 Markdown",
    copyMarkdownError: "无法复制 Markdown",
    previous: "上一页",
    next: "下一页",
  },
  search: {
    dialogLabel: "搜索文档",
    placeholder: "搜索文档...",
    empty: "输入内容以搜索所有文档",
    noResults: "未找到“{{query}}”的结果",
    browseAllDocs: "浏览全部文档",
    navigate: "导航",
    open: "打开",
    close: "关闭",
  },
  agent: {
    emptyState: "向我询问任何有关 Jami Studio 的问题",
    suggestionGettingStarted: "如何开始使用 Jami Studio？",
    suggestionActions: "actions 如何工作？",
    suggestionPolling: "解释轮询同步模型",
    suggestionDeploy: "如何部署到生产环境？",
  },
  errors: {
    notFoundTitle: "页面未找到",
    notFoundBody: "你要查找的页面不存在或已移动。",
    goHome: "返回首页",
    readDocs: "阅读文档",
    genericTitle: "出现问题",
    genericBody: "发生了意外错误。",
    sendFeedback: "发送反馈",
    feedbackPlaceholder: "描述此 Docs 错误出现前发生了什么。",
    openGitHubIssue: "打开 GitHub issue",
  },
  home: {
    hero: {
      badge: "开源框架",
      titleLine1: "面向",
      titleAccent: "agentic apps 的框架",
      body: "Jami Studio 是一个开源 framework，用于以 agent 为核心快速构建健壮的应用程序。",
      primaryCta: "试用一个 app",
      secondaryCta: "阅读文档",
    },
    code: {
      quickStartComment: "用一条命令创建 app",
      skillInstallComment: "把 Jami Studio 规划加入你已经使用的 coding agent",
      frameworkComment: "一个 action 驱动 agent、UI、HTTP、MCP、A2A 和 CLI。",
      frameworkDescription: "从本地 app-agent loop 打招呼。",
    },
    actionSurface: {
      eyebrow: "真正面向代理，而不是外挂式 AI",
      title: "一个 action 覆盖整套应用能力面",
      body: "只定义一次操作。Jami Studio 会把它变成 UI action、agent 工具、HTTP 端点、MCP/A2A 能力面、CLI 命令、权限检查和审计记录。",
      buildAction: "构建一个 action",
      benefits: {
        oneActionDefinition: {
          title: "一个 action 定义",
          body: "UI、agent、HTTP、MCP、A2A 和 CLI 都调用同一个操作。",
        },
        scopedByDefault: {
          title: "默认带访问范围",
          body: "Auth、共享、治理和审计日志会随工作一起生效。",
        },
        headedOrHeadless: {
          title: "Apps、automations 和 agents",
          body: "同一个 operation 可从 chat、UI、定时任务、队列、外部 agents 或 scripts 运行。",
        },
        contextRichInput: {
          title: "富上下文输入",
          body: "聊天、语音、skills、指令和 UI 状态都会留在循环中。",
        },
        openAgentProtocols: {
          title: "开放的 agent 协议",
          body: "A2A、MCP、MCP apps 和外部 agents 都是框架级原语。",
        },
        observableByDesign: {
          title: "天生可观测",
          body: "Trace、eval、反馈和审计历史让 agent 工作可检查。",
        },
      },
    },
    modules: {
      pageEyebrow: "模块化框架",
      title: "面向 agentic 应用的内置模块",
      body: "这些生产级部分经过人工验证，agent 不应该从零临时发明。你可以直接使用、检查源码，或在应用需要不同能力时替换模块。",
      pageBody:
        "Jami Studio 提供经过人工验证的生产级组件，避免 agent 从零 improvisation。你可以直接使用、检查源码，或在产品需要不同能力时替换模块。",
      viewAll: "查看所有模块",
      browseApps: "浏览由模块构建的应用",
      items: {
        autoStateSyncing: {
          title: "自动状态同步",
          body: "Agent 的更改会更新 UI，UI 状态也会继续对 agent 可见，不需要另一座桥。",
        },
        actions: {
          title: "操作 actions",
          body: "一次定义工作，然后从 UI、agent、HTTP、MCP、A2A 和 CLI 使用它。",
        },
        sqlStateOrm: {
          title: "SQL 状态与 ORM",
          body: "持久应用数据、application state、迁移，以及不绑定供应商的 schema。",
        },
        dbAdmin: {
          title: "数据库管理",
          body: "Agent 可读的 schema、查询界面、迁移和管理工具，无需自建后台。",
        },
        authGovernance: {
          title: "认证与治理",
          body: "登录、组织、多租户、权限、审批和策略 hook。",
        },
        sharing: {
          title: "共享能力",
          body: "分享链接、限定访问、公开或私有资源、评论和审阅界面。",
        },
        realtimeCollaboration: {
          title: "实时协作",
          body: "多人编辑、在线状态、乐观 UI，以及服务端支持的协调。",
        },
        agentInteroperability: {
          title: "Agent 互操作",
          body: "A2A、MCP、MCP apps、外部 agents、harness agents 和跨应用交接。",
        },
        automationsQueues: {
          title: "自动化与队列",
          body: "事件触发工作、定时任务、后台运行和可靠 mutations。",
        },
        agentUiSurface: {
          title: "Agent UI 能力面",
          body: "聊天、skills、指令、生成式 UI、语音输入和 agent 可见上下文。",
        },
        observability: {
          title: "可观测性",
          body: "Trace、eval、反馈、实验，以及证明 agents 做了什么的证据。",
        },
        workspaces: {
          title: "工作区",
          body: "可组合的 agentic apps 可互相发现，并通过 A2A 协调。",
        },
        sourceOwnership: {
          title: "源码所有权",
          body: "文档和源码位于 agents 可以检查、fork、eject、patch 或替换的位置。",
        },
        auditLogs: {
          title: "审计日志",
          body: "持久记录人和 agent 的更改，并按用户可访问的资源限定范围。",
        },
      },
    },
    framework: {
      title: "面向 Jami Studio apps 的框架",
      body1:
        "Jami Studio 是一个开源框架，用来构建 agentic applications：从 Chat 开始，定义共享 actions，再围绕同一份 state 添加 UI、jobs 和协作。",
      body2: "使用你自己的数据库、托管服务、模型栈和 app 代码。",
      cta: "阅读框架指南",
      primitives: {
        actions: {
          title: "行动",
          description: "只定义一次工作。从 agent、UI、API、MCP 和 A2A 使用它。",
        },
        sharedState: {
          title: "共享状态",
          description:
            "由 SQL 支撑的 app 状态让人、agents 和 sessions 保持同步。",
        },
        agentRuntime: {
          title: "agent runtime",
          description:
            "app-agent loop、tools、skills、memory、jobs 和可观测性一起交付。",
        },
        backendAgnostic: {
          title: "后端无关",
          description:
            "接入任何 Drizzle 支持的 SQL 数据库和 Nitro 兼容的托管环境。",
        },
      },
    },
    templates: {
      title: "试用一个 Jami Studio app",
      eyebrow: "Fork 一个可运行的 app，让 agent 继续改进它。",
      cta: "查看 apps",
    },
    skills: {
      title: "用一个 skill 试试",
      body: "用一条命令，把可视化规划和 PR recaps 加到 Claude Code、Codex、Cursor、Pi、OpenCode 或 VS Code。",
      planBody:
        "在改代码之前，先查看带有图表、线框、文件地图和评论的可审阅 plans。",
      recapBody:
        "用 PR 或 diff 的可视化摘要，让 reviewers 在原始代码行之前先看清整体形状。",
      cta: "浏览 Skills Guide",
    },
    connected: {
      title: "agent 和 UI 完全连接",
      body: "agent 和 UI 是同一系统里的平等成员。每个 action 都双向可用：点击它，或直接提出请求。",
      tabs: {
        agentSees: {
          title: "agent 能看到一切",
          description: "它可以读取和更新应用中的任何 UI、任何数据、任何状态。",
        },
        uiTalks: {
          title: "UI 与 agent 对话",
          description:
            "按钮、表单和 workflows 会把结构化内容推给 agent，让所有引导流程都通过 agent，包括 skills、rules 和 instructions。",
        },
        agentUpdates: {
          title: "agent 更新自己的代码",
          description:
            "它可以修改 app 本身来改变 features 和功能。你的 tools 会越来越好。",
        },
        everything: {
          title: "一切都双向工作",
          description:
            "UI 中可用的每个 action 也都可供 agent 使用。你可以点击来做事，也可以让 agent 去做。",
        },
      },
    },
    comparison: {
      titleLine1: "不必在 app 和 agent 之间二选一。",
      titleAccent: "Agent-native apps 二者兼备。",
      columns: {
        saas: "SaaS 工具",
        agents: "原始 AI Agents",
        internal: "内部工具",
        native: "Jami Studio App",
      },
      rows: {
        ui: "UI",
        ai: "AI",
        customization: "定制",
        ownership: "所有权",
      },
      cells: {
        polishedButRigid: "精致但僵硬",
        none: "无",
        mixedQuality: "质量参差",
        fullUi: "完整 UI，fork 即用",
        boltedOn: "后加上去",
        powerful: "强大",
        shallowlyConnected: "浅层连接",
        agentFirst: "Agent-first，已集成",
        cant: "不能",
        instructionsAndSkills: "instructions 和 skills",
        fullHighMaintenance: "完整，但维护成本高",
        agentModifies: "Agent 修改 app",
        rented: "租来的",
        somewhatYours: "部分属于你",
        youOwnCode: "代码归你所有",
      },
    },
    quickStart: {
      title: "从一条命令开始",
      body: "一条命令会创建一个聊天优先的本地 app，背后有 actions、durable threads 和 SQLite 支撑。只有 automation-first workflow 暂时不需要浏览器 UI 时才使用 `--headless`。",
    },
    finalCta: {
      title: "为 agentic 时代而建的软件",
      body: "从一条命令或 durable action 开始，让它跑过 app-agent loop，然后在不重写操作的情况下成长为 UI、jobs 和协作。开源。可 fork。属于你。",
      primaryCta: "试用一个 app",
      secondaryCta: "阅读文档",
      githubCta: "在 GitHub 查看",
    },
    batteries: {
      titleLine1: "内置全套能力，",
      titleLine2: "久经实战检验",
      body: "不必从空白提示词和临时拼凑的代码开始，Jami Studio 为代理提供构建真实软件所需的可靠部件和最佳实践。",
      browseToolkits: "浏览工具包",
    },
    featureCloud: {
      notifications: "通知",
      recurringJobs: "周期任务",
      actions: "行动",
      agentTeams: "代理团队",
      monorepos: "莫诺回购",
      permissions: "权限",
      rbac: "RBAC",
      organizations: "组织",
      workspaceSecrets: "工作区密钥",
      docsSearch: "文档搜索",
      sourceSearch: "源码搜索",
      contextAwareness: "上下文感知",
      observability: "可观测性",
      realtimeSync: "实时同步",
      sqlState: "SQL 状态",
      multiTenancy: "多租户",
      dataLoaders: "数据加载器",
      liveQueries: "实时查询",
      agentInstructions: "代理须知",
      providerGrants: "提供者补助金",
      comments: "评论",
      reviewLinks: "查看链接",
      privacyControls: "隐私控制",
      skills: "技能",
      security: "安全性",
      auditLogs: "审核日志",
      workspaces: "工作空间",
      voiceInput: "语音输入",
      mcpApps: "MCP 应用程序",
      generativeUi: "生成式 UI",
      toolCalls: "工具调用",
      agentSidebar: "代理侧边栏",
      sharedActions: "共同行动",
      uiSurfaces: "UI 表面",
      i18n: "i18n",
      mcpAuth: "MCP 授权",
      battleTestedComponents: "久经实战的基础模块",
      mcpA2a: "MCP + A2A",
      externalAgents: "外部代理",
      a2aHandoffs: "A2A 切换",
      humanHandoff: "人工切换",
      agentContext: "代理上下文",
      durableResume: "持久的简历",
      extensions: "扩展",
      sharingPrivacy: "分享与隐私",
      realTimeCollaboration: "实时协作",
      sso: "SSO",
      oauth: "OAuth",
      mcpServers: "MCP 服务器",
      scopedAccess: "范围访问",
      dbAdapters: "数据库适配器",
      auth: "授权",
      approvals: "批准",
      automations: "自动化",
      governance: "治理",
      jobs: "工作机会",
      agUi: "AG-UI",
      dispatch: "调度",
      backgroundRuns: "后台运行",
      rateLimits: "速率限制",
      queues: "队列",
      cronSchedules: "Cron 时间表",
      analytics: "分析",
      experiments: "实验",
      feedbackLoops: "反馈循环",
      fileUploads: "文件上传",
      evals: "埃瓦尔斯",
      templates: "应用",
      providerApis: "提供商 APIs",
      agentWebSurfaces: "代理网络表面",
      templateSkills: "应用技巧",
      oneClickForks: "一键叉子",
      localFileMode: "本地文件模式",
      memory: "内存",
      webhooks: "网络钩子",
      http: "HTTP",
      selfEditingCode: "自编辑代码",
      cli: "CLI",
      crossAppSso: "跨应用 SSO",
      schemaMigrations: "架构迁移",
      hostedDeploys: "托管部署",
      environmentSetup: "环境设置",
      oauthCallbacks: "OAuth 回调",
      exports: "出口",
      dashboards: "仪表板",
    },
  },
  common: {
    copied: "已复制",
    copyCommand: "复制命令",
    copyCode: "复制代码",
    tryIt: "试用",
    customizeIt: "自定义",
    editOnline: "在线编辑",
    runLocally: "本地运行",
    viewDocs: "查看文档",
    source: "来源",
    readDocs: "阅读文档",
  },
  templatesPage: {
    title: "您拥有的开源、代理本机应用程序",
    eyebrow: "Fork 一个可运行的 app，让 agent 继续改进它。",
    body: "你可以自定义一切。",
    community: "想要空白应用？从框架指南开始从零构建。",
    createYourOwn: "从零开始",
  },
  buildFromScratch: {
    title: "从零开始构建",
    description: "从框架指南开始，或加入等候名单以在线构建。",
    readDocs: "阅读文档",
    buildOnline: "在线构建",
    popoverTitle: "加入等候名单",
    popoverBody:
      "Jami Studio 可以在云端快速创建并自定义应用——包含 actions、身份验证、SQL 状态和智能体聊天。加入等候名单以获得早期访问权限。",
    joinWaitlist: "加入候补名单",
  },
  templateCard: {
    pasteIntoTerminal: "粘贴到您的终端中。",
    newToCli: "CLI 新手？",
    screenshotAlt: "{{name}} 应用屏幕截图",
  },
  templateDetail: {
    allTemplates: "所有应用",
    notFoundMetaTitle: "未找到应用 — Jami Studio",
    notFoundTitle: "找不到应用",
    notFoundBody: "浏览应用目录以查找可用的应用程序。",
    badge: "Jami Studio {{name}}",
    title: "{{name}} 应用",
  },
  templates: {
    clips: {
      replaces: "替代或增强 Loom、Granola 和 Wisprflow",
      description:
        "带有浏览器调试捕获、日历同步会议记录和 Fn-hold 语音听写的屏幕录制 — 所有这些都经过转录、总结和搜索，代理可以编辑其中任何内容。",
    },
    plan: {
      replaces: "面向 Codex、Claude Code 和编码代理的可视化计划模式",
      description:
        "把可视化计划作为 app-backed skill 安装。你的编码代理可以打开包含图表、线框、原型、注释、评论和可共享评审链接的结构化计划。",
    },
    design: {
      replaces: "替代或增强设计原型工具",
      description:
        "Agent-native HTML 原型工作室。生成交互式 Alpine/Tailwind 设计，比较变体，实时微调控件，并导出结果。",
    },
    content: {
      replaces: "替换或增强 Obsidian 为 MDX、Notion、Google Docs",
      description:
        "编辑本地 Markdown/MDX 文件（如 Obsidian），生成丰富的交互式自定义块，并使用 AI 代理来起草、重写和发布。",
    },
    slides: {
      replaces: "替换或增强 Google Slides、Pitch",
      description:
        "根据提示生成完整的演示文稿。视觉或对话方式编辑。 AI 图像生成、8 种布局和内置演示模式。",
    },
    analytics: {
      replaces: "替换或增强 Amplitude、Mixpanel、Looker",
      description:
        "连接任何数据源、提示任何图表、构建可重用的仪表板。代理编写 SQL、生成可视化并改进应用程序。",
    },
    mail: {
      replaces: "替换或增强 Superhuman、Gmail",
      description:
        "Superhuman 风格的电子邮件客户端，具有键盘快捷键、AI 分类、多帐户支持和电子邮件自动化。拥有您的收件箱工作流程。",
    },
    forms: {
      replaces: "替换或增强 Typeform、Google Forms",
      description:
        "代理原生表单生成器。根据提示生成表单，以可视方式或对话方式编辑字段，并将提交内容发送到 Slack、Discord、Google Sheets 或 webhooks。",
    },
    brain: {
      replaces: "替换或增强团队维基、Glean 式回忆和机构记忆工具",
      description:
        "通过来自批准的 Slack、Clips、Granola、GitHub 和转录源的引用内存进行整页公司聊天，并内置审核门、评估和共享连接准备情况。",
    },
    assets: {
      replaces: "替换或增强 DAMs、品牌资产库和 AI 媒体生成器",
      description:
        "用于上传、品牌库、可搜索参考和品牌图像/视频生成的数字资产管理器，其他应用程序可以通过 A2A 调用或嵌入为选择器。",
    },
    calendar: {
      replaces: "替换或增强 Google Calendar、Calendly",
      description:
        "具有 Google 同步、可用性管理和公共预订页面的完整日历。该代理会查找空闲时段、创建活动并管理您的日程安排。",
    },
    dispatch: {
      replaces: "代理本机应用程序的任务控制",
      description:
        "对堆栈中的每个代理进行集中消息传递和管理。通过 Slack、Telegram 或网络与您的代理交谈；通过 A2A 路由作业、保留内存、批准操作以及跨应用程序委派。",
    },
    chat: {
      replaces: "适合您自己的代理的最小 ChatGPT 风格应用程序",
      description:
        "Chat - 第一个应用程序支架，具有耐用的线程、标准侧边栏、操作、身份验证、实时同步以及用于添加屏幕或插入您自己的代理后端的干净路径。",
    },
  },
  templateLanding: {
    analytics: {
      s001: "Analytics 模板屏幕截图",
      s002: "数据连接器",
      s003: "图表类型",
      s004: "查询浏览器",
      s005: "自然语言",
      s006: "所有模板",
      s007: "Amplitude、Mixpanel 和 Looker 的开源替代品",
      s008: "连接任何数据源，提示任何图表，构建可重复使用的仪表板 - AI 代理写入 SQL。",
      s009: "试用",
      s010: "你能做什么",
      s011: "更换分析堆栈所需的一切。",
      s012: "自然语言查询",
      s013: "用简单的英语询问。代理写入 SQL 并构建图表。",
      s014: "可重复使用的仪表板",
      s015: "具有日期控件、子视图和可调整大小的面板的持久仪表板。",
      s016: "直接 BigQuery 访问历史记录、行计数和可共享 URL。",
      s017: "自我提升",
      s018: "代理修改应用程序本身。需要新的图表类型吗？就问吧。",
      s019: "连接一切",
      s020: "适用于流行服务的多个内置连接器。代理根据需要编写新的。",
      s021: "CRM 和收入",
      s022: "工程",
      s023: "GitHub、Jira、Sentry — PR、票据、冲刺和错误跟踪。",
      s024: "基础设施",
      s025: "通讯",
      s026: "Content 和 SEO",
      s027: "社区",
      s028: "生活数据字典",
      s029: "带有查询模板、连接模式、已知问题和更新频率的指标定义。从 Notion 与社区驱动的验证同步。",
      s030: "每个指标的查询模板和示例输出",
      s031: "通过审阅者批准进行信任评分和验证",
      s032: "AI 支持的指标建议和发现",
      s033: "记录数据滞后、依赖性和有效日期范围",
      s034: "每周活跃用户数",
      s035: "每日",
      s036: "约 2 小时",
      s037: "不包括内部 @company 电子邮件",
      s038: "已验证 ✓",
      s039: "比较如何",
      s040: "仪表板UI",
      s041: "是的，刚性的",
      s042: "No",
      s043: "是的，完全可定制",
      s044: "有限公司",
      s045: "是的，短暂的",
      s046: "是的，持久图表",
      s047: "内置 SDKs",
      s048: "手动上传",
      s049: "多种来源+定制",
      s050: "数据字典",
      s051: "基础",
      s052: "具有上下文的完整指标",
      s053: "定制化",
      s054: "仅配置",
      s055: "仅提示",
      s056: "完整源代码",
      s057: "定价",
      s058: "每个座位、每个事件",
      s059: "订阅",
      s060: "免费和开源",
      s061: "几分钟内即可开始使用",
      s062: "分叉模板，连接数据，开始构建仪表板。",
      s063: "阅读文档",
      s064: "查看所有模板",
    },
    calendar: {
      s001: "Calendar 模板屏幕截图",
      s002: "Calendar 观看次数",
      s003: "代理动作",
      s004: "预订链接类型",
      s005: "所有模板",
      s006: "开源 Google Calendar 和 Calendly 替代方案",
      s007: "多帐户 Google Calendar 同步、可配置的可用性和可定制的 Calendly 式预订链接 - 并由 AI 代理代表您进行安排。",
      s008: "试用",
      s009: "登录仅使用基本 Google 身份，但连接 Calendar 同步要求访问日历。某些工作区管理员可能需要获得托管演示的批准。在本地运行以使用您自己的 Google OAuth 客户端。",
      s010: "你能做什么",
      s011: "替换日历和日程安排堆栈所需的一切。",
      s012: "多个 Calendar 视图",
      s013: "具有拖放事件管理功能的月、周和日视图。",
      s014: "自然语言调度",
      s015: "告诉代理找到一个空档、创建一个事件或重新安排——它会处理剩下的事情。",
      s016: "可定制的预订链接",
      s017: "创建多个具有不同持续时间和可用性的 Calendly 样式预订页面。访客选择一个有效的老虎机。",
      s018: "自我提升",
      s019: "代理修改应用程序本身。需要新的视图或预订流程？就问吧。",
      s020: "通过 OAuth 2.0 连接多个 Google 帐户。从所有日历中提取事件并创建同步回 Google 的事件。",
      s021: "具有自动令牌刷新功能的多账户 OAuth 2.0",
      s022: "基于拉取的同步 — 无需 Webhook",
      s023: "在 Google 上创建、更新和删除事件",
      s024: "创建可自定义的预订链接，任何人都可以与您预订时间。每个预订类型的可配置可用性设置。",
      s025: "日期选择器+时段选择",
      s026: "尊重您的空闲时间和现有活动",
      s027: "访客信息采集+确认",
      s028: "代理驱动的调度",
      s029: "代理运行脚本来同步日历、创建事件、检查可用性和管理预订。全部通过自然语言。",
      s030: "“同步我本月的 Google Calendar”",
      s031: "“下周二找个 30 分钟的时间来参加团队会议”",
      s032: "“每个工作日上午 9 点定期举行站立会议”",
      s033: "“显示我下周有空”",
      s034: "比较如何",
      s035: "日历 UI",
      s036: "饱满、坚硬",
      s037: "最小",
      s038: "完整、完全可定制",
      s039: "AI 调度",
      s040: "自然语言，全程掌控",
      s041: "预订页面",
      s042: "预约时段",
      s043: "是的，品牌有限",
      s044: "完全可定制，拥有自己的域名",
      s045: "定制化",
      s046: "仅设置",
      s047: "仅品牌推广",
      s048: "完整源代码",
      s049: "定价",
      s050: "免费/工作空间",
      s051: "每席每月 $10+",
      s052: "免费和开源",
      s053: "几分钟内即可开始使用",
      s054: "分叉模板，连接 Google Calendar，并开始使用 AI 进行调度。",
      s055: "阅读文档",
      s056: "查看所有模板",
      s057: "托管演示说明",
      s058: "双向同步",
    },
    clips: {
      s001: "Clips 模板屏幕截图",
      s002: "屏幕录制",
      s003: "浏览器调试日志",
      s004: "口授",
      s005: "能看到+听到",
      s006: "所有模板",
      s007: "Loom 的开源替代方案",
      s008: "将 Clips 链接粘贴到代理中，即使其模型无法摄取原始视频或音频，它也可以听到文字记录、读取摘要并查看带时间戳的帧。",
      s009: "试用",
      s010: "你能做什么",
      s011: "记录、转录和调试 — 一个应用程序、一个库，无需订阅堆栈。",
      s012: "一键录屏",
      s013: "自动成绩单",
      s014: "每次屏幕录制、会议和听写都会有完整的记录。特工可以将其用作音频层，观众可以单击任意行跳转到该时刻。",
      s015: "一键通听写",
      s016: "可搜索 Video 库",
      s017: "屏幕录像、会议记录和听写都编入索引。根据所说内容（而不仅仅是标题）在整个图书馆中进行搜索。",
      s018: "对每个成绩单进行全文搜索",
      s019: "文件夹、标签和团队工作区",
      s020: "带时间戳的评论和反应",
      s021: "品牌玩家页面",
      s022: "自托管播放器。定制品牌、号召性用语和分析——这是您的代码。",
      s023: "自定义域和主题",
      s024: "观看次数、观看率、流失率",
      s025: "可嵌入任何网站",
      s026: "由代理支持的视频工作流程",
      s027: "向代理询问有关您的库的任何信息，或将共享的 Clips 链接粘贴到另一个代理中 - 它可以读取文字记录，检查带时间戳的帧，读取捕获的控制台错误和错误背后的失败请求，并从您的录音中起草书面后续内容。",
      s028: "“从今天的站立会议中提取行动项目”",
      s029: "“找到我描述发射计划的听写”",
      s030: "“起草本次会议的后续电子邮件”",
      s031: "“阅读此错误剪辑中的控制台错误并提出修复方案”",
      s032: "比较如何",
      s033: "屏幕录制",
      s034: "是",
      s035: "No",
      s036: "浏览器调试捕获（控制台+网络）",
      s037: "Calendar-同步会议记录",
      s038: "一键通听写",
      s039: "AI 摘要和行动项目",
      s040: "有限公司",
      s041: "完全代理：章节、动作、Q&A",
      s042: "代理编辑成绩单和代码",
      s043: "可粘贴代理媒体链接",
      s044: "仅成绩单",
      s045: "日志+截图包",
      s046: "仅注释",
      s047: "仅文本",
      s048: "文字记录 + 带时间戳的帧",
      s049: "数据所有权",
      s050: "供应商的存储",
      s051: "供应商的云",
      s052: "您拥有代码",
      s053: "定价",
      s054: "每用户每月 $15-30",
      s055: "免费+付费等级",
      s056: "每用户每月 $18-25",
      s057: "每用户每月 $12-15",
      s058: "免费和开源",
      s059: "几分钟内即可开始使用",
      s060: "分叉模板，插入存储，然后开始录制您的团队实际拥有的剪辑。",
      s061: "阅读文档",
      s062: "查看所有模板",
    },
    content: {
      s001: "Content 模板屏幕截图",
      s002: "所有模板",
      s003: "为 MDX 开源 Obsidian",
      s004: "编辑本地 Markdown/MDX 文件（如 Obsidian），生成丰富的交互式自定义块，并使用了解您文档的 AI 代理进行编写。",
      s005: "试用",
      s006: "写",
      s007: "适用于本地 Markdown/MDX 的丰富编辑器，包含格式、标题、代码块和媒体。",
      s008: "使用 AI 进行优化",
      s009: "重写、扩展、总结或改变任何选择的基调。",
      s010: "构建 MDX 块",
      s011: "生成交互式组件并使其在文档中保持可编辑状态。",
      s012: "你需要的一切",
      s013: "完整的内容工作区 - 例如 Obsidian 对应 MDX，在需要时提供 Notion 式协作。",
      s014: "项目和文件",
      s015: "使用嵌套文档组织成项目。侧边栏树导航和搜索。",
      s016: "品牌意识 AI",
      s017: "代理会学习您的声音、风格指南和语气。每个草稿听起来都像你。",
      s018: "双向 Notion 同步。从 Notion 导入页面，在本地编辑，然后推送更改。",
      s019: "本地 Markdown/MDX 文件",
      s020: "像 Obsidian 一样直接编辑存储库文档，预览更改，并在使用托管模式时将它们同步回来。",
      s021: "自定义 MDX 块",
      s022: "生成交互式本地组件，将它们插入到 MDX 中，并从文档 UI 中编辑它们的 props。",
      s023: "脚本自动化",
      s024: "批量内容生成、交叉引用和发布管道。",
      s025: "自我提升",
      s026: "代理修改应用程序本身。需要新的工作流程吗？就问吧。",
      s027: "写在这里，到处发布",
      s028: "通过脚本连接到任何无头 CMS。代理处理整个工作流程。",
      s029: "代理自主运行发布脚本",
      s030: "Markdown、HTML 或您喜欢的任何格式",
      s031: "用于文件优先工作流程的可选本地 Markdown/MDX 同步",
      s032: "编辑器中的草稿内容",
      s033: "“将此发布到 WordPress”",
      s034: "代理运行发布脚本",
      s035: "Content 住在您的网站上",
      s036: "比较如何",
      s037: "编辑UI",
      s038: "饱满、坚硬",
      s039: "仅 Chat",
      s040: "齐全，可定制",
      s041: "品牌知名度",
      s042: "每次对话",
      s043: "坚持不懈，训练有素",
      s044: "本地 MDX 文件",
      s045: "某些工具中的 Markdown",
      s046: "手动复制粘贴",
      s047: "使用自定义块直接编辑文件",
      s048: "内容管理系统发布",
      s049: "单独的步骤",
      s050: "集成工作流程",
      s051: "定制化",
      s052: "仅插件",
      s053: "仅提示",
      s054: "完整源代码",
      s055: "定价",
      s056: "每个座位",
      s057: "订阅",
      s058: "免费和开源",
      s059: "几分钟内即可开始使用",
      s060: "带上您的 MDX 文档，生成交互式块，然后开始使用 AI 进行写作。",
      s061: "阅读文档",
      s062: "查看所有模板",
    },
    design: {
      s001: "Design 模板屏幕截图",
      s002: "描述",
      s003: "生成",
      s004: "精炼",
      s005: "所有模板",
      s006: "开源 AI HTML 原型工作室",
      s007: "根据提示生成交互式 Alpine/Tailwind 原型，比较变体，使用调整控件进行优化，并导出您拥有的真实文件。",
      s008: "试用",
      s009: "它是如何运作的",
      s010: "你需要的一切",
      s011: "一个原型工作室，有一个代理来编写和完善源代码。",
      s012: "完整的 HTML 原型",
      s013: "生成独立的 Alpine/Tailwind HTML，在预览 iframe 中渲染并可以直接导出。",
      s014: "变体生成",
      s015: "从多个方向入手，在应用程序中进行比较，不断完善最强的设计。",
      s016: "调整控制",
      s017: "在代理处理较大的结构和副本更改时，直观地调整常见的设计变量。",
      s018: "对话编辑",
      s019: "“使标题更大胆”、“尝试更温暖的调色板”、“添加 CTA 按钮”。代理更新底层 HTML。",
      s020: "Design 系统",
      s021: "保存可重复使用的设计系统首选项，以便新一代更接近您的产品语言。",
      s022: "导出到任何地方",
      s023: "当您准备好共享或移交时，从生成的原型中导出 HTML、ZIP 或 PDF。",
      s024: "源优先预览",
      s025: "预览是从代理编辑和导出使用的相同 HTML 渲染的，因此概念和切换之间的转换较少。",
      s026: "生成原型的 iframe 预览",
      s027: "可导出的 HTML、ZIP 和 PDF 工件",
      s028: "对话细化",
      s029: "代理编辑原型源。简单的英语说明会改变文案、布局、颜色、间距和交互。",
      s030: "“让这看起来更高级”",
      s031: "“尝试更深的调色板”",
      s032: "“让英雄布局更具编辑性”",
      s033: "“生成这个的三个变体”",
      s034: "比较如何",
      s035: "静态模型工具",
      s036: "一次性发电机",
      s037: "可视化编辑器",
      s038: "视觉优先",
      s039: "提示优先",
      s040: "代理+预览+代码",
      s041: "AI一代",
      s042: "限制/插件",
      s043: "一键提示",
      s044: "迭代式、对话式",
      s045: "可编辑的输出",
      s046: "工具原生文件",
      s047: "经常静态",
      s048: "完成 HTML/CSS/JS",
      s049: "定制化",
      s050: "仅插件",
      s051: "仅提示",
      s052: "完整源代码",
      s053: "定价",
      s054: "每席每月 $15 起",
      s055: "每幅图像的学分",
      s056: "免费和开源",
      s057: "几分钟内即可开始使用",
      s058: "分叉模板并开始使用编辑源的代理生成交互式原型。",
      s059: "阅读文档",
      s060: "查看所有模板",
    },
    dispatch: {
      s001: "Dispatch 模板屏幕截图",
      s002: "+ Telegram 支持",
      s003: "代理间",
      s004: "内存",
      s005: "周期任务",
      s006: "所有模板",
      s007: "您的代理人的大本营",
      s008: "通过 Slack、Telegram 或任何信使与您的代理交谈，它会路由到您的其他应用程序。作业、内存、批准和 A2A 授权 — 全部集中在一处。",
      s009: "试用",
      s010: "你能做什么",
      s011: "Dispatch 是整个代理原生堆栈的面向信使的前门。",
      s012: "Bidirectional messaging with thread context, Block Kit replies, and inline approvals.从任何地方通话。",
      s013: "Dispatch 通过 A2A 协议将工作路由到其他代理本机应用程序。 Mail、幻灯片、设计——所有这些都可以通过一次聊天来实现。",
      s014: "经常性工作",
      s015: "Schedule the agent to run on a cron — daily standups, weekly digests, hourly checks.结果会出现在您的信使中。",
      s016: "批准",
      s017: "在 Slack 中控制一键批准背后的敏感操作。配置每个操作：发送电子邮件、发布更新、运行自动化。",
      s018: "持久代理内存",
      s019: "Dispatch 记住跨对话、线程和通道的上下文。告诉它一次你的偏好——它就会把它们带出去。",
      s020: "从每次对话中自动捕获学习内容",
      s021: "每个用户、每个组织和全局内存范围",
      s022: "检查并编辑代理记住的内容",
      s023: "跨应用路由",
      s024: "Dispatch 是枢纽。连接其他代理本机应用程序，每个请求都会路由到可以完成该工作的代理。",
      s025: "“向设计团队发送套牌” → Slides 代理",
      s026: "“回复最新销售邮件”→Mail代理商",
      s027: "“总结本季度的事件” → Analytics 代理",
      s028: "随时随地与您的代理交谈",
      s029: "7 AM 在 Slack 上私信您的代理人，在火车上从 Telegram 进行跟进，获得 Block Kit 回复，其中包含“打开线程”按钮以跳入完整对话。",
      s030: "“明天我的日历上有什么？”",
      s031: "“为 Monday 全员创建一套套牌”",
      s032: "“每个工作日 9 AM 举行早间站立会议”",
      s033: "“部署完成时告诉我”",
      s034: "比较如何",
      s035: "已关闭 AI 助理",
      s036: "跨应用路由",
      s037: "每个机器人",
      s038: "单品",
      s039: "供应商定义",
      s040: "持久、有范围、可检查",
      s041: "自定义代码",
      s042: "有限公司",
      s043: "定制化",
      s044: "SDK管道",
      s045: "关闭",
      s046: "完整源代码",
      s047: "定价",
      s048: "每个应用程序费用",
      s049: "每席每月 $20 起",
      s050: "免费和开源",
      s051: "几分钟内即可开始使用",
      s052: "分叉模板，连接 Slack 或 Telegram，并将您的代理放入每个对话中。",
      s053: "阅读文档",
      s054: "查看所有模板",
    },
    forms: {
      s001: "Forms 模板屏幕截图",
      s002: "描述",
      s003: "生成",
      s004: "路线",
      s005: "所有模板",
      s006: "Typeform 和 Google Forms 的开源 AI 替代方案",
      s007: "根据提示生成完整表单，以对话方式优化字段，并将提交路由到 Slack、Discord、Google Sheets 或 Webhook。拥有您的数据和工作流程——无需按回复付费。",
      s008: "试用",
      s009: "它是如何运作的",
      s010: "你需要的一切",
      s011: "内置 AI 的完整表单生成器。",
      s012: "所有字段类型",
      s013: "文本、电子邮件、数字、长文本、选择、多选、复选框、单选、日期、评级和比例。",
      s014: "视野编辑",
      s015: "在编辑器中或通过询问代理来更新标签、占位符、选项、所需状态和字段顺序。",
      s016: "定制造型",
      s017: "匹配您的品牌——颜色、字体、布局。或者只是要求代理商“让它看起来像我们的营销网站”。",
      s018: "提交集成",
      s019: "当每个响应到达时，将结构化提交发送到 Slack、Discord、Google Sheets 或 Webhook。",
      s020: "公共共享页面",
      s021: "每个表单都有一个带有完整 SEO 的公共 URL。嵌入任何网站，或托管在您自己的域上。",
      s022: "提交仪表板",
      s023: "可排序的表、过滤器、CSV 导出和每次提交的详细视图。所有数据都存储在您的 SQL 数据库中。",
      s024: "视觉+对话编辑",
      s025: "拖动字段、编辑标签、设置验证。或者仅描述更改，然后代理更新相同的表单架构。",
      s026: "拖放字段重新排序",
      s027: "与编辑器一起实时预览",
      s028: "具有完整历史记录的撤消/重做",
      s029: "会话形式构建",
      s030: "跳过逐个领域的苦差事。描述一次形式，然后用通俗易懂的语言进行完善。",
      s031: "“创建工作申请表”",
      s032: "“添加所需的经验级别下拉菜单”",
      s033: "“将电子邮件字段设为必填”",
      s034: "“将每个提交内容发布到 Slack”",
      s035: "比较如何",
      s036: "AI 表单生成器",
      s037: "可视化编辑器",
      s038: "是的，模板绑定",
      s039: "有限/无",
      s040: "视觉+代理+代码",
      s041: "AI一代",
      s042: "无/基本",
      s043: "一击，僵硬",
      s044: "迭代式、对话式",
      s045: "提交集成",
      s046: "手册",
      s047: "有限公司",
      s048: "数据所有权",
      s049: "供应商的服务器",
      s050: "供应商托管",
      s051: "您的 SQL 数据库",
      s052: "定价",
      s053: "每次回复费用",
      s054: "订阅",
      s055: "免费和开源",
      s056: "几分钟内即可开始使用",
      s057: "分叉模板并开始收集您完全拥有的提交内容。",
      s058: "阅读文档",
      s059: "查看所有模板",
    },
    mail: {
      s001: "Mail 模板屏幕截图",
      s002: "键盘优先",
      s003: "收件箱分类",
      s004: "意见",
      s005: "可定制",
      s006: "所有模板",
      s007: "Superhuman 和 Gmail 的开源替代方案",
      s008: "试用",
      s009: "托管演示使用 Jami Studio 的共享 Google 应用程序进行 Gmail 访问，因此 Google 可能会要求您确认后再继续。在本地运行以使用您自己的 Google OAuth 客户端。",
      s010: "你能做什么",
      s011: "将电子邮件客户端替换为您完全拥有的 AI 驱动的收件箱所需的一切。",
      s012: "键盘快捷键",
      s013: "AI 分类",
      s014: "代理会读取您的收件箱、显示重要内容、起草回复并自动处理日常电子邮件。设置自动标记和自动归档的自动化。",
      s015: "智能搜索",
      s016: "在整个收件箱中进行自然语言搜索。 “上周关于预算的电子邮件”就有效了。",
      s017: "自我提升",
      s018: "代理修改应用程序本身。需要自定义文件夹、过滤器或自动化？就问吧。",
      s019: "AI-支持收件箱管理",
      s020: "让代理完成繁重的工作——根据自然语言指令进行分类、标签、存档和草拟响应。",
      s021: "自动标记、优先排序、自动归档规则",
      s022: "AI-起草常规消息的回复",
      s023: "主题摘要和行动项目",
      s024: "键盘优先作曲",
      s025: "以思考的速度撰写和发送电子邮件。全键盘导航、Notion 式快捷键和 AI 辅助绘图。",
      s026: "多个草稿选项卡可同时撰写多封电子邮件",
      s027: "AI 将短草稿扩展为完整的电子邮件",
      s028: "稍后发送、暂停和后续提醒",
      s029: "由代理支持的电子邮件工作流程",
      s030: "代理通过自然语言阅读、起草和组织您的电子邮件。每个工作流程都是一个可以检查和扩展的脚本。",
      s031: "“总结本周我未读的电子邮件”",
      s032: "“起草对投资者更新帖子的回复”",
      s033: "“存档 7 天以上的所有新闻通讯”",
      s034: "“查找所有等待我回复的电子邮件”",
      s035: "比较如何",
      s036: "键盘快捷键",
      s037: "基础",
      s038: "优秀",
      s039: "完全可定制",
      s040: "AI 援助",
      s041: "智能撰写",
      s042: "AI 回复",
      s043: "全程代理：分类、起草、自动化",
      s044: "定制化",
      s045: "仅设置",
      s046: "仅主题",
      s047: "完整源代码",
      s048: "数据所有权",
      s049: "Google 的服务器",
      s050: "供应商托管",
      s051: "您拥有代码",
      s052: "定价",
      s053: "免费/工作空间",
      s054: "每个用户 $30/月",
      s055: "免费和开源",
      s056: "几分钟内即可开始使用",
      s057: "分叉模板，连接您的电子邮件提供商，然后开始使用 AI 管理您的收件箱。",
      s058: "阅读文档",
      s059: "查看所有模板",
      s060: "托管演示说明",
    },
    plan: {
      s001: "计划模板截图",
      s002: "块类型",
      s003: "代理集成",
      s004: "可分享的链接",
      s005: "原型跑步者",
      s006: "添加技能",
      s007: "一个命令可将计划技能安装到 Claude Code、Codex、Pi、Cursor、OpenCode、GitHub Copilot / VS Code 和类似代理项目中。无需部署单独的应用程序。",
      s008: "代理打开计划",
      s009: "请您的代理规划一项功能。它调用 /visual-plan 并在浏览器或 VS Code 中打开该计划 - 结构化块，而不是 Markdown 墙。",
      s010: "评论与评论",
      s011: "Pin 对任何块的注释。提出问题、标记疑虑或批准部分 - 代理可以看到所有反馈。",
      s012: "代理迭代",
      s013: "代理会阅读您的评论并就地更新计划。差异准确地显示了发生了什么变化以及原因。",
      s014: "所有模板",
      s015: "Codex、Claude Code 和编码代理的视觉计划",
      s016: "通过一个命令进行安装。您的代理使用线框图、图表、带注释的代码和可共享的审阅链接打开结构化计划，而不是在终端中倾倒大量的降价内容。",
      s017: "试用",
      s018: "代理可以做什么",
      s019: "每个块类型都是一等公民 - 结构化数据，而不是原始 HTML，因此代理可以随着工作的进展读取和更新计划。",
      s020: "线框图",
      s021: "粗略的 UI 模型基于您的真实产品 - 而不是通用的桌面占位符。",
      s022: "图表",
      s023: "内嵌呈现的架构流程图、数据模型和序列图。",
      s024: "带注释的代码",
      s025: "带有每行注释、差异和更改理由的真实源文件——而不是原始代码转储。",
      s026: "可分享的链接",
      s027: "每个计划都有一个公共 URL。与团队成员共享以进行异步审阅、评论和批准。",
      s028: "桌面文件同步",
      s029: "将托管计划从 Jami Studio Desktop 镜像到本地 MDX 文件，无需克隆应用程序或运行 CLI。",
      s030: "，因此审查位于代码旁边。",
      s031: "它是如何运作的",
      s032: "规划存在于共享应用程序中 - 您和代理都可以在功能的整个生命周期中读取和更新它。",
      s033: "丰富的块库",
      s034: "计划由结构化块组成，而不是自由格式的 HTML。代理知道每个块的模式，并且可以精确地创建、更新和推理它们。",
      s035: "带注释的代码",
      s036: "插入前验证所有者",
      s037: "发出自动化事件",
      s038: "添加",
      s039: "比较如何",
      s040: "终端中的 Markdown",
      s041: "视觉渲染",
      s042: "No",
      s043: "基础",
      s044: "丰富的块、线框图、图表",
      s045: "代理可以读取和更新",
      s046: "是的，原始文本",
      s047: "有限公司",
      s048: "是的，结构化模式",
      s049: "可分享的链接",
      s050: "是",
      s051: "是的，有评论",
      s052: "实时 Alpine.js 沙箱",
      s053: "适用于 Codex / Claude Code / Pi",
      s054: "是的，一命令安装",
      s055: "开源",
      s056: "是的，MIT 已获得许可",
      s057: "几秒钟内即可开始",
      s058: "一个命令可为 Claude Code、Codex、Pi、Cursor、OpenCode、GitHub Copilot / VS Code 和类似代理项目添加视觉规划。无需单独部署。",
      s059: "阅读文档",
      s060: "查看所有模板",
      s061: "VS Code 交接",
      s062: "使用以下扩展在 VS Code 侧边面板中打开计划链接：",
      s063: "Jami Studio Plans 扩展",
      s064: "线框图 — 带组件插槽的草图式 UI 模型",
      s065: "带注释的代码 — 带逐行注释的源文件",
      s066: "图表 — 流程图、序列图或架构图",
      s067: "原型 — iframe 中的实时 Alpine.js 沙箱",
      s068: "决策 — 带理由的已确定选择",
      s069: "API 端点 — 方法、路径和请求/响应类型",
      s070: "数据模型 — 带字段注释的架构",
      s071: "文件树 — 带路径注释的项目结构",
      s072: "// 示例计划块",
      s073: "ChatGPT Canvas 与 Notion",
      s074: "不适用",
    },
    slides: {
      s001: "Slides 模板屏幕截图",
      s002: "描述",
      s003: "生成",
      s004: "精炼",
      s005: "所有模板",
      s006: "PowerPoint 和 Canva 的开源 AI 替代品",
      s007: "根据提示生成完整的套牌，然后通过对话方式进行完善或进行可视化编辑。",
      s008: "试用",
      s009: "它是如何运作的",
      s010: "你需要的一切",
      s011: "内置 AI 的完整演示工作室。",
      s012: "8 种幻灯片布局",
      s013: "标题、部分、内容、两栏、图像、声明、全屏和空白。",
      s014: "视觉+代码编辑",
      s015: "单击可编辑样式，双击可编辑文本。切换到原始 HTML 以实现完全控制。",
      s016: "AI 图像生成",
      s017: "使用 Gemini 生成图像。品牌一致性的风格参考。 3 种变体可供选择。",
      s018: "标志和图像搜索",
      s019: "通过 Logo.dev 或 Brandfetch 搜索公司徽标。 Google 库存照片图像。",
      s020: "拖放重新排序",
      s021: "对侧栏中的幻灯片重新排序。使用悬停操作复制或删除。",
      s022: "演示模式",
      s023: "全屏显示键盘导航、自动隐藏控件和演讲者备注。",
      s024: "分享与协作",
      s025: "生成用于只读演示文稿访问的共享链接。带有标记条目的完整撤消/重做历史记录。",
      s026: "共享具有只读访问权限的链接",
      s027: "Cmd+Z 具有完整历史记录的撤消/重做",
      s028: "导航至历史上的任意点",
      s029: "对话细化",
      s030: "代理直接编辑幻灯片，UI 通过轮询同步刷新。",
      s031: "“把标题放大一点”",
      s032: "“在幻灯片 3 上添加图表”",
      s033: "“将配色方案更改为蓝色”",
      s034: "“为幻灯片 5 添加演讲者注释”",
      s035: "比较如何",
      s036: "AI 幻灯片生成器",
      s037: "可视化编辑器",
      s038: "是的，模板绑定",
      s039: "有限/无",
      s040: "视觉+代码+代理",
      s041: "AI一代",
      s042: "基本/无",
      s043: "一击，僵硬",
      s044: "迭代式、对话式",
      s045: "图像生成",
      s046: "基础",
      s047: "定制化",
      s048: "仅主题",
      s049: "仅提示",
      s050: "完整源代码",
      s051: "定价",
      s052: "免费/每个座位",
      s053: "订阅",
      s054: "免费和开源",
      s055: "几分钟内即可开始使用",
      s056: "分叉模板并开始使用 AI 创建演示文稿。",
      s057: "阅读文档",
      s058: "查看所有模板",
    },
  },
  skillsPage: {
    metaTitle:
      "代理技能 - 面向编码代理的 Visual Plan、Visual Recap 和 Visual Edit",
    metaDescription:
      "安装由 Jami Studio 应用支持的技能，让你的编码代理能进行可视化规划、PR 回顾和本地可视化编辑。",
    metaOgDescription:
      "为你的编码代理添加由 Jami Studio 应用驱动的斜杠命令，可托管、检查并自定义。",
    metaKeywords:
      "代理技能, visual plan, visual recap, visual edit, 编码代理, Claude Code, Codex, PR 审查, 规划, Jami Studio",
    heroTitle: "给你的编码代理新的超能力",
    heroBody:
      "安装由 Jami Studio 应用支持、可完全自定义的应用技能：实现前做可视化规划，变更落地后做可视化 PR 回顾，并可视化编辑本地 UI 流程。",
    sectionTitle: "面向编码代理的应用支持技能",
    sectionBody:
      "使用托管的可分享应用链接、本地文件，或自托管/自定义应用；代理会获得指令，并在需要时获得匹配的 MCP 表面。",
    ctaBody:
      "适用于 Claude Code、Codex、Cursor、Pi、OpenCode、GitHub Copilot / VS Code 以及类似编码代理。",
    readVisualPlansDocs: "阅读 Visual Plans 文档",
    browseTemplates: "浏览模板",
    visualPlan: {
      name: "可视化计划",
      tagline: "写代码前先审阅",
      description:
        "把编码任务转成可分享的计划，包含图表、文件说明和可选 UI 草图。",
      feature1: "在变更落地前看清实现形状",
      feature2: "评论、修订、批准或交接",
      videoAriaLabel: "Visual Plan 技能演示视频",
    },
    visualRecap: {
      name: "可视化回顾",
      tagline: "变更后再审阅",
      description:
        "把 PR 或 git diff 转成可分享的变更回顾，说明改了什么以及为什么。",
      feature1: "总结 schema、API 和文件变更",
      feature2: "可选择发布一条固定的 PR 评论",
      videoAriaLabel: "Visual Recap 技能演示视频",
    },
    visualEdit: {
      name: "可视化编辑",
      tagline: "编辑本地 UI 流程",
      description:
        "把正在运行的 localhost 应用作为 URL 支撑的屏幕打开到 Design 中，用于可视化审阅和编辑。",
      feature1: "从有序 URL 布局多屏流程",
      feature2: "检查、复制并优化实时路由状态",
      videoAriaLabel: "Visual Edit 技能演示视频",
    },
  },
  downloadPage: {
    title: "下载 Jami Studio",
    body: "所有 Jami Studio 应用都在一个桌面外壳中。内置生产应用，并提供用于本地开发的开发模式开关。",
    openDesktop: "打开 Jami Studio",
    downloadInstaller: "下载安装程序",
    viewInstallers: "查看安装程序",
    viewInstallersOnGithub: "在 GitHub 查看安装程序",
    latestRelease: "Latest desktop release: {{version}}",
    loadError: "无法加载最新桌面版。releases 页面包含所有安装程序。",
    checkingRelease: "正在检查最新桌面版...",
    runFromSource: "或从源码运行",
    runFromSourceBody:
      "还没有适用于你平台的安装程序，或更喜欢 CLI？使用 npm 创建新应用并在本地运行；支持 macOS、Windows 和 Linux。",
    viewAllReleases: "在 GitHub 查看所有 releases",
    platforms: {
      mac: {
        primary: "下载 Apple Silicon 版本",
        alternative: "Intel Mac 版本",
      },
      windows: {
        primary: "下载 Windows 版本",
        alternative: "ARM64",
        note: "Windows 10 或更高版本。",
      },
      linux: {
        primary: "下载 Linux 归档包",
        appImage: "下载 AppImage",
        deb: "下载 .deb",
        note: "归档包无需 FUSE 即可使用。某些发行版上的 AppImage 可能需要 FUSE 2。",
      },
    },
  },
  brandPage: {
    eyebrow: "品牌资源",
    title: "Jami Studio 标志",
    body: "下载用于文章、演示文稿和社区项目的官方标志与图形。SVG 文件在任何尺寸下都保持清晰，并支持透明背景。",
    horizontal: {
      title: "横版标志",
      body: "推荐用于文章标题、网站、演示文稿及其他横向版面。",
    },
    symbol: {
      title: "图形标记",
      body: "独立图形适用于头像、应用图标和紧凑版面。",
    },
    lightBackground: "用于浅色背景",
    darkBackground: "用于深色背景",
    downloadSvg: "下载 SVG",
    usage: {
      title: "品牌使用规范",
      clear: {
        title: "保持清晰",
        body: "保持标志的原始比例，并在周围留出足够空间。",
      },
      contrast: {
        title: "确保对比度",
        body: "浅色背景使用深色版本，深色背景使用浅色版本。",
      },
      original: {
        title: "使用原始素材",
        body: "请勿改色、裁剪、旋转、拉伸或重新排列标志和图形。",
      },
    },
  },
  legal: {
    lastUpdated: "上次更新：{{date}}",
    privacy: {
      eyebrow: "隐私政策",
      title: "Jami Studio 托管应用程序",
      intro:
        "本政策解释了 Jami Studio 在运行 Jami Studio 托管应用程序、托管模板、演示和官方浏览器扩展时如何收集、使用、共享和保留数据。",
      scopeCards: {
        hosted: {
          title: "托管应用程序",
          body: "当 Jami Studio 为您运营 Jami Studio 服务或托管模板时涵盖。",
        },
        openSource: {
          title: "开源",
          body: "不包括您对 MIT 许可的源代码本身的使用。",
        },
        selfHosted: {
          title: "自托管",
          body: "不包括由其他人操作的分叉、自定义或部署。",
        },
      },
      sections: {
        scope: "适用范围",
        information: "我们收集的信息",
        clipsExtension: "Jami Studio Clips Chrome 扩展",
        use: "我们如何使用信息",
        sharing: "共享和第三方",
        chromeLimitedUse: "Chrome Web Store 限制使用",
        retention: "保留和删除",
        security: "安全性",
        changes: "变更及联系方式",
      },
      paragraphs: {
        scope1:
          "Jami Studio 是开源的，源代码可在 MIT 许可证下获得。本政策仅适用于 Jami Studio 为 Jami Studio 用户运营的托管应用程序和服务。它不适用于其他人对代码的使用，包括分叉、自定义模板、私有部署或自托管版本。如果您操作自己的部署，则您应对自己的数据实践和隐私政策负责。",
        scope2Prefix: "本政策旨在补充 Jami Studio 更广泛的政策",
        scope2Suffix: "对于 Jami Studio 托管应用程序行为。",
        clips1:
          "Jami Studio Clips Chrome extension 可帮助您开始基于浏览器的录制，并在启用后将浏览器诊断附加到剪辑。它可能会收集所选的捕获源、您选择包含的摄像头和麦克风媒体、活动选项卡标题和 URL，以及将扩展程序连接到托管 Clips 所需的身份验证状态。",
        clips2:
          "开发者日志是可选的。启用后，扩展程序可能会在记录处于活动状态时从所选选项卡收集经过编辑的控制台消息、JavaScript 异常和 fetch/XHR 元数据，例如方法、URL、状态、计时和失败详细信息。该扩展程序并非旨在收集请求正文、响应正文、cookie 或授权标头。",
        clipsAnchor:
          "对于 Chrome Web Store 披露，请使用此部分作为扩展隐私政策锚点：",
        sharing1:
          "我们不会出售 Jami Studio 托管应用程序数据或将其用于第三方广告。当您使用的功能需要这些服务时，我们会与帮助运营托管服务的服务提供商共享数据，例如云基础设施、存储、身份验证、电子邮件、可观察性、AI 和转录提供商。",
        sharing2:
          "当您连接集成时，托管应用程序可能会根据您的配置和提供商自己的条款向该提供商发送数据或从该提供商接收数据。为了安全、防止滥用、遵守法律或保护用户和服务，我们也可能会在需要时披露信息。",
        chromeLimitedUse:
          "对于 Jami Studio Clips Chrome extension，我们对从 Chrome extension APIs 收到的信息的使用遵守 Chrome Web Store 用户数据政策，包括限制使用要求。扩展程序收集的浏览器活动用于提供面向用户的记录和诊断工作流程，而不是用于广告、转售、信誉或不相关的分析。",
        retention1:
          "我们会根据需要保留托管应用程序数据，以提供服务、维护工作区历史记录、遵守义务、解决争议或提高可靠性和安全性。用户可以通过相关的应用程序控件（如果可用）删除剪辑、文档、资源和其他托管应用程序内容。",
        retention2:
          "删除的内容可能会在备份、日志或审核记录中保留一段有限的时间，然后根据操作保留计划将其删除。",
        security:
          "我们使用合理的管理、技术和组织保障措施来保护托管应用程序数据，包括访问控制、传输加密、监控和操作安全实践。没有任何在线服务可以保证完美的安全性，因此用户应避免在录音或提示中包含秘密或敏感信息，除非他们打算与托管应用程序共享该信息。",
        changes1:
          "随着 Jami Studio 托管应用程序的变化，我们可能会更新此政策。页面顶部的更新日期显示该政策上次修订的时间。",
        changes2Prefix:
          "对于隐私请求或疑问，请通过以下列出的支持和隐私渠道联系 Jami Studio",
      },
      dataCategories: {
        account: {
          title: "帐户和工作区信息",
          body: "用于登录并使托管工作区分开的姓名、电子邮件地址、组织成员身份、身份验证标识符和应用程序设置。",
        },
        hostedContent: {
          title: "托管应用程序内容",
          body: "Content 您在托管 Jami Studio 模板中创建或上传，例如录音、成绩单、文档、评论、任务、提示、代理响应、文件和配置。",
        },
        integrations: {
          title: "连接的集成数据",
          body: "来自您选择连接的服务的数据，例如日历、Slack、电子邮件、存储或开发人员工具，仅限于托管应用程序中显示的范围和工作流程。",
        },
        usage: {
          title: "用途和技术数据",
          body: "用于操作、保护和改进托管服务的设备、浏览器、IP 地址、诊断日志、页面和功能使用情况、错误和安全事件。",
        },
      },
      uses: {
        provide:
          "提供、同步和操作托管的 Jami Studio 应用程序及其代理工作流程。",
        transform:
          "当您要求托管应用程序执行此操作时，可以记录、转录、总结、搜索、共享或转换内容。",
        auth: "验证用户身份、管理组织、实施访问控制并防止滥用。",
        support: "调试事件、提供支持、衡量可靠性并改善托管产品体验。",
        comply: "遵守法律、安全和平台义务。",
      },
      links: {
        builderPrivacy: "隐私政策",
        builderPrivacyFull: "Jami Studio 隐私政策",
      },
    },
    terms: {
      eyebrow: "服务条款",
      title: "Jami Studio 托管应用程序",
      intro:
        "当 Jami Studio 为您运营 Jami Studio 托管应用程序、托管模板、演示和官方托管服务时，这些条款适用。",
      scopeCards: {
        hosted: {
          title: "托管应用程序",
          body: "当您使用由 Jami Studio 操作的 Jami Studio 应用程序或模板时，将受到保护。",
        },
        openSource: {
          title: "开源",
          body: "MIT 许可的源代码在其开源许可证下仍然可用。",
        },
        selfHosted: {
          title: "自托管",
          body: "由您或其他人操作的单独部署不是 Jami Studio 托管服务。",
        },
      },
      sections: {
        scope: "范围和相关术语",
        hostedService: "托管服务",
        accounts: "帐户和工作区",
        content: "您的内容和权限",
        agents: "代理、AI 输出和集成",
        acceptableUse: "可接受的使用",
        openSource: "开源和自托管",
        suspension: "暂停和终止",
        disclaimers: "免责声明和责任",
        changes: "变更及联系方式",
      },
      paragraphs: {
        scope1:
          "Jami Studio 是开源的，其源代码可在 MIT 许可证下获得。这些条款仅适用于 Jami Studio 为 Jami Studio 用户运营的托管应用程序和服务。它们不管理在 Jami Studio 之外运行的分叉、自定义模板、私有部署或自托管版本。",
        scope2Prefix: "这些术语补充了 Jami Studio 的更广泛范围",
        scope2Middle: "和 Jami Studio",
        scope2Suffix:
          "如果您代表公司或组织使用托管 Jami Studio 应用程序，则表示您有权接受该组织的这些条款。",
        hostedService:
          "Jami Studio 可以提供托管的 Jami Studio 应用程序、模板、演示、共享工作区、浏览器扩展和相关代理工作流程。随着产品的发展，托管服务可能会更新、限制、暂停或终止。",
        accounts1:
          "您对帐户信息的准确性、帐户下的活动以及保证凭据的安全负责。托管 Jami Studio 应用程序可能包括组织功能、邀请、共享资源、连接集成和特定于应用程序的访问控制。仅邀请用户并连接您有权使用的服务。",
        accounts2:
          "如果您认为帐户、工作区、集成或共享资源已被泄露或滥用，请立即联系 Jami Studio 支持人员。",
        content1:
          "您保留对您创建、上传、记录、导入或连接到托管 Jami Studio 应用程序的内容的所有权。您授予 Jami Studio 托管、处理、传输、显示、转换、分析和存储该内容所需的有限权限，以便托管应用程序及其代理工作流程可以运行。",
        content2:
          "您有责任拥有您向服务提供的内容、录音、提示、文件、凭据和连接的集成数据所需的权利和权限。",
        agents1:
          "托管 Jami Studio 应用程序可以根据您的要求运行 AI 代理、工具、自动化和提供商集成。 AI 生成的输出可能不完整、不准确或不适合特定用途。在依赖重要的输出、操作、导出和消息之前先对其进行审查。",
        agents2:
          "当您连接第三方服务时，您对这些服务的使用仍受其自身的条款、限制、权限和隐私惯例的约束。",
        openSource:
          "这些条款不会更改 Jami Studio 代码的开源许可。如果您下载、分叉、修改或自托管 Jami Studio，则 MIT 许可证和您为自己的部署设置的条款将管辖该使用。您负责您所操作的部署的安全性、隐私性、合规性、操作和用户支持。",
        suspensionPrefix:
          "当需要保护用户、遵守法律、防止滥用、解决安全风险或运营服务时，Jami Studio 可以暂停或限制对托管 Jami Studio 服务的访问。您可以随时停止使用托管服务。某些数据可能会在备份、日志或审核记录中保留一段有限的时间，如",
        disclaimers1:
          "托管 Jami Studio 服务按原样和可用状态提供，并遵守适用法律以及您与 Jami Studio 签订的任何单独书面协议。 Jami Studio 不保证托管应用程序、集成、自动化或 AI 输出不会中断、无错误或满足所有要求。",
        disclaimers2Prefix:
          "在法律允许的最大范围内，Jami Studio 对托管 Jami Studio 服务的责任受到 Jami Studio 更广泛的条款中所述的限制。",
        disclaimers2Suffix: "或适用于您的使用的其他书面协议。",
        changes1:
          "随着 Jami Studio 托管应用程序的变化，我们可能会更新这些条款。页面顶部的更新日期显示了条款的最新修订时间。",
        changes2Prefix:
          "如果对这些条款有疑问，请通过 Jami Studio 中列出的支持渠道联系 Jami Studio",
      },
      hostedServicePoints: {
        create: "创建和操作托管 Jami Studio 工作区和模板应用程序。",
        workflows: "运行您选择使用的代理工作流程、操作、自动化和集成。",
        store:
          "存储提供服务所需的托管应用程序内容、设置、组织数据和连接帐户状态。",
        improve: "测量、保护、调试和改进托管 Jami Studio 服务。",
      },
      acceptableUse: {
        laws: "请勿使用托管 Jami Studio 应用程序来违反法律、侵犯权利或伤害人员或系统。",
        bypass: "不要尝试绕过访问控制、速率限制、安全边界或租户隔离。",
        malware: "请勿上传恶意软件、凭证盗窃材料或旨在破坏服务的内容。",
        spam: "请勿使用该服务发送垃圾邮件、未经授权进行抓取或滥用连接的提供商。",
        sensitive:
          "除非您获得授权并且该应用程序适合该用途，否则请勿将机密或敏感的受监管数据放入托管应用程序中。",
      },
      links: {
        builderTerms: "服务条款",
        privacyPolicy: "隐私政策",
      },
    },
  },
  nav: {
    overview: "概览",
    coreArchitecture: "核心架构",
    dataAuthGovernance: "数据、认证与治理",
    usingYourAgent: "使用你的 Agent",
    workspace: "工作区",
    integrations: "集成",
    buildApps: "构建应用",
    advancedRuntime: "高级：扩展运行时",
    templatesSection: "应用",
    gettingStarted: "入门",
    whatIsAgentNative: "什么是 Jami Studio？",
    agentSurfaces: "Agent 界面",
    keyConcepts: "核心概念",
    agentNativeToolkit: "工具包",
    toolkitOverview: "概览",
    toolkitUiPrimitives: "UI 原语",
    toolkitSharing: "共享",
    toolkitCollaboration: "协作",
    toolkitSettings: "设置",
    toolkitOrgTeam: "组织与团队",
    toolkitSetupConnections: "设置与连接",
    toolkitCommandNavigation: "命令与导航",
    toolkitResources: "资源",
    toolkitAgentUx: "智能体体验",
    toolkitHistory: "历史",
    toolkitCommentsReview: "评论与审核",
    toolkitObservability: "可观测性",
    featureKits: "功能套件",
    appChrome: "应用外壳",
    templatesOverview: "模板",
    pureAgentApps: "自动化优先应用",
    faq: "常见问题",
    server: "服务器",
    client: "客户端",
    routing: "路由",
    actions: "行动",
    humanApproval: "人工审批",
    publicAgentWeb: "公共 Agent Web",
    database: "数据库",
    internationalization: "国际化",
    localFileMode: "本地文件模式",
    fileUploads: "文件上传",
    deployment: "部署",
    progress: "进度",
    authentication: "认证",
    multiTenancy: "多租户",
    securityDataScoping: "安全与数据作用域",
    sharingPrivacy: "共享与隐私",
    trackingAnalytics: "跟踪与分析",
    auditLog: "审计日志",
    doctorCodeChecks: "Doctor（代码检查）",
    observability: "可观测性",
    observationalMemory: "观测记忆",
    ciEvalGate: "CI 评测门禁",
    usingYourAgentOverview: "概览",
    contextAwareness: "上下文感知",
    agentMentions: "Agent 提及",
    voiceInput: "语音输入",
    dropInAgent: "嵌入式 Agent",
    componentApi: "组件 API",
    nativeChatUi: "原生聊天 UI",
    generativeUi: "生成式 UI",
    realTimeCollaboration: "实时协作",
    workspaceOverview: "工作区概览",
    skills: "技能",
    customAgentsTeams: "自定义 Agents 与团队",
    workspaceGovernance: "工作区治理",
    recurringJobs: "定期任务",
    automations: "自动化",
    extensions: "扩展",
    dataPrograms: "数据程序",
    multiAppWorkspaces: "多应用工作区",
    onboardingApiKeys: "入门与 API 密钥",
    messaging: "消息（Slack、Email...）",
    dispatch: "调度",
    a2aProtocol: "A2A 协议",
    mcpClients: "MCP 客户端（添加工具）",
    mcpServer: "MCP 服务器（暴露你的应用）",
    externalAgents: "外部 Agents（连接主机）",
    mcpApps: "MCP Apps（内联 UI）",
    crossAppSso: "跨应用 SSO",
    notifications: "通知",
    automationConnectors: "工作流连接器",
    workspaceConnections: "工作区连接",
    creatingTemplates: "创建模板",
    writingAgentInstructions: "编写 Agent 指令",
    embeddingSdk: "嵌入 SDK",
    frames: "Frames",
    agentNativeCodeUi: "Jami Studio 代码 UI",
    harnessAgents: "Harness 代理",
    adapters: "适配器",
    cliAdapters: "CLI 适配器",
    processors: "循环内处理器",
    durableResume: "持久恢复",
    durableBackgroundRuns: "持久后台运行",
    blueprintInstaller: "Blueprint 安装器",
    chat: "聊天",
    calendar: "日历",
    content: "内容",
    plans: "Plans",
    visualPlans: "可视化计划",
    prVisualRecap: "PR 可视化回顾",
    planPluginMarketplace: "Plan 插件与市场",
    slides: "幻灯片",
    analytics: "分析",
    mail: "邮件",
    clips: "剪辑",
    brain: "知识库",
    assets: "资产",
    design: "设计",
    forms: "表单",
  },
} satisfies typeof enUS;

export default zhCN;
