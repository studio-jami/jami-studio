 ...
 *
 * Provisions a full-app branch (one branch per design) in the configured
 * compatibility project and hands the user's prompt to the active runtime
 * agent to scaffold the app. The branch/container is not
 * necessarily ready immediately — call `sync-fusion-app` to poll the
 * container and pick up the preview URL once it boots.
 *
 * Gated behind the full-app-building runtime feature flag. When the active runtime is not configured
...
export default defineAction({
  description:
    "Create a brand-new full-app design: provisions a Jami Studio runtime " +
    "branch and hands the prompt to the active app agent to scaffold a real " +
    "running app. Use this when the user wants a real app (not an HTML " +
    "prototype) built from scratch for a design. Requires the Jami Studio runtime to be " +
  ...
  schema: z.object({
    ...
    branchName: z
      ...
      .optional()
      .describe(
        "Optional branch name for the runtime app agent to use. If omitted, it generates one.",
      ),
  }),
