...
> - **Jami Studio-hosted branch + `fusion` first; our own durable container later.**
...
### Tier B — Real app (localhost **or** fusion / Jami Studio)
...
- **`fusion`** _(the new hosted tier)_ — connect Jami Studio and run the app as a
  Jami Studio-hosted branch (today) or, later, our own durable container. Same
...
feature behind a Connect-Jami Studio CTA. _Motion crosses into Alpine because we make
it CSS-first._

---

## 3. The "Make it real" upgrade flow (Connect Jami Studio)
...
> **Make this a real app** — connect Jami Studio to unlock components, props, data
...
"connect-builder-card"`, `{ configured, builderEnabled, connectUrl }`) runs the
   Jami Studio OAuth / `cli-auth` flow and stores a scoped credential
...
2. **Migrate** _(NEW)_ — there is no inline→React migration today. Feed the
   ...
   seed to the existing Jami Studio cloud agent (`runBuilderAgent()` via
...
3. **Provision** _(exists; today = Jami Studio-hosted branch)_ —
   `startWorkspaceAppCreation()` returns `{ branchName, url, status }`: a GitHub
   branch in Jami Studio's hosting running as a full React app, tracked pending until
...
4. **Reconnect** _(NEW — implement `fusion`)_ — flip the design's `sourceType`
   ...
   stub to fill in). `fusion` points the editor at the Jami Studio-hosted app's dev URL
   and uses the bridge for snapshot + code context. Gated panels light up.
5. **Deploy** _(exists)_ — merge the Jami Studio branch → the existing template deploy
   path (`packages/core/src/deploy/build.ts`, Netlify / CF Pages).

### 3.2 The three genuinely-new pieces (everything else is reuse)

1. **Migration**: inline HTML/Alpine + tokens → a real React seed for the Jami Studio
   cloud agent.
2. **`fusion` source mode**: bridge + rendering so the editor drives a
   Jami Studio-hosted app the same way it drives an inline iframe.
...
- **Jami Studio & app-creation** — `connect-builder` card, `runBuilderAgent()`,
...
- **Jami Studio/branch:** `connect-builder-app`, `migrate-inline-design-to-app`,
...
type DesignNavigationState = {
  ...
  designId: string;
  sourceType: "inline" | "localhost" | "fusion"; // Jami Studio is a flavor of fusion
  screenId?: string;
  fileId?: string;
  route?: string;
...
motion edits safely, use review outputs, and offer Jami Studio migration CTAs.
...
- **Security** — Jami Studio credentials via Vault + scoped reader (never copied into
...
5. **Connect Jami Studio / real-app tier.** Harden bridge writes, implement `fusion`,
...
- **Agent parity:** agent can inspect the selected component via `view-screen`,
  ...
  an affected surface.
- **Real-app smoke (Jami Studio/React fixture):** select a React component → props +
...
- **Container assumption** — the real-app tier is a Jami Studio-hosted branch today, not
...
3. **Real-app hosting** — Jami Studio-hosted branch **[recommended for v1; exists]** vs.
...
4. **Migration fidelity** — pixel-match gate vs. "good enough + agent cleanup"
   before cutover. Affects how aggressive the Connect-Jami Studio CTA can be.
