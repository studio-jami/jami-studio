const messages = {
  ...
  builderCredits: {
    pausedTitle: "Jami Studio 额度已暂停",
    transcriptionDescription:
      "升级 Jami Studio 即可恢复备份转录。现有的原生转录仍会显示。",
    cleanupDescription:
      "升级 Jami Studio 即可恢复转录清理、摘要和 AI 标题。原生转录已保留。",
    settingsDescription:
      "升级 Jami Studio 即可恢复备份转录、转录清理、摘要和 AI 标题。现有的原生转录仍会显示。",
    titleDescription: "AI 标题生成已暂停。你的转录仍可使用。",
    upgrade: "升级 Jami Studio",
  ...
  recordingPage: {
    ...
    finishBuilderOrS3: "完成 Jami Studio 弹出窗口或配置 S3 存储，然后重试。",
    ...
    loomSourcePreserved:
      "Loom 源链接被保留。连接存储：Jami Studio（免费套餐存储 + AI）或 S3 兼容存储，Clips 将重试保存自己的副本。",
    clipDataPreserved:
      "Your clip data is still preserved.连接Jami Studio或S3存储，Clips将自动上传。",
    details: "细节",
  ...
  sharePage: {
    ...
    loomPreservedManage:
      "Loom 源链接被保留。连接存储：Jami Studio（免费套餐存储 + AI）或 S3 兼容存储，然后重试导入。",
    videoPreservedManage:
      "视频被保留。连接Jami Studio或S3存储，Clips将完成上传。",
    creatorNeedsStorage:
      "创建者需要先连接存储才能完成此剪辑：Jami Studio（免费套餐存储 + AI）或 S3 兼容存储。",
    signInStorage:
      "如果这是您的剪辑，请在此处登录以连接存储：Jami Studio（免费套餐存储 + AI）或 S3 兼容存储，并完成上传。",
  ...
  transcriptPanel: {
    ...
    providerNeedsAttentionDescription:
      "当前提供商已暂停此请求。切换到 Jami Studio 或更新密钥，然后重试。",
    ...
    cleanupBuilderFailed:
      "即使 Jami Studio 已连接，清理也无法完成。保留了本机转录本。",
    cleanupPaused:
      "清理已暂停。请在“设置”中连接 AI：Jami Studio（免费额度）或你自己的 LLM 密钥。",
    builderNoResponse: "没有收到Jami Studio的回复。允许弹出窗口并重试。",
...
- 现在可以直接在 Clips 中查看最新变化。更新日志已加入命令菜单（Cmd+K）和设置。`,
    ...
    apiSetup: "AI 设置",
    apiSetupDescription:
      "使用 Jami Studio 免费额度或你自己的 LLM 密钥连接 AI。",
    builderEasySetup: "Jami Studio 免费额度",
    builderAiAvailable: "Clips 可使用包含的 AI 额度和托管转录。",
    builderAiDescription:
      "优先使用 Jami Studio，可获得内置 AI 额度、对象存储、上传和托管转录。",
  ...
  stitchManager: {
    ...
    connectStorage:
      "拼接录制前请连接存储：Jami Studio（免费套餐存储 + AI）或 S3 兼容存储。",
    created: "拼接录制已创建",
    failed: "拼接录制失败",
  ...
  storageSetup: {
    builderTimeout: "5 分钟内未收到 Jami Studio 响应。请检查弹出窗口并重试。",
    builderConnected: "Jami Studio 已连接",
    ...
    whyPrompt: "为什么会看到这个？",
    whyDescription:
      "Clips 是 100% 免费且开源的应用，所以你需要连接一种方式来存储剪辑。使用 Jami Studio 可获得免费套餐存储和 AI，或使用 S3 兼容对象存储和你自己的 LLM 密钥。",
  },
  captureInstall: {
  ...
  recordRoute: {
    ...
    connectStorageToFinish:
      "在下一屏连接存储：Jami Studio（免费套餐存储 + AI）或 S3 兼容存储。Clips 将完成保存。",
    connectStorageToRetryLoom:
      "在下一屏连接存储：Jami Studio（免费套餐存储 + AI）或 S3 兼容存储。Clips 将重试导入。",
  },
  dictateRoute: {
